...
test("metric name catalogs expose canonical names", () => {
  ...
  assert.equal(typedOpenOptions.maxParallelDecodeRankTasks, 1);
...
test("open rejects invalid flow-control caps before native I/O", async () => {
  ...
  await assert.rejects(
    async () => open("unused", { maxParallelDecodeRankTasks: 0 }),
    /max_parallel_decode_rank_tasks must be greater than zero/,
  );
...
test("vectorDistance exposes dense metric catalog", () => {
  assert.equal(Math.abs(vectorDistance("minkowski:3", [0, 0], [1, 2]) - Math.cbrt(9)) < 1e-6, true);
  assert.equal(vectorDistance("cosine", [1, 0], [1, 0]), 0);
  assert.equal(Math.abs(vectorDistance("gower", [1, 2, 0, 4], [1, 4, 3, 0]) - 2.25) < 1e-6, true);
  assert.equal(
  ...
  assert.equal(Math.abs(vectorDistance("squared-chord", [1, 4], [4, 1]) - 2) < 1e-6, true);
...
test("recallAtK measures top-k overlap", () => {
  assert.equal(
    Math.abs(
      recallAtK(["doc-a", "doc-b", "doc-c", "doc-d"], ["doc-c", "doc-x", "doc-a", "doc-a"], 3) -
        2 / 3,
    ) < 1e-6,
    true,
  );
...
test("tieAwareRecallAtK counts equal-distance hits without ids", () => {
  ...
  assert.equal(Math.abs(tieAwareRecallAtK([0, 0, 0.2], [0, 0.2, 0.3], 3) - 2 / 3) < 1e-6, true);
...
test("create rejects non-integer layout options", async () => {
  for (const [options, expected] of [
    ...
    [{ graphNeighbors: 1.5 }, /graph_neighbors must be an integer when set/],
  ] as const) {
...
test("searchVectors and getVector return stored vectors", async () => {
  ...
  assert.deepEqual(await index.searchVectors([0.8, 0], { k: 2 }), [
    [1, 0],
    [0, 0],
  ]);
...
test("search buffer variants accept contiguous Float32Array query", async () => {
  ...
  assert.deepEqual(await index.searchVectorsBuffer(new Float32Array([0.8, 0]), { k: 2 }), [
    [1, 0],
    [0, 0],
  ]);
});
...
test("addBuffer accepts contiguous Float32Array rows", async () => {
  ...
  await index.addBuffer(new Float32Array([0, 0, 1, 0, 9, 0]), { ids: ["a", "b", "c"] });
...
test("search batch variants preserve query order", async () => {
  ...
  assert.deepEqual(
    await index.searchVectorsBatch(
      [
        [0.1, 0],
        [9.9, 0],
      ],
...
test("search batch buffer variants accept contiguous Float32Array rows", async () => {
  ...
  assert.deepEqual(await index.searchIdsBatchBuffer(new Float32Array([0.1, 0, 9.9, 0]), { k: 1 }), [
    ["left"],
    ["right"],
  ]);
  assert.deepEqual(
    await index.searchVectorsBatchBuffer(new Float32Array([0.1, 0, 9.9, 0]), { k: 1 }),
    [[[0, 0]], [[10, 0]]],
  );
});
...
test("searchBatchWithReport preserves query order and counters", async () => {
  ...
  assert.deepEqual(
    reports.map((report) => report.hits[0]?.id),
    ["left", "right"],
  );
  assert.deepEqual(
    reports.map((report) => report.segmentsTotal),
    [3, 3],
  );
...
test("create and open accept numeric ramBudget byte counts", async () => {
  const dir = mkdtempSync(join(tmpdir(), "borsuk-ts-"));
  const index = await create({
    uri: localUri(dir),
    metric: "euclidean",
...
test("stats propagates corrupt paged routing metadata", async () => {
  ...
  await assert.rejects(() => reopened.stats(), /parquet|routing layer page index/i);
});
...
test("search reports recall guarantee and guaranteed option errors are typed", async () => {
  ...
  await assert.rejects(
    ...
    (error) => error instanceof BorsukError && error.code === "recall_guarantee_violated",
  );
});
...
test("approx search rejects invalid budgets", async () => {
  ...
  for (const [options, expected] of [
    ...
    [{ maxCandidatesPerSegment: 1.5 }, /max_candidates_per_segment must be an integer when set/],
  ] as const) {
    await assert.rejects(
      () =>
        index.searchWithReport([0, 0], {
...
test("search rejects zero k", async () => {
  ...
  await assert.rejects(() => index.searchIds([0, 0], { k: Number.NaN }), /k must be an integer/);
  await assert.rejects(
