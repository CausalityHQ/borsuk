...
index.add(embed(chunks), ids=[str(i) for i in range(len(chunks))],
...
for question in sys.stdin:
    hits = index.search_with_report(embed([question])[0], k=4, include_metadata=True).hits
    context = "\n\n".join(hit.metadata["text"] for hit in hits)
    reply = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[{"role": "user", "content": f"Answer only from this context:\n{context}\n\nQuestion: {question}"}],
    )
