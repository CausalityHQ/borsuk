...
- [ ] **Step 1: Strengthen the exact-global scorer contract for external queries.** Test rank limits 10 through 4,096 and the frozen cumulative reciprocal-rank reducer: one-based rank `r` adds `floor(2^32 / r)` checked `u64` units to both distinct row pages, followed by descending score and ascending page-ordinal selection of exactly eight pages. Include a case where one early isolated page beats a weak page but loses to a page with enough later corroborating rows. Persist the first ten ranked source ordinals, distance bits, and pages for diagnosis; during serialization rerun exact scoring and the reducer from authenticated construction, query, truth, and assignment inputs and require every sample to match. Reject distance-scale weights, tunable cutoffs, and any own-row/page exclusion surface because external queries have no construction identity.
- [ ] **Step 2: Run the cumulative exact-global open gate once.** Preserve the prior rank-sharp rejection. Stop below 975,000 aggregate or 995,000 oracle attainment; do not train or tune a router and do not try another reducer on the same development cohort.
