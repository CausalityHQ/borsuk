...
Load the pinned V20 root/codebook through existing validation. Read every exact block through the existing authenticated range decoder and its matching authenticated V20 row-code slice, preserving group/card/leaf authority and canonical source ordinal. Join exact rows and codes only after their independent row counts and checksums agree. Construct the projected directory once per arm; release the V20 code planes immediately after representative codes/spreads are derived. Reuse `resident_global_selected_cells` for routing.
