...
- [ ] **Step 1: Add the research-only V24 evidence converter through TDD.** Create `scripts/build_v25_containment_open.py` and `scripts/test_build_v25_containment_open.py`. Authenticate the exact V24 construction/page-row Parquets; reject schema, metadata, source inventory, page relation, vector, and digest drift; select the registered SplitMix cohort and independent pseudoqueries; remap to dense V25-local ordinals; and emit construction, assignment, pseudoquery, truth, and source-map Parquets plus one canonical receipt. The source map binds local ordinals to original dataset ordinals. The converter must not import into or add a V24 reader to production. Run only the named unittest while iterating, then the complete converter file and scoped Ruff/pycompile/diff once.
- [ ] **Step 2: Prove a tiny real-Parquet conversion smoke.** Use a hand-derived fixture with noncontiguous selected dataset ordinals, primary/replica rows in different batches, distance ties, own-page exclusion, and literal top-ten/page-oracle expectations. Require deterministic byte identities across input batch boundaries and reject output-directory reuse.
- [ ] **Step 3: Freeze the open manifest.** Bind the two immutable V24 source identities, exactly 262,144 SplitMix-ranked real rows, the Parquet source-map identity, 512 independently ranked leave-self-out pseudoqueries, exactly eight pages, exact truth work `12,884,901,888` dimensions, both seeds, selected-source digest, source/archive/index identities, every V25 output identity, schemas, gates, resource stops, binary/source identities, output prefix, and no-restart semantics.
- [ ] **Step 4: Run the authentic boundary smoke.** Use one fresh `causality` Spot instance and the exact production binary. A non-GREEN smoke terminates before the screen.
- [ ] **Step 5: Run controls sequentially.** Compute layout then exact-global. Stop immediately if layout is below 975,000 ppm, exact-global aggregate is below 975,000 ppm, or exact-global oracle attainment is below 995,000 ppm. Record 995,000 ppm layout recall as the target, not a hard gate. Do not train a hierarchy or codebook in this task.
- [ ] **Step 6: Authenticate and classify the terminal.** Recompute every metric from evidence Parquet, verify progress completed exactly, record RSS/PSI/swap/wall/CPU, terminate the instance, and explicitly clean staged files.
- [ ] **Step 7: Commit evidence.** If rejected, record the closure and stop this plan. If passed, record `rank-reducer-candidate` and authorize Task 6. Commit with message `Record V25 containment causal screen`.
