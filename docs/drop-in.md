...
`list`/`list_paginated` (Python and TypeScript), Qdrant `scroll`, and Chroma
`get` walk stored records (S3 Vectors lists *indexes/buckets*, not vectors, so it
has none). Sparse-vector
