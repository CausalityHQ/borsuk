...
| Physical object layout | `BuildConfig::physical_layout` | not user-selectable | not user-selectable | role-policy v3; Parquet for durable tables and Arrow IPC for sidecars | The writer persists the fixed role policy. Removed experimental table-format aliases are rejected; users do not provide a collection-size or format hint. |
...
| Inner-parallel decode/rank cap | `OpenOptions::max_parallel_decode_rank_tasks` | `max_parallel_decode_rank_tasks` | `maxParallelDecodeRankTasks` | `1` | Per-collection admission into the shared CPU pool for inner-parallel approximate ANN head decode/rank work. S3 downloads remain governed independently, and the serial exact-rerank stage does not consume this permit. Raise only from a matched-recall concurrency benchmark. |
| Exact-read physical amplification | `OpenOptions::exact_read_max_physical_amplification` | `exact_read_max_physical_amplification` | `exactReadMaxPhysicalAmplification` | `1` | Bounds exact-rerank range bytes relative to selected exact-block bytes. The default reads selected blocks only; `2..=5` may coalesce small gaps to trade bounded extra S3 bytes for fewer GETs. |
...
| `global_base_head_admission_us` / `globalBaseHeadAdmissionUs`, `global_base_head_fetch_us` / `globalBaseHeadFetchUs`, `global_base_head_decode_admission_us` / `globalBaseHeadDecodeAdmissionUs`, `global_base_head_decode_us` / `globalBaseHeadDecodeUs`, and `global_base_approximate_us` / `globalBaseApproximateUs` | Aggregate microseconds spent waiting for head transient-memory admission, completing authenticated head fetch waves, waiting for bounded decode/rank admission, decoding heads, and scoring/ranking their PQ codes. | Fetch time includes collection-wide leaf-read admission; use the `borsuk_leaf_read_wait_*` flow-control counters to distinguish queueing from storage. Internal work may overlap, so compare with wall time rather than summing across concurrent requests. |
| `global_base_exact_admission_us` / `globalBaseExactAdmissionUs`, `global_base_exact_fetch_us` / `globalBaseExactFetchUs`, `global_base_exact_cpu_us` / `globalBaseExactCpuUs`, and `global_base_exact_rerank_us` / `globalBaseExactRerankUs` | Exact transient-memory admission wait, complete exact fetch-wave time, post-I/O decode/MVCC/exact-score CPU, and the existing end-to-end exact-wave total. | Fetch time includes collection-wide leaf-read admission; correlate with `borsuk_leaf_read_wait_*`. The split preserves the established exact total while distinguishing memory, fetch-wave, and local CPU pressure. |
...
| `OpenOptions::max_parallel_decode_rank_tasks` | Bounds collection-local inner-parallel approximate decode/rank stages entering the shared CPU pool after their bytes arrive. | **Smoother application CPU:** the default prevents concurrent searches from fragmenting Rayon across many inner jobs. Fetched bytes remain charged to transient-memory admission while waiting; S3 reads and serial exact rerank use their own controls. |
