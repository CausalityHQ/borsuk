...
// Clickable, rotatable 3D walk-through of a query: bubble → prune → read → rerank.
function initVectorViz() {
  const stage = document.querySelector("[data-viz-svg]");
  if (!stage) return;
  const stepsRoot = document.querySelector("[data-viz-steps]");
  const rotate = document.querySelector("[data-viz-rotate]");
  const caption = document.querySelector("[data-viz-caption]");

  // Three clusters of four vectors, one query near the middle cluster. Colors
  // match the segment palette; the query is dark.
  const palette = ["#2f7f73", "#c14d32", "#6f4a31"];
  const clusters = [
    { center: [-2.4, -0.6, 0.4], color: palette[0] },
    { center: [2.2, 0.2, 1.1], color: palette[1] },
    { center: [0.2, 2.2, -2.2], color: palette[2] },
  ];
  const jitter = [
    [0.5, 0.4, 0.3],
    [-0.4, 0.5, -0.3],
  ...
  const points = [];
  clusters.forEach((cluster, segment) => {
    jitter.forEach((offset) => {
      points.push({
        pos: cluster.center.map((value, axis) => value + offset[axis]),
        segment,
        color: cluster.color,
      });
    });
  });
  const query = [1.7, 0.1, 0.7];

  // Per-segment centroid + radius (max distance from centroid), computed from
  // contents exactly like BORSUK does.
  const segments = clusters.map((cluster, index) => {
    const members = points.filter((point) => point.segment === index);
    const centroid = [0, 1, 2].map(
      (axis) => members.reduce((sum, point) => sum + point.pos[axis], 0) / members.length,
    );
    const radius = Math.max(...members.map((point) => distance(point.pos, centroid)));
    return { centroid, radius, color: cluster.color };
  });
  // Route: lower bound = distance(query, centroid) − radius. Pick the two lowest.
  const ranked = segments
    .map((segment, index) => ({ index, bound: distance(query, segment.centroid) - segment.radius }))
    .sort((a, b) => a.bound - b.bound);
  const selected = new Set([ranked[0].index, ranked[1].index]);
  // Exact winners: nearest two points among the selected (read) segments.
  const winners = points
    .map((point, index) => ({ index, d: distance(point.pos, query) }))
    .filter((entry) => selected.has(points[entry.index].segment))
    .sort((a, b) => a.d - b.d)
    .slice(0, 2)
    .map((entry) => entry.index);

  const captions = [
    "Twelve vectors live as points in space. RAM holds only a tiny map, not the vectors.",
    "BORSUK groups nearby vectors into segments — bubbles with a centroid and a radius sized to their contents.",
    "For a query, each bubble's lower bound is distance(query, centroid) − radius. Far bubbles are pruned without ever being read; the two closest stay.",
    "Only the surviving bubbles are fetched from object storage and their vectors decoded — a fraction of the data.",
    "The read candidates are exact-scored on full vectors, and the true nearest neighbours are returned.",
  ];

  let step = 0;
  const buttons = stepsRoot ? [...stepsRoot.querySelectorAll("[data-viz-step]")] : [];

  const render = () => {
    const angle = (Number(rotate?.value ?? 35) * Math.PI) / 180;
    stage.innerHTML = renderViz({ points, query, segments, selected, winners, step, angle });
    if (caption) caption.textContent = captions[step];
    buttons.forEach((button) =>
      button.classList.toggle("is-active", Number(button.dataset.vizStep) === step),
    );
  };

  buttons.forEach((button) =>
    button.addEventListener("click", () => {
      step = Number(button.dataset.vizStep);
      render();
    }),
  );
...
// Project a rotated 3D point to 2D with a light isometric tilt. Returns {x,y,depth}.
function projectPoint([x, y, z], angle, width, height) {
  const rx = x * Math.cos(angle) + z * Math.sin(angle);
  const rz = -x * Math.sin(angle) + z * Math.cos(angle);
  const scale = 46;
  const screenX = width / 2 + (rx - rz) * scale * 0.72;
  const screenY = height / 2 - y * scale + (rx + rz) * scale * 0.34;
  return { x: screenX, y: screenY, depth: rx + rz };
}

function renderViz({ points, query, segments, selected, winners, step, angle }) {
  const width = 640;
  const height = 380;
  const parts = [];
  const q = projectPoint(query, angle, width, height);

  // Bubbles (steps 1+), drawn back-to-front and dimmed when pruned.
  if (step >= 1) {
    const bubbles = segments
      .map((segment, index) => ({ segment, index, proj: projectPoint(segment.centroid, angle, width, height) }))
      .sort((a, b) => a.proj.depth - b.proj.depth);
    for (const { segment, index, proj } of bubbles) {
      const isSelected = selected.has(index);
      const active = step < 2 || step >= 3 ? true : isSelected;
      const pruned = step >= 2 && !isSelected;
      const r = segment.radius * 46 * 0.72;
      parts.push(
        `<circle cx="${proj.x.toFixed(1)}" cy="${proj.y.toFixed(1)}" r="${r.toFixed(1)}" fill="${segment.color}" fill-opacity="${pruned ? 0.05 : 0.12}" stroke="${segment.color}" stroke-opacity="${pruned ? 0.25 : 0.85}" stroke-width="${step >= 2 && isSelected ? 3 : 1.5}"${pruned ? ' stroke-dasharray="4 4"' : ""} />`,
      );
      parts.push(
        `<circle cx="${proj.x.toFixed(1)}" cy="${proj.y.toFixed(1)}" r="3" fill="${segment.color}" fill-opacity="${pruned ? 0.3 : 1}" />`,
      );
      if (step >= 2 && pruned) {
        parts.push(
          `<text x="${proj.x.toFixed(1)}" y="${(proj.y - r - 6).toFixed(1)}" text-anchor="middle" font-size="11" fill="#8c9287">pruned</text>`,
        );
      }
    }
  }

  // Points, back-to-front. Faded when their segment is pruned or not yet read.
  const projected = points
    .map((point, index) => ({ point, index, proj: projectPoint(point.pos, angle, width, height) }))
    .sort((a, b) => a.proj.depth - b.proj.depth);
  for (const { point, index, proj } of projected) {
    const inRead = selected.has(point.segment);
    let opacity = 1;
    if (step === 2) opacity = inRead ? 1 : 0.25;
    if (step >= 3) opacity = inRead ? 1 : 0.12;
    const isWinner = step >= 4 && winners.includes(index);
    if (isWinner) {
      parts.push(
        `<line x1="${q.x.toFixed(1)}" y1="${q.y.toFixed(1)}" x2="${proj.x.toFixed(1)}" y2="${proj.y.toFixed(1)}" stroke="#26352d" stroke-width="1.5" stroke-opacity="0.7" />`,
      );
    }
    parts.push(
      `<circle cx="${proj.x.toFixed(1)}" cy="${proj.y.toFixed(1)}" r="${isWinner ? 7 : 5}" fill="${point.color}" fill-opacity="${opacity}" stroke="${isWinner ? "#26352d" : "none"}" stroke-width="${isWinner ? 2.5 : 0}" />`,
    );
  }

  // The query marker (steps 2+).
  if (step >= 2) {
    parts.push(
      `<circle cx="${q.x.toFixed(1)}" cy="${q.y.toFixed(1)}" r="7" fill="#fffaf0" stroke="#26352d" stroke-width="3" />`,
    );
    parts.push(
      `<text x="${q.x.toFixed(1)}" y="${(q.y - 12).toFixed(1)}" text-anchor="middle" font-size="12" font-weight="700" fill="#26352d">query</text>`,
    );
  }
