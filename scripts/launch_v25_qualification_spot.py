#!/usr/bin/env python3
"""Spot-only, multi-AZ launcher boundary for V25 qualification phases."""
...
def build_v25_spot_plan(
    *,
    ...
    if profile != "causality" or not image_id or not instance_type or not subnet_ids:
        raise ValueError("V25 Spot plan authority differs")
    if len(set(subnet_ids)) != len(subnet_ids) or any(not subnet for subnet in subnet_ids):
        raise ValueError("V25 Spot subnet inventory differs")
    return V25SpotPlan(
        profile=profile,
        image_id=image_id,
...
def run_v25_spot_phase(
    ...
    for subnet_id in plan.subnet_ids:
        ...
        if len(instances) != 1 or len(observed_ids) != 1:
            ...
            raise RuntimeError("V25 Spot launch response differs")
        instance_id = observed_ids[0]
        break
    if instance_id is None:
        raise RuntimeError(f"V25 Spot capacity unavailable in {len(failures)} subnets")
    try:
