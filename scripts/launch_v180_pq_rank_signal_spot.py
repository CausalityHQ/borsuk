#!/usr/bin/env python3
"""Launch one immutable source-only V180 PQ rank signal gate on Spot."""
...
SCHEMA = "borsuk-v180-pq-rank-signal-spot-v1"
TAG = "borsuk-v180-pq-rank-signal"
WALL_SECONDS = 14_400
...
ARTIFACTS = (
    ...
    "out/fit-labels.jsonl", "out/fit-seal.json",
    "out/holdout-labels.jsonl", "out/summary.json",
    "prepare-resources.txt", "fit-resources.txt",
...
def user_data(commit: str, archive_sha: str, archive_key: str, prefix: str) -> str:
    return f'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v180-hard-stop --on-active={WALL_SECONDS}s /usr/sbin/shutdown -h now
root=/mnt/v180-pq-rank-signal
...
/usr/bin/time -v -o prepare-resources.txt .venv/bin/python -m \
  scripts.v180_pq_rank_signal prepare --output out \
...
# The complete GT-blind ranking is durable before any exact truth is scored.
...
phase=fit
/usr/bin/time -v -o fit-resources.txt .venv/bin/python -m \
  scripts.v180_pq_rank_signal fit --output out \
  --source source.parquet --old-layout old-layout.npy \
  --old-sq8 old-sq8.bin --router router --order order.npy \
  --v164-terminal v164-terminal.json --prepare-sha256 "$seal_sha"
# The source-fit model is durable before holdout exact truth is scored.
aws s3 cp out/fit-labels.jsonl "s3://{BUCKET}/{prefix}/sealed/fit-labels.jsonl" --only-show-errors
aws s3 cp out/fit-seal.json "s3://{BUCKET}/{prefix}/sealed/fit-seal.json" --only-show-errors
fit_sha=$(sha256sum out/fit-seal.json | cut -d ' ' -f1)
phase=evaluate
/usr/bin/time -v -o evaluate-resources.txt .venv/bin/python -m \
  scripts.v180_pq_rank_signal evaluate --output out \
  --source source.parquet --old-layout old-layout.npy \
  --old-sq8 old-sq8.bin --router router --order order.npy \
  --v164-terminal v164-terminal.json --prepare-sha256 "$seal_sha" \
  --fit-sha256 "$fit_sha"
...
def _launch() -> None:
    ...
    if subprocess.run(["git", "merge-base", "--is-ancestor", "HEAD",
        ...
        raise ValueError("V180 source commit is not pushed to origin/main")
    ...
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as tar:
        required = {"scripts/v166_surrogate_probe.py",
                    ...
                    "scripts/v180_pq_rank_signal.py",
                    "docs/research/v180-pq-rank-signal-prereg.md"}
        if not required.issubset(tar.getnames()):
            raise ValueError("source archive lacks V180 gate")
    archive_sha = hashlib.sha256(archive).hexdigest()
    archive_key = f"research/v180-pq-rank-signal/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v180-pq-rank-signal/{commit}/runs/{attempt}"
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V180 worker already active")
    ...
    receipt = ec2.run_instances(
        ClientToken="v180-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
    ...
    while time.monotonic() < deadline:
        if not missing(s3, prefix + "/terminal.json"):
            ...
            for name, identity in terminal.get("artifacts", {}).items():
                ...
                if size != identity["bytes"] or digest.hexdigest() != identity["sha256"]:
                    raise ValueError(f"V180 S3 artifact read-back differs: {name}")
            terminal["terminal_sha256"] = hashlib.sha256(raw).hexdigest()
            print(json.dumps(terminal, sort_keys=True), flush=True)
            if terminal.get("status") != "complete":
                raise RuntimeError("V180 terminal reports failed; see closed artifacts")
            return
        ...
        if state in {"terminated", "shutting-down"}:
            ...
            if time.monotonic() - started >= WALL_SECONDS - 60:
                raise TimeoutError("V180 worker reached wall cap before terminal")
            raise RuntimeError("Spot worker stopped before terminal; restart full cell")
        time.sleep(20)
    ec2.terminate_instances(InstanceIds=[instance_id])
    raise TimeoutError("V180 cell exceeded wall cap")


def main() -> None:
    # All V180 controllers on this devbox share one process-lifetime lock.
    # The EC2 tag check below also rejects an already running worker.
    with open("/tmp/borsuk-v180-pq-rank-signal-launch.lock", "a+") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as exc:
            raise RuntimeError("V180 launcher is already running here") from exc
        _launch()
