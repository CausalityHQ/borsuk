#!/bin/bash
# One frozen V135 exact-hull route replay on a Causality Spot worker.
set -euo pipefail
...
deadline_epoch=$((started_epoch + V135_WALL_SECONDS))
monitor_pid=
imds_token=
imds() {
...
finish() {
  ...
  for path in install.log compile.log compile-resources.txt \
    ...
    if [ -f "$path" ] && ! timeout 900 aws s3 cp "$path" \
        "$V135_OUTPUT_PREFIX/artifacts/$path" --only-show-errors; then
      upload_failed=1
    fi
  done
...
PY
  aws s3 cp terminal.json "$V135_OUTPUT_PREFIX/terminal.json" --only-show-errors
  shutdown -h now || true
  exit "$code"
}
trap finish EXIT
...
phase=evaluate
mkdir /sys/fs/cgroup/v135-serving
sync
echo 3 >/proc/sys/vm/drop_caches
run_science evaluate bash -c \
  'echo "$BASHPID" >/sys/fs/cgroup/v135-serving/cgroup.procs; exec "$@"' \
  v135 "$CARGO_TARGET_DIR/release/examples/v134_native_route" \
  prepared replay.jsonl summary.json
{
  cat /sys/fs/cgroup/v135-serving/memory.current
  cat /sys/fs/cgroup/v135-serving/memory.peak
  cat /sys/fs/cgroup/v135-serving/memory.stat
} >cgroup-memory.txt
phase=complete
exit 0
