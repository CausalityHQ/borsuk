#!/usr/bin/env python3
"""One preregistered 100k PQ-screened resident neighborhood cell on Spot."""
...
SCHEMA = "borsuk-v211-pq-resident-neighborhood-100k-spot-v1"
TAG = "borsuk-v211-pq-resident-neighborhood-100k"
WALL_SECONDS = 3_600
...
INPUTS = tuple(row for row in V163_INPUTS if row[0] == "source.parquet") + tuple(
    ...
    row for row in V170_INPUTS if row[0] in {"order.npy", "v113-ids.npy",
                                            "v113-books.npy", "v113-codes.npy"})
if {row[0] for row in INPUTS} != {"source.parquet", "sq8.bin", "requests.jsonl",
                                  "order.npy", "v113-ids.npy", "v113-books.npy",
                                  "v113-codes.npy"}:
    raise RuntimeError("V211 frozen input roster differs")
...
ARTIFACTS = ("raw.jsonl", "pretruth-seal.json", "summary.json",
             "prepare-resources.txt", "score-resources.txt", "install.log", "run-closed.log")
...
def user_data(commit: str, archive_sha: str, archive_key: str, prefix: str) -> str:
    template = r'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v211-hard-stop --on-active=@@WALL@@s /usr/sbin/shutdown -h now
root=/mnt/v211-pq-resident-neighborhood-100k
...
@@DOWNLOADS@@
phase=prepare
/usr/bin/time -v -o prepare-resources.txt .venv/bin/python -m scripts.v211_pq_resident_neighborhood_100k prepare \
  --source source.parquet --old-sq8 sq8.bin --order order.npy \
  --pq-ids v113-ids.npy --pq-books v113-books.npy --pq-codes v113-codes.npy \
  --requests requests.jsonl --raw raw.jsonl --seal pretruth-seal.json
for name in raw.jsonl pretruth-seal.json; do
  aws s3api put-object --bucket '@@BUCKET@@' --key '@@PREFIX@@/sealed/'"$name" \
    --body "$name" --if-none-match '*' --no-cli-pager >/dev/null
done
seal_sha=$(sha256sum pretruth-seal.json | cut -d ' ' -f1)
remote_sha=$(aws s3 cp 's3://@@BUCKET@@/@@PREFIX@@/sealed/pretruth-seal.json' - \
  --only-show-errors | sha256sum | cut -d ' ' -f1)
[ "$seal_sha" = "$remote_sha" ]
phase=truth
@@LATE_DOWNLOADS@@
/usr/bin/time -v -o score-resources.txt .venv/bin/python -m scripts.v211_pq_resident_neighborhood_100k score \
  --source source.parquet --old-sq8 sq8.bin --order order.npy \
  --pq-ids v113-ids.npy --pq-books v113-books.npy --pq-codes v113-codes.npy \
  --requests requests.jsonl --raw raw.jsonl --seal pretruth-seal.json \
  --truth truth.parquet --baseline v193-raw.jsonl --summary summary.json \
...
'''
    ...
    if "@@" in template:
        raise ValueError("unresolved V211 worker placeholder")
    return template
...
def launch(attempt: str) -> None:
    ...
    archive_key = f"research/v211-pq-resident-neighborhood-100k/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v211-pq-resident-neighborhood-100k/{commit}/runs/{attempt}"
    ...
    put_if_absent(prefix + "/reservation.json", json.dumps({
        ...
        "neighborhood": 32768, "shortlist_budgets": [4096, 8192, 16384],
        "gate": "hits>=paired-V193-full-rank;p05>=98;PQ-plus-FP16-p95<=10ms",
    ...
    receipt = ec2.run_instances(
        ClientToken="v211-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
        ...
        while time.monotonic() < deadline:
            try:
                ...
                if (terminal.get("schema") != SCHEMA or terminal.get("instance_id") != instance_id
                    ...
                    raise ValueError("V211 terminal identity differs")
                terminal["terminal_sha256"] = hashlib.sha256(raw).hexdigest()
                print(json.dumps(terminal, sort_keys=True), flush=True)
                if terminal.get("status") != "complete":
                    raise RuntimeError("V211 failed; inspect only closed terminal artifacts")
                if set(terminal.get("artifacts", {})) != set(ARTIFACTS):
                    raise ValueError("V211 artifact roster differs")
                for name, identity in terminal["artifacts"].items():
                    ...
                    if size != identity["bytes"] or value.hexdigest() != identity["sha256"]:
                        raise ValueError(f"V211 artifact readback differs: {name}")
            ...
            if state in {"terminated", "shutting-down"}:
                raise RuntimeError("V211 worker stopped before terminal")
            time.sleep(20)
        raise TimeoutError("V211 Spot cell exceeded wall cap")
    finally:
        try:
            state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
...
if __name__ == "__main__":
    ...
    with open("/tmp/borsuk-v211-resident-neighborhood-launch.lock", "a+") as lock:
