#!/usr/bin/env python3
"""Launch and authenticate one immutable ReLAION-1M capped replay on Spot."""
...
def worker_script(plan: BoundedReaderSpotPlan) -> str:
    values = {
        ...
        "V109_ARCHIVE_BYTES": plan.source_archive.bytes,
        "V109_WALL_SECONDS": 7200,
    }
    for role in INPUTS:
        identity = INPUTS[role]
        values[f"V109_{role}_URI"] = identity.uri
        values[f"V109_{role}_SHA256"] = identity.sha256
        values[f"V109_{role}_BYTES"] = identity.bytes
    exports = "\n".join(
...
aws s3 cp "$V109_ARCHIVE_URI" source.tar.gz --only-show-errors
[ "$(stat -c%s source.tar.gz)" = "$V109_ARCHIVE_BYTES" ]
printf '%s  source.tar.gz\\n' "$V109_ARCHIVE_SHA256" | sha256sum -c -
mkdir repo
tar -xzf source.tar.gz -C repo
exec bash repo/scripts/run_v109_capped_replay_remote.sh
"""
    script = "#!/bin/bash\n" + exports + "\n" + body
        ...
        while True:
            try:
                ...
                if time.monotonic() >= deadline:
                    raise TimeoutError("V109 Spot terminal deadline exceeded") from error
                time.sleep(15)
                continue
...
def readback(s3: object, terminal: dict[str, object]) -> dict[str, object]:
    ...
    required = {"hashes.log", "manifest-export.log", "prefix.log",
                ...
                "prefix-resources.txt", "decision.json"}
    if not isinstance(artifacts, dict) or not required.issubset(artifacts):
        raise ValueError("V109 terminal artifact roster differs")
    bodies = {}
    for name, identity in artifacts.items():
        ...
        if len(body) != identity["bytes"] or hashlib.sha256(body).hexdigest() != identity["sha256"]:
            raise ValueError("V109 artifact readback differs")
        bodies[name] = body
    decision = json.loads(bodies["decision.json"])
    prefix = json.loads(bodies["prefix-reduction.json"])
    if (decision.get("schema") != "borsuk-v109-decision-v1"
        ...
        raise ValueError("V109 prefix binding differs")
    if decision["prefix_decision"] == "advance-full":
        if not {"full.log", "full-evidence.jsonl", "full-reduction.json",
                "full-resources.txt"}.issubset(artifacts):
            raise ValueError("V109 full artifact roster differs")
        ...
        if (full.get("evidence_sha256") != artifacts["full-evidence.jsonl"]["sha256"]
            ...
            raise ValueError("V109 full binding differs")
    elif decision["prefix_decision"] not in ("harness-mismatch", "stop-capped-planner"):
        raise ValueError("V109 decision differs")
    return terminal
