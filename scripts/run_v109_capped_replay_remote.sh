...
publish_terminal() {
  ...
  for name in hashes.log manifest-export.log prefix.log prefix-evidence.jsonl prefix-reduction.json \
      prefix-resources.txt full.log full-evidence.jsonl full-reduction.json full-resources.txt \
      worker.log decision.json interrupt-stop.txt pressure-stop.txt swap-stop.txt; do
    if [ -f "$name" ] && ! aws s3 cp "$name" "$V109_OUTPUT_PREFIX/artifacts/$name" --only-show-errors; then
...
names=("hashes.log", "manifest-export.log", "prefix.log", "prefix-evidence.jsonl",
       "prefix-reduction.json", "prefix-resources.txt", "full.log", "full-evidence.jsonl",
       "full-reduction.json", "full-resources.txt", "worker.log", "decision.json",
...
PY
  aws s3 cp terminal.json "$V109_OUTPUT_PREFIX/terminal.json" --only-show-errors
  shutdown -h now
  exit "$code"
}
...
run_science() {
  ...
  setsid timeout --signal=TERM --kill-after=30 "$V109_WALL_SECONDS" \
...
for role in SOURCE QUERIES TRUTH LAYOUT SQ8; do
  ...
  eval uri=\$V109_${role}_URI
  eval expected_bytes=\$V109_${role}_BYTES
  eval expected_sha=\$V109_${role}_SHA256
...
phase=prefix
run_science prefix .venv/bin/python repo/scripts/v109_capped_reader_replay.py \
  --manifest manifest.bin --sq8 sq8.bin --output prefix-evidence.jsonl \
  --queries 200 --regions 1024
PYTHONPATH=repo .venv/bin/python -m scripts.validate_v109_capped_replay \
...
good=(p['capped']['recall100_ppm']>=990000 and p['capped']['p05_hits']>=90
      and p['capped']['cap_violations']==0)
decision='advance-full' if matched and good else ('harness-mismatch' if not matched else 'stop-capped-planner')
Path('decision.json').write_text(json.dumps({'schema':'borsuk-v109-decision-v1',
...
phase=full
run_science full .venv/bin/python repo/scripts/v109_capped_reader_replay.py \
