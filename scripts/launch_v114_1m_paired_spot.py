#!/usr/bin/env python3
"""Reserve, launch and observe one source-frozen V114 paired 1M Spot cell."""
...
SCHEMA = "borsuk-v114-1m-paired-spot-v1"
...
def validate_plan(plan: Plan) -> None:
    if (
        ...
        raise ValueError("V114 1M immutable Spot plan differs")
...
def user_data(plan: Plan) -> str:
    validate_plan(plan)
    variables = [
        f"export V114_SOURCE_COMMIT={_q(plan.source_commit)}",
        f"export V114_OUTPUT_PREFIX={_q(plan.output_prefix.rstrip('/'))}",
        f"export V114_MANIFEST_SHA256={_q(FROZEN_MANIFEST_SHA256)}",
        f"export V114_WALL_SECONDS={plan.wall_seconds}",
    ]
    for role, identity in INPUTS.items():
        variables += [
            f"export V114_{role}_URI={_q(identity.uri)}",
            f"export V114_{role}_SHA256={_q(identity.sha256)}",
            f"export V114_{role}_BYTES={identity.bytes}",
        ]
    environment = "\n".join(variables)
    return f"""#!/bin/bash
set -euo pipefail
...
trap - EXIT
exec bash repo/scripts/run_v114_1m_paired_remote.sh
"""


def launch_spec(plan: Plan, zone: str, subnet: str) -> dict[str, object]:
    token = "v114-1m-" + hashlib.sha256(
    ...
    return {
        ...
        "TagSpecifications": [{"ResourceType": "instance", "Tags": [
            {"Key": "Name", "Value": "borsuk-v114-1m-paired"},
...
def _location(uri: str) -> tuple[str, str]:
    ...
    if not bucket or not slash or not key:
        raise ValueError("V114 1M S3 location differs")
...
def _reserve(s3: object, plan: Plan, bucket: str, prefix: str) -> None:
    if not _missing(s3, bucket, f"{prefix}/reservation.json") or not _missing(
        ...
        raise ValueError("V114 1M immutable attempt already registered")
    ...
    event_id = "borsuk-v114-1m-reservation-if-none-match"
...
def launch_and_monitor(plan: Plan) -> dict[str, object]:
    """Observe only terminal/infrastructure health, then terminate compute."""
    import boto3
    ...
    if instance_id is None:
        raise RuntimeError("V114 1M Spot capacity unavailable; reservation retained")
        ...
        while time.monotonic() < deadline:
            if not _missing(s3, bucket, f"{prefix}/terminal.json"):
                ...
                if (
                    ...
                    raise ValueError("V114 1M terminal identity differs")
                return terminal
            state = ec2.describe_instances(InstanceIds=[instance_id])[
                "Reservations"
            ][0]["Instances"][0]["State"]["Name"]
            if state in {"stopped", "shutting-down", "terminated"}:
                raise RuntimeError(f"V114 1M Spot {instance_id} ended without terminal")
            time.sleep(20)
        raise TimeoutError(f"V114 1M Spot {instance_id} terminal deadline exceeded")
    finally:
