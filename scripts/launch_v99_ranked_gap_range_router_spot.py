#!/usr/bin/env python3
"""Launch and monitor one immutable V99 G1 attempt on Causality EC2 Spot."""
...
def derive_attempt_prefix(campaign: str, source_commit: str, timestamp: str) -> str:
    ...
    if (
        ...
        raise ValueError("V99 attempt prefix differs")
    return f"{campaign}/{source_commit}/{timestamp}/a0001"


def build_plan(**values: object) -> V99SpotPlan:
    """Freeze and validate one claim-eligible V99 Spot plan."""

    plan = V99SpotPlan(**values)
    ...
    if (
        ...
        raise ValueError("V99 Spot plan differs")
    return plan
...
def worker_script(plan: V99SpotPlan) -> str:
    """Bind a plan to a size-bounded authenticated bootstrap."""
    exports = {
        "V99_ATTEMPT": plan.attempt,
        "V99_BOOTSTRAP_RESAMPLES": plan.bootstrap_resamples,
        "V99_CRITIQUE_SHA256": plan.critique_result_sha256,
        "V99_MAXIMUM_BYTES": plan.config.maximum_bytes,
        "V99_MAXIMUM_EXPOSED_PAGES": plan.config.maximum_exposed_pages,
        "V99_MAXIMUM_GETS": plan.config.maximum_gets,
        "V99_MAXIMUM_ROOT_GROUPS": plan.config.maximum_root_groups,
        ...
        "V99_SOURCE_ARCHIVE_BYTES": plan.source_archive.bytes,
        "V99_SOURCE_ARCHIVE_SHA256": plan.source_archive.sha256,
        "V99_SOURCE_ARCHIVE_URI": plan.source_archive.uri,
        "V99_SOURCE_COMMIT": plan.source_commit,
        "V99_SPOT_PRICE_MICROS": plan.spot_price_usd_per_hour_micros,
        "V99_WALL_SECONDS": plan.wall_seconds,
    }
    for role, identity in sorted(plan.inputs.items()):
        prefix = f"V99_{role.upper()}"
...
root=/mnt/v99-ranked-gap-g1
mkdir -p "$root" && cd "$root"
aws s3 cp "$V99_SOURCE_ARCHIVE_URI" source.tar.gz --only-show-errors
[ "$(stat -c%s source.tar.gz)" = "$V99_SOURCE_ARCHIVE_BYTES" ]
printf '%s  source.tar.gz\\n' "$V99_SOURCE_ARCHIVE_SHA256" >hashes.txt
sha256sum -c hashes.txt
mkdir repo
tar -xzf source.tar.gz -C repo
exec bash repo/scripts/v99_ranked_gap_range_router_run_remote.sh
"""
    script = f"#!/bin/bash\n{header}\n{bootstrap}"
    if len(script.encode()) > 16_384:
        raise ValueError("V99 user data exceeds EC2 limit")
    return script


def build_launch_specs(plan: V99SpotPlan) -> list[dict[str, object]]:
    ...
    for target in plan.targets:
        token_material = f"v99:{plan.source_commit}:{target.availability_zone}:a0001"
        specs.append(
            {
                "BlockDeviceMappings": [
                    {
...
def launch_one_spot(plan: V99SpotPlan, *, ec2_client: object) -> str:
    ...
    for request in build_launch_specs(plan):
        ...
        if (
            ...
            raise RuntimeError("V99 Spot launch response differs")
        return instances[0]["InstanceId"]
    raise RuntimeError(f"V99 Spot capacity unavailable in {failures} zones")
...
def _launch_receipt(plan: V99SpotPlan, instance_id: str | None) -> bytes:
    value = {
...
def claim_reserved_attempt(plan: V99SpotPlan, *, s3_client: object) -> None:
...
def claim_launched_attempt(
    plan: V99SpotPlan, *, s3_client: object, instance_id: str
) -> None:
    """Persist the exact launched instance under a create-only key."""

    if not instance_id.startswith("i-"):
        raise ValueError("V99 instance identity differs")
    bucket, prefix = _s3_location(plan.output_prefix)
    _atomic_put(
        s3_client,
        bucket=bucket,
...
def canonical_terminal_bytes(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("V99 terminal differs")
    terminal = AttemptTerminal(
        schema="borsuk-v99-spot-terminal-v1",
        attempt=plan.attempt,
        status=status,
...
def validate_terminal_bytes(
    body: bytes, plan: V99SpotPlan, instance_id: str
) -> AttemptTerminal:
    ...
    try:
        ...
        raise ValueError("V99 terminal JSON differs") from error
    if (
        ...
        raise ValueError("V99 terminal canonical bytes differ")
    expected_keys = {
        "schema",
        "attempt",
        "status",
    ...
    if set(value) != expected_keys or type(value.get("evidence")) is not dict:
        raise ValueError("V99 terminal schema differs")
    evidence = {
    ...
    if expected != body:
        raise ValueError("V99 terminal authority differs")
        ...
        while True:
            try:
                ...
                if monotonic() >= deadline:  # type: ignore[operator]
                    raise TimeoutError(
                        "V99 attempt exceeded terminal deadline"
                    ) from error
                sleep(poll_seconds)  # type: ignore[operator]
                continue
            body = response["Body"].read()
            if response.get("ContentLength") != len(body):
                raise ValueError("V99 terminal object length differs")
            return validate_terminal_bytes(body, plan, instance_id)
    finally:
        ec2_client.terminate_instances(InstanceIds=[instance_id])


def ensure_unstarted(plan: V99SpotPlan, *, s3_client: object) -> None:
    ...
    for name in ("reservation.json", "launch.json", "terminal.json"):
        try:
            ...
            if code in ("404", "NoSuchKey", "NotFound") or any(
                ...
                continue
            raise
        raise ValueError("V99 immutable attempt already exists")


def parse_args(argv: Sequence[str] | None = None) -> V99SpotPlan:
