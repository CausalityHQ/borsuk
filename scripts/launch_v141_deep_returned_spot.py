#!/usr/bin/env python3
"""Launch and monitor one frozen V141 D96 returned-quality Spot attempt."""
...
REGION = "eu-central-1"
TAG = "borsuk-v141-deep-returned"
RUNNER = "scripts/run_v141_deep_returned_remote.sh"
INPUTS = (
    ("research/v122-deep-image-100k/afe07cb5a9ba8518263375595f589639fdf3f4f1/runs/v122-20260924T011355Z/a0001/artifacts/subset/source.parquet", 36_528_755),
    ...
    ("research/v122-deep-image-100k/afe07cb5a9ba8518263375595f589639fdf3f4f1/runs/v122-20260924T011355Z/a0001/artifacts/truth.npy", 800_128),
...
def validate(plan: Plan) -> None:
    ...
    if (
        ...
        or not output_key.startswith(
            f"research/v141-deep-returned/{plan.source_commit}/runs/"
        )
        ...
        raise ValueError("immutable V141 plan differs")
...
def user_data(plan: Plan) -> str:
    ...
    exports = "\n".join(
        (
            f"export V141_SOURCE_COMMIT={plan.source_commit}",
            f"export V141_ARCHIVE_SHA256={plan.archive_sha256}",
            f"export V141_OUTPUT_PREFIX={shlex.quote(plan.output_prefix)}",
            f"export V141_WALL_SECONDS={plan.wall_seconds}",
        )
    )
    return f"""#!/bin/bash
set -euo pipefail
systemd-run --unit=v141-hard-stop --on-active={plan.wall_seconds}s /usr/sbin/shutdown -h now
...
PY
    aws s3 cp terminal.json "$V141_OUTPUT_PREFIX/terminal.json" --only-show-errors || true
    shutdown -h now || true
  fi
}}
...
def launch_spec(plan: Plan) -> dict:
    token = "v141-" + hashlib.sha256(plan.output_prefix.encode()).hexdigest()[:48]
    return {
        "ClientToken": token,
...
def check_source_and_inputs(s3: object, plan: Plan) -> None:
    ...
    with tarfile.open(fileobj=archive, mode="r:gz") as source:
        if (RUNNER not in source.getnames()
                or "scripts/v141_deep_returned_quality.py" not in source.getnames()
                or "scripts/v114_1m_paired.py" not in source.getnames()
                or "scripts/v124_source_tier_precision.py" not in source.getnames()
                or "docs/research/inputs/v141-deep-route-only.jsonl" not in source.getnames()):
            raise ValueError("source archive lacks V141 runner or evaluator")
...
def reserve(plan: Plan) -> None:
    receipt = json.dumps({
        "schema": "borsuk-v141-deep-returned-reservation-v1",
...
def launch_and_monitor(plan: Plan) -> dict:
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V141 worker already active")
    ...
    try:
        while time.monotonic() < deadline:
            if not missing(s3, prefix + "/terminal.json"):
                ...
                if (terminal.get("schema") != "borsuk-v141-deep-returned-feasibility-spot-v1"
                    ...
                    raise ValueError("V141 terminal identity differs")
                ...
                if terminal.get("status") == "complete":
                    ...
                    if decision != {"schema": "borsuk-v141-decision-v1",
                                    "deep_completed": True}:
                        raise ValueError("V141 decision differs")
                    artifacts = terminal.get("artifacts", {})
                    if not required.issubset(artifacts):
                        raise ValueError("V141 complete terminal lacks required artifacts")
                    for name, metadata in artifacts.items():
                        ...
                        if length != metadata["bytes"] or digest.hexdigest() != metadata["sha256"]:
                            raise ValueError(f"V141 artifact differs: {name}")
                    terminal["authenticated_artifact_count"] = len(artifacts)
                print(json.dumps(terminal, sort_keys=True), flush=True)
                return terminal
            state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
            if state == "terminated":
                raise RuntimeError("V141 instance terminated without terminal marker")
            time.sleep(15)
        raise TimeoutError(f"V141 terminal missing after worker deadline: {instance_id}")
...
def main() -> None:
    ...
    if terminal.get("status") != "complete" or terminal.get("exit_code") != 0:
        raise RuntimeError("V141 terminal reports a failed attempt")
