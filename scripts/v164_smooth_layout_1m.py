#!/usr/bin/env python3
"""V164 source-only layout, GT-blind exact-primary plan and exact rerank."""
...
ROW_BYTES = DIMS + 12
SCHEMA = "borsuk-v164-smooth-layout-1m-v1"
...
def construct(source_path: Path, old_layout_path: Path, old_sq8_path: Path,
    ...
    geometry = np.empty_like(vectors)
    for start in range(0, ROWS, 16384):
        stop = min(start + 16384, ROWS)
        norms = np.linalg.norm(vectors[start:stop].astype(np.float64), axis=1)
        if not np.isfinite(norms).all() or (norms <= 0).any():
            raise ValueError("V164 cosine source vector norm differs")
        geometry[start:stop] = vectors[start:stop] / norms[:, None]
    clusters = cluster_count(ROWS)
    if clusters != 8192:
        raise ValueError("V164 smooth cluster rule differs")
    order = _fit_order(geometry, clusters)
    if not np.array_equal(np.sort(order), np.arange(ROWS)):
        raise ValueError("V164 source-only order is not a permutation")
    with membership_path.open("xb") as output:
    ...
    if new_sq8_path.stat().st_size != ROWS * ROW_BYTES:
        raise ValueError("V164 relaid SQ8 length differs")
    layout_seal.write_text(canonical({
        ...
        "old_sq8_sha256": SQ8_SHA, "order_file_sha256": sha256(membership_path),
        "new_sq8_sha256": sha256(new_sq8_path),
        "order_sha256": hashlib.sha256(order.astype("<i8").tobytes()).hexdigest(),
        "layout_method": "v120-lloyd-centroid-chain-cosine",
...
def read_order(membership_path: Path, old_layout_path: Path,
    ...
    order = np.load(membership_path, allow_pickle=False)
    if (order.shape != (ROWS,) or order.dtype != np.int64
            or not np.array_equal(np.sort(order), np.arange(ROWS))):
        raise ValueError("V164 order permutation differs")
...
def route(primary: list[int], nominees: list[int], old_order: np.ndarray,
    ...
    if (not 1 <= len(ranges) <= CAP_GETS or amount > CAP_BYTES
        ...
        raise ValueError("V164 physical cap differs")
...
def plan(requests_path: Path, sealed_path: Path, old_layout_path: Path,
    ...
    if (layout.get("gt_opened") is not False
            or layout.get("order_file_sha256") != sha256(membership_path)
            or layout.get("new_sq8_sha256") != sha256(new_sq8_path)):
        raise ValueError("GT-blind V164 layout seal differs")
...
def score(source_path: Path, old_layout_path: Path, old_sq8_path: Path,
    ...
    if (sha256(requests_path) != REQUEST_SHA
            ...
            or json.loads(plan_seal.read_text()).get("plans_sha256")
                != sha256(plans_path)):
        raise ValueError("V164 score input identity differs")
    source_ids, vectors, _ = source_arrays(source_path)
    old_order, old_sq8 = old_layout_and_sq8(old_layout_path, old_sq8_path,
                                             source_ids)
    ...
    with scored_path.open("x") as output:
        for request, plan_row in itertools.zip_longest(jsonl(requests_path),
            ...
            if (request is None or plan_row is None
                ...
                raise ValueError("V164 score row count/order differs")
            ...
            if len(returned) != 512 or len(set(returned)) != 512:
                raise ValueError("V164 SQ8 returned width differs")
            ...
            if not 512 <= union.size <= 1024:
                raise ValueError("V164 source union width differs")
            ...
            count += 1
    if count != QUERIES:
        raise ValueError("V164 score query count differs")
    ...
    with raw_path.open("x") as output:
        for plan_row, scored, control, prior in itertools.zip_longest(
            ...
            if any(row is None or row.get("query_ordinal") != count
                ...
                raise ValueError("V164 reduction query order differs")
            ...
            count += 1
    if count != QUERIES:
        raise ValueError("V164 reduction query count differs")
