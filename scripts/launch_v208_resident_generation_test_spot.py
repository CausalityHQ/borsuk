#!/usr/bin/env python3
"""Run one bounded targeted Rust resident-generation test cell on Spot."""
...
SCHEMA = "borsuk-v208-resident-generation-test-spot-v1"
TAG = "borsuk-v208-resident-generation-test"
IMAGE = "ami-06121aa3085b6f918"
WALL_SECONDS = 7_200
ARTIFACTS = ("generation-tests.log", "plane-tests.log", "install.log",
             "run-closed.log")
...
def user_data(commit: str, archive_sha: str, archive_key: str,
    ...
    template = r'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v208-hard-stop --on-active=@@WALL@@s /usr/sbin/shutdown -h now
root=/mnt/v208-resident-generation-test
...
cd repo
phase=generation_tests
"$CARGO_HOME/bin/cargo" test --release --locked -p borsuk --lib resident_nominee_generation::tests --jobs 6 >"$root/generation-tests.log" 2>&1
phase=plane_tests
"$CARGO_HOME/bin/cargo" test --release --locked -p borsuk --lib resident_fp16_tier::tests --jobs 6 >"$root/plane-tests.log" 2>&1
phase=complete
'''
    replacements = {
    ...
    if "@@" in template:
        raise ValueError("unresolved V208 worker placeholder")
    return template
...
def launch(attempt: str) -> None:
    ...
    if subprocess.run(["git", "merge-base", "--is-ancestor", "origin/main", "HEAD"],
        ...
        raise ValueError("V208 source is not a fast-forward descendant of origin/main")
    archive = archive_source(commit)
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as tar:
        required = {"crates/borsuk/src/resident_nominee_generation.rs",
                    "crates/borsuk/src/resident_fp16_tier.rs",
                    "docs/research/v208-resident-generation-verification.md"}
        if not required.issubset(tar.getnames()):
            raise ValueError("V208 source archive lacks frozen gate")
    archive_sha = hashlib.sha256(archive).hexdigest()
    archive_key = f"research/v208-resident-generation-test/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v208-resident-generation-test/{commit}/runs/{attempt}"
    ...
    receipt = ec2.run_instances(
        ClientToken="v208-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
        ...
        UserData=base64.b64encode(bootstrap(
            user_data(commit, archive_sha, archive_key, prefix)).encode()).decode(),
    )
        ...
        while time.monotonic() < deadline:
            try:
                ...
                if (terminal.get("schema") != SCHEMA
                    ...
                    raise ValueError("V208 terminal identity differs")
                terminal["terminal_sha256"] = hashlib.sha256(raw).hexdigest()
                print(json.dumps(terminal, sort_keys=True), flush=True)
                if terminal.get("status") != "complete":
                    raise RuntimeError("V208 failed; inspect only closed terminal artifacts")
                if set(terminal.get("artifacts", {})) != set(ARTIFACTS):
                    raise ValueError("V208 artifact roster differs")
                for name, identity in terminal["artifacts"].items():
                    ...
                    if size != identity["bytes"] or digest.hexdigest() != identity["sha256"]:
                        raise ValueError(f"V208 artifact readback differs: {name}")
            ...
            if state in {"terminated", "shutting-down"}:
                raise RuntimeError("V208 worker stopped before terminal")
            time.sleep(20)
        raise TimeoutError("V208 Spot cell exceeded wall cap")
    finally:
        try:
            state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
...
def main() -> None:
    ...
    with open("/tmp/borsuk-v208-resident-generation-test-launch.lock", "a+") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as error:
            raise RuntimeError("V208 launcher is already active") from error
        launch(args.attempt)
