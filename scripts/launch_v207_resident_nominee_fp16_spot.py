#!/usr/bin/env python3
"""One sealed resident-nominee FP16 diagnostic on Causality Spot."""
...
SCHEMA = "borsuk-v207-resident-nominee-fp16-spot-v1"
TAG = "borsuk-v207-resident-nominee-fp16"
...
V206_PREFIX = ("research/v206-deep-image-fp16/ac6738bdec80e9ba94ee505c271168ab50d0ff88/"
               "runs/a0002/artifacts/")
INPUTS = (
    ...
    ("rosters.jsonl", V121 + "rosters.jsonl", 4_039_308,
     "eb24f9a22d237db7416696415220e3fba559eadf7c7165ac6c8fd230a1d91700"),
    ("prior.jsonl", V206_PREFIX + "raw.jsonl", 2_931_323,
...
ARTIFACTS = ("resident-fp16.bin", "raw.jsonl", "pretruth-seal.json", "evidence.jsonl", "summary.json",
...
def user_data(commit: str, archive_sha: str, archive_key: str,
    ...
    template = r'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v207-hard-stop --on-active=@@WALL@@s /usr/sbin/shutdown -h now
root=/mnt/v207-resident-nominee-fp16
...
import numpy as np
from scripts.v207_resident_nominee_fp16 import rank
rng=np.random.default_rng(207)
source=rng.normal(size=(512,96)).astype(np.float32)
ids=np.arange(512,dtype=np.int64)
query=rng.normal(size=96).astype(np.float32)
for payload in (source,source.astype(np.float16)):
    actual=rank(ids,payload,query)
    vectors=payload.astype(np.float64)
    q=query.astype(np.float64);q/=np.linalg.norm(q)
    scores=(vectors@q)/np.linalg.norm(vectors,axis=1)
    expected=ids[np.lexsort((ids,-scores))[:100]].tolist()
    assert actual==expected
print('V207 synthetic top100 parity pass')
PY
phase=inputs
@@DOWNLOAD@@
phase=return
/usr/bin/time -v -o return-resources.txt .venv/bin/python -m scripts.v207_resident_nominee_fp16 return \
  --source source.parquet --layout layout.npy --queries queries.jsonl \
  --rosters rosters.jsonl --prior prior.jsonl \
  --plane resident-fp16.bin --raw raw.jsonl --seal pretruth-seal.json >return.log 2>&1
...
@@TRUTH_DOWNLOAD@@
phase=score
/usr/bin/time -v -o score-resources.txt .venv/bin/python -m scripts.v207_resident_nominee_fp16 score \
...
'''
    ...
    if "@@" in template:
        raise ValueError("unresolved V207 worker placeholder")
    return template
...
def launch(attempt: str) -> None:
    ...
    if subprocess.run(["git", "merge-base", "--is-ancestor", "HEAD", "origin/main"],
        ...
        raise ValueError("V207 source commit is not pushed to origin/main")
    archive = archive_source(commit)
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as tar:
        required = {"scripts/v207_resident_nominee_fp16.py",
                    "docs/research/v207-resident-nominee-fp16-prereg.md"}
        if not required.issubset(tar.getnames()):
            raise ValueError("V207 source archive lacks frozen gate")
    archive_sha = hashlib.sha256(archive).hexdigest()
    archive_key = f"research/v207-resident-nominee-fp16/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v207-resident-nominee-fp16/{commit}/runs/{attempt}"
    ...
    for _, key, size, digest in INPUTS + TRUTH_INPUT:
        if len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
            raise ValueError("V207 frozen input SHA-256 malformed")
        if s3.head_object(Bucket=BUCKET, Key=key)["ContentLength"] != size:
            raise ValueError("V207 frozen input length differs")
    ...
    receipt = ec2.run_instances(
        ClientToken="v207-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
        ...
        while time.monotonic() < deadline:
            try:
                ...
                if terminal.get("status") != "complete":
                    raise RuntimeError("V207 failed; inspect only closed terminal artifacts")
                if set(ARTIFACTS) != set(terminal.get("artifacts", {})):
                    raise ValueError("complete V207 artifact roster differs")
                for name, identity in terminal["artifacts"].items():
                    ...
                    if size != identity["bytes"] or digest.hexdigest() != identity["sha256"]:
                        raise ValueError(f"V207 S3 artifact read-back differs: {name}")
                ...
                if (summary.get("schema") != "borsuk-v207-resident-nominee-fp16-v1-summary"
                        ...
                        or summary.get("hits", {}).get("prior_fp16") != 99547):
                    raise ValueError("V207 summary identity differs")
            ...
            time.sleep(20)
        raise TimeoutError("V207 Spot cell exceeded wall cap")
    finally:
        try:
            state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
...
def main() -> None:
    ...
    with open("/tmp/borsuk-v207-resident-nominee-fp16-launch.lock","a+") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as error:
            raise RuntimeError("V207 launcher is already active") from error
        launch(args.attempt)
