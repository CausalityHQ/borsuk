...
def _validate_common(
    ...
    if (
        not attempt_id.startswith(("v30-", "v32-"))
...
def _validate_containment(plan: V32ContainmentSpotPlan) -> None:
    _validate_common(
        plan.attempt_id,
        plan.source_commit,
        plan.source_archive_uri,
...
def _evaluation_script(plan: V30EvaluationPlan) -> str:
    ...
    return "\n".join(
        [
            ...
            'source_dir="$root/source"',
            'archive="$root/source.tar.zst"',
            'mkdir -p "$root/resident" "$source_dir" /opt/borsuk',
            'exec >"$root/worker.log" 2>&1',
            f"output_bucket={shlex.quote(output_bucket)}",
            f"output_key={shlex.quote(output_key)}",
            "terminal=failed",
            ...
            "qualifier=/opt/borsuk/v30_s3_qualify",
            "set +e",
            '"$qualifier" >"$root/qualifier-probe.out" 2>"$root/qualifier-probe.err"',
            "probe_status=$?",
            "set -e",
            'test "$probe_status" -eq 1',
            "grep -F -- '--execute is required' \"$root/qualifier-probe.err\" >/dev/null",
...
def build_v32_containment_spot_specs(
    plan: V32ContainmentSpotPlan, targets: tuple[SpotTarget, ...]
) -> tuple[dict[str, object], ...]:
    """Build Spot specifications for the page-free 1M containment gate."""
    _validate_containment(plan)
    if not targets:
        raise ValueError("V32 Spot target inventory differs")
    script = _containment_script(plan)
    return tuple(_spec(target, script) for target in targets)
...
def execute_v30_spot_phase(
    *,
    plan: V30ConstructionPlan | V30EvaluationPlan | V32ContainmentSpotPlan,
