"""Read back and independently recount a closed pq96/source Spot attempt."""
...
from scripts.launch_native_hundred_thousand_pq96_spot import validate_terminal_bytes
from scripts.native_hundred_thousand_pq96_worker import ARTIFACT_FILES
...
def closeout_receipts(root: Path, plan: SpotLayoutPlan, instance_id: str) -> dict[str, object]:
    ...
    for role, path in ARTIFACT_FILES.items():
        ...
        if _identity(root / path) != (recorded["encoded_bytes"], recorded["sha256"]):
            raise ValueError(f"pq96 {role} readback differs")
    ...
    if (
        ...
        or decision.get("schema") != "borsuk-hundred-thousand-pq96-decision-v1"
        ...
        raise ValueError("pq96 decision receipt differs")
    ...
    for phase, values in decision["resources"].items():
        ...
        if rss is None or swaps is None or values != {
            ...
            raise ValueError("pq96 resource readback differs")
    ...
    if (
        ...
        raise ValueError("pq96 decision or resource gate differs")
    return {
        "schema": "borsuk-hundred-thousand-pq96-closeout-v1",
        "source_commit": plan.source_commit,
...
def closeout_closed_attempt(
    ...
    if validation_path.read_bytes() != original_validation:
        raise ValueError("pq96 independent replay differs")
    return {**receipt, "validation": replay}


def compare_frozen_source_control(pq_plans: Path, sign_plans: Path) -> int:
    """Check that the rescue's exact-source arm matches the closed sign screen."""
    left_body, right_body = pq_plans.read_bytes(), sign_plans.read_bytes()
    left, right = json.loads(left_body), json.loads(right_body)
    if (
        left_body != _canonical(left) or right_body != _canonical(right)
        or len(left.get("samples", [])) != 1000
        or len(right.get("samples", [])) != 1000
    ):
        raise ValueError("PQ96 paired source control differs")
    for ordinal, (pq, sign) in enumerate(zip(left["samples"], right["samples"], strict=True)):
        if (
            pq.get("query_ordinal") != ordinal
            or sign.get("query_ordinal") != ordinal
            or pq.get("selected_groups") != sign.get("selected_groups")
            or pq.get("source") != sign.get("source")
        ):
            raise ValueError("PQ96 paired source control differs")
    return len(left["samples"])
