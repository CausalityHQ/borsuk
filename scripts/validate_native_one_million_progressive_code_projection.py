#!/usr/bin/env python3
"""Independent geometry and truth-mask replay for mirrored progressive waves."""
...
def validate_closed(
    ...
    independent_sign = {
        "base": tuple(int(104 * count) for count in independent_counts[:base_pages]),
        "delta": tuple(int(104 * count) for count in independent_counts[base_pages:]),
    }
    independent_magnitude = {
        "base": tuple(int(96 * count) for count in independent_counts[:base_pages]),
        "delta": tuple(int(96 * count) for count in independent_counts[base_pages:]),
    }
    if (independent_sign, independent_magnitude) != lengths:
    ...
    for ordinal, (item, prior, recorded) in enumerate(zip(
        ...
        if code != _replay_cover(ranked, independent_sign, 32, 16_777_216):
            raise ValueError("code-wave independent admission differs")
        mirrored_bytes = sum(
            sum(independent_magnitude[role][first:end])
            for role, first, end in code["ranges"]
        )
        mirror = {**code, "encoded_bytes": mirrored_bytes}
        if (
            item["code"]["magnitude"] != mirror
            or mirror["gets"] > 32
            or mirrored_bytes > 16_777_216
            or mirrored_bytes > code["encoded_bytes"]
        ):
            raise ValueError("code-wave independent mirror differs")
        included = _geometry(code, independent_sign)
    ...
    plan_items = [item["code"]["sign"] for item in plans["samples"]]
    magnitude_items = [item["code"]["magnitude"] for item in plans["samples"]]
    metrics = {
        "gt100_hits": sum(hits),
        ...
        "maximum_magnitude_bytes": max(item["encoded_bytes"] for item in magnitude_items),
        "total_gets": sum(item["gets"] for item in plan_items),
        "total_bytes": sum(item["encoded_bytes"] for item in plan_items),
        "total_magnitude_bytes": sum(item["encoded_bytes"] for item in magnitude_items),
    ...
    advance = (
        ...
        and metrics["maximum_magnitude_bytes"] <= 16_777_216
    )
    if (
    ...
    (out / "progressive-code-wave-validation.json").write_bytes(_canonical(validation))
    return validation
