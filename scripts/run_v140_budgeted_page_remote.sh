#!/bin/bash
# One frozen, fail-fast, source-only budgeted-page feasibility screen on Spot.
set -euo pipefail
...
deadline_epoch=$((started_epoch + V140_WALL_SECONDS))
monitor_pid=
imds_token=
imds() {
...
finish() {
  ...
  for path in install.log download.log deep.log relaion.log \
    ...
    if [ -f "$path" ] && ! timeout 900 aws s3 cp "$path" \
        "$V140_OUTPUT_PREFIX/artifacts/$path" --only-show-errors; then
      upload_failed=1
    fi
  done
...
PY
  aws s3 cp terminal.json "$V140_OUTPUT_PREFIX/terminal.json" --only-show-errors
  shutdown -h now || true
  exit "$code"
}
trap finish EXIT
...
phase=evaluate-deep
mkdir /sys/fs/cgroup/v140-evaluate
sync
echo 3 >/proc/sys/vm/drop_caches
run_science deep bash -c \
  'echo "$BASHPID" >/sys/fs/cgroup/v140-evaluate/cgroup.procs; exec "$@"' \
  v140 .venv/bin/python repo/scripts/v140_budgeted_page_rank.py \
...
printf '%s\n' '{"schema":"borsuk-v140-decision-v1","deep_completed":true,"relaion_completed":true}' >decision.json
{
  cat /sys/fs/cgroup/v140-evaluate/memory.current
  cat /sys/fs/cgroup/v140-evaluate/memory.peak
  cat /sys/fs/cgroup/v140-evaluate/memory.stat
} >cgroup-memory.txt
phase=complete
exit 0
