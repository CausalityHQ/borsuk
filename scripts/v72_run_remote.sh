...
aws s3 cp "$prefix/v72_resident_row_router.py" probe.py --only-show-errors || exit 92
...
set +e
/usr/bin/time -v .venv/bin/python probe.py --source source.parquet --development-query query.parquet   --ground-truth gt.parquet --layout-order layout-order.npy   --output result.json > run.log 2> time.log
code=$?
set -e
[ "$code" -ne 0 ] && exit "$code"
