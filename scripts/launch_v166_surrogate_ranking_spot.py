#!/usr/bin/env python3
"""Launch one immutable GT-blind V166 ranking probe on Causality Spot."""
...
SCHEMA = "borsuk-v166-surrogate-ranking-spot-v1"
TAG = "borsuk-v166-surrogate-ranking"
WALL_SECONDS = 14_400
...
ARTIFACTS = (
    "out/means.npy", "out/residuals.npy", "out/cases.jsonl",
...
def user_data(commit: str, archive_sha: str, archive_key: str, prefix: str) -> str:
    return f'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v166-hard-stop --on-active={WALL_SECONDS}s /usr/sbin/shutdown -h now
root=/mnt/v166-surrogate-ranking
...
/usr/bin/time -v -o prepare-resources.txt .venv/bin/python -m \
  scripts.v166_surrogate_ranking_run prepare --output out \
...
/usr/bin/time -v -o plan-resources.txt .venv/bin/python -m \
  scripts.v166_surrogate_ranking_run plan --output out
phase=check
/usr/bin/time -v -o check-resources.txt .venv/bin/python -m \
  scripts.v166_surrogate_ranking_run check --output out >check.json
phase=complete
'''
...
def main() -> None:
    ...
    if subprocess.run(["git", "merge-base", "--is-ancestor", "HEAD",
        ...
        raise ValueError("V166 source commit is not pushed to origin/main")
    archive = archive_source(commit)
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as tar:
        required = {"scripts/v166_surrogate_core.py", "scripts/v166_surrogate_probe.py",
                    "scripts/v166_surrogate_ranking_run.py",
                    "docs/research/v166-surrogate-ranking-prereg.md"}
        if not required.issubset(tar.getnames()):
            raise ValueError("source archive lacks V166 gate")
    archive_sha = hashlib.sha256(archive).hexdigest()
    archive_key = f"research/v166-surrogate-ranking/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v166-surrogate-ranking/{commit}/runs/{attempt}"
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V166 worker already active")
    ...
    receipt = ec2.run_instances(
        ClientToken="v166-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
    ...
    while time.monotonic() < deadline:
        if not missing(s3, prefix + "/terminal.json"):
            ...
            for name, identity in terminal.get("artifacts", {}).items():
                ...
                if size != identity["bytes"] or digest.hexdigest() != identity["sha256"]:
                    raise ValueError(f"V166 S3 artifact read-back differs: {name}")
            terminal["terminal_sha256"] = hashlib.sha256(raw).hexdigest()
            print(json.dumps(terminal, sort_keys=True), flush=True)
            if terminal.get("status") != "complete":
                raise RuntimeError("V166 terminal reports failed; see closed artifacts")
            return
        ...
        if state in {"terminated", "shutting-down"}:
            ...
            if time.monotonic() - started >= WALL_SECONDS - 60:
                raise TimeoutError("V166 worker reached wall cap before terminal")
            raise RuntimeError("Spot worker stopped before terminal; restart full cell")
        time.sleep(20)
    ec2.terminate_instances(InstanceIds=[instance_id])
    raise TimeoutError("V166 cell exceeded wall cap")
