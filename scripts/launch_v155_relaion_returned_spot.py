#!/usr/bin/env python3
"""Launch and monitor one frozen V155 D768 returned-quality Spot attempt."""
...
REGION = "eu-central-1"
TAG = "borsuk-v155-relaion-returned-quality"
RUNNER = "scripts/run_v155_relaion_returned_remote.sh"
INPUTS = (
    ...
    ("research/v142-relaion-returned/da63de3292905fe9246ee50c078345592e7c9083/runs/v142-20260924T100356Z/a0003/terminal.json", 2_165),
    ("research/v142-relaion-returned/da63de3292905fe9246ee50c078345592e7c9083/runs/v142-20260924T100356Z/a0003/artifacts/scored.jsonl", 16_714_942),
    ("research/v154-relaion-graph/8f2c5ce4bffadc421fb4a48e4deccf989f803770/runs/v154-20260924T143744Z/a0002/terminal.json", 2_672),
    ("research/v154-relaion-graph/8f2c5ce4bffadc421fb4a48e4deccf989f803770/runs/v154-20260924T143744Z/a0002/artifacts/science.jsonl", 99_226_409),
)
...
def validate(plan: Plan) -> None:
    ...
    if (
        ...
        or not output_key.startswith(
            f"research/v155-relaion-returned-quality/{plan.source_commit}/runs/"
        )
        ...
        raise ValueError("immutable V155 plan differs")
...
def user_data(plan: Plan) -> str:
    ...
    exports = "\n".join(
        (
            f"export V155_SOURCE_COMMIT={plan.source_commit}",
            f"export V155_ARCHIVE_SHA256={plan.archive_sha256}",
            f"export V155_OUTPUT_PREFIX={shlex.quote(plan.output_prefix)}",
            f"export V155_WALL_SECONDS={plan.wall_seconds}",
        )
    )
    return f"""#!/bin/bash
set -euo pipefail
systemd-run --unit=v155-hard-stop --on-active={plan.wall_seconds}s /usr/sbin/shutdown -h now
...
PY
    aws s3 cp terminal.json "$V155_OUTPUT_PREFIX/terminal.json" --only-show-errors || true
    shutdown -h now || true
  fi
}}
...
def launch_spec(plan: Plan) -> dict:
    token = "v155-" + hashlib.sha256(plan.output_prefix.encode()).hexdigest()[:48]
    return {
        "ClientToken": token,
...
def check_source_and_inputs(s3: object, plan: Plan) -> None:
    ...
    with tarfile.open(fileobj=archive, mode="r:gz") as source:
        if (RUNNER not in source.getnames()
                or "scripts/v155_relaion_returned_quality.py" not in source.getnames()
                or "scripts/v155_independent_recount.py" not in source.getnames()
                ...
                or "docs/research/v155-relaion-returned-quality-prereg.md" not in source.getnames()):
            raise ValueError("source archive lacks V155 runner or evaluator")
...
def reserve(plan: Plan) -> None:
    receipt = json.dumps({
        "schema": "borsuk-v155-relaion-returned-quality-reservation-v1",
...
def launch_and_monitor(plan: Plan) -> dict:
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V155 worker already active")
    ...
    try:
        while time.monotonic() < deadline:
            if not missing(s3, prefix + "/terminal.json"):
                ...
                if (terminal.get("schema") != "borsuk-v155-relaion-returned-quality-spot-v1"
                    ...
                    raise ValueError("V155 terminal identity differs")
                ...
                required = {"install.log", "compile.log", "compile-resources.txt",
                            "download.log", "prepare-resources.txt", "sparse.raw.jsonl",
                            "flat-score-resources.txt", "flat.scored.jsonl",
                            "sparse-score-resources.txt", "sparse.scored.jsonl",
                            "replay-resources.txt", "replay.jsonl", "replay.seal.json",
                            "seal-upload.log",
                ...
                if terminal.get("status") == "complete":
                    ...
                    if decision.get("schema") != "borsuk-v155-decision-v1" or decision.get(
                            "verdict") not in {"pass", "reject"}:
                        raise ValueError("V155 decision differs")
                    if not required.issubset(terminal.get("artifacts", {})):
                        raise ValueError("V155 complete terminal lacks required artifacts")
                    terminal["decision"] = decision
                artifacts = terminal.get("artifacts", {})
                needed = {"replay.jsonl", "evidence.jsonl", "summary.json",
                          "truth.ids.bin", "sq8.ids.bin", "replay.seal.json"}
                authenticated = {}
                for name, metadata in artifacts.items():
                    ...
                    if length != metadata["bytes"] or digest.hexdigest() != metadata["sha256"]:
                        raise ValueError(f"V155 artifact differs: {name}")
                    if name in needed:
                        authenticated[name] = b"".join(chunks)
                if terminal.get("status") == "complete":
                    seal = json.loads(authenticated["replay.seal.json"])
                    if (seal.get("plans_sha256") !=
                            "ab9bac04c32445a85928dd8978468b1c40d937da2f89960e71d8d21d132ea4d1"
                            or seal.get("flat_scored_sha256") !=
                            artifacts["flat.scored.jsonl"]["sha256"]
                            or seal.get("sparse_scored_sha256") !=
                            artifacts["sparse.scored.jsonl"]["sha256"]
                            or seal.get("replay_sha256") !=
                            artifacts["replay.jsonl"]["sha256"]
                            or seal.get("replay_sha256") !=
                            json.loads(authenticated["summary.json"])["replay_sha256"]):
                        raise ValueError("V155 GT-blind replay seal differs")
                    import sys
                    sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
                    from scripts.v155_independent_recount import recount
                    with tempfile.TemporaryDirectory() as directory:
                        paths = {}
                        for name in needed:
                            path = Path(directory) / name
                            path.write_bytes(authenticated[name])
                            paths[name] = path
                        result = recount(paths["replay.jsonl"], paths["evidence.jsonl"],
                                         paths["summary.json"], paths["truth.ids.bin"],
                                         paths["sq8.ids.bin"], paths["replay.seal.json"])
                    if result["verdict"] != terminal["decision"]["verdict"]:
                        raise ValueError("V155 independent verdict differs")
                    terminal["independent_recount"] = result
                terminal["authenticated_artifact_count"] = len(artifacts)
                print(json.dumps(terminal, sort_keys=True), flush=True)
                return terminal
            state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
            if state == "terminated":
                raise RuntimeError("V155 instance terminated without terminal marker")
            time.sleep(15)
        raise TimeoutError(f"V155 terminal missing after worker deadline: {instance_id}")
...
def main() -> None:
    ...
    if terminal.get("status") != "complete" or terminal.get("exit_code") != 0:
        raise RuntimeError("V155 terminal reports a failed attempt")
