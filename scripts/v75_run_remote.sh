#!/bin/bash
set -u
root=/mnt/v75-validation
prefix=s3://borsuk-bench-453182569524-euc1/research/v75-algorithm-first/validation
...
aws s3 cp "$prefix/v73_export_manifest.py" export.py --only-show-errors || exit 91
...
89abdb9c94fc6ebc1a3252a2a878548ab7d2c60d9b100c565c16a279911292bf  export.py
HASHES
[ "$?" -ne 0 ] && exit 92
phase=build
...
aws s3 cp "$base/validation-query.parquet" query.parquet --only-show-errors || exit 96
aws s3 cp "$base/validation-gt100.parquet" gt.parquet --only-show-errors || exit 96
...
export BORSUK_V71_URI="$sq8" BORSUK_V71_MANIFEST="$root/manifest.bin" BORSUK_V71_QUERIES=1000
: > run.log
for point in "512 2 128" "256 2 128"; do
  set -- $point
  BORSUK_V71_SHORTLIST=$1 BORSUK_V71_GAP=$2 BORSUK_V71_CONCURRENCY=$3     BORSUK_V71_OUTPUT="$root/result-s$1-g$2-c$3.json"     ./reader/target/release/v71_native_reader >> run.log 2>&1 || exit 98
done
phase=complete
exit 0
