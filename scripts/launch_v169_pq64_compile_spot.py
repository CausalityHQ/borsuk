#!/usr/bin/env python3
"""Run one immutable narrow PQ64 crate test on Causality Spot."""
...
SCHEMA = "borsuk-v169-pq64-compile-spot-v1"
TAG = "borsuk-v169-pq64-compile"
...
def user_data(commit: str, archive_sha: str, archive_key: str,
    ...
    return f'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v169-pq64-compile-stop --on-active={WALL_SECONDS}s /usr/sbin/shutdown -h now
root=/mnt/v169-pq64-compile
...
/usr/bin/time -v -o test-resources.txt timeout --signal=TERM --kill-after=30 3300 \
  ...
  -p borsuk --lib pq64_nominee::tests --jobs 4 -- --nocapture >test.log 2>&1
phase=complete
'''
...
def launch(attempt: str) -> None:
    ...
    if subprocess.run(["git", "merge-base", "--is-ancestor", "HEAD",
        ...
        raise ValueError("V169 source commit is not pushed to origin/main")
    archive = archive_source(commit)
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as tar:
        if not {"crates/borsuk/src/pq64_nominee.rs", "Cargo.toml",
                "Cargo.lock"}.issubset(tar.getnames()):
            raise ValueError("source archive lacks PQ64 crate test")
    archive_sha = hashlib.sha256(archive).hexdigest()
    archive_key = f"research/v169-pq64-compile/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v169-pq64-compile/{commit}/runs/{attempt}"
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V169 compile worker already active")
    ...
    put_if_absent(prefix + "/reservation.json", json.dumps({
        ...
        "source_archive_sha256": archive_sha,
        "test": "cargo test --locked -p borsuk --lib pq64_nominee::tests",
        "interruption_policy": "discard and restart under a new attempt",
    }, sort_keys=True).encode())
    receipt = ec2.run_instances(
        ClientToken="v169pq64-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
    ...
    while time.monotonic() - started < WALL_SECONDS + 600:
        if not missing(s3, prefix + "/terminal.json"):
            ...
            for name, identity in terminal.get("artifacts", {}).items():
                ...
                if size != identity["bytes"] or digest.hexdigest() != identity["sha256"]:
                    raise ValueError(f"V169 S3 artifact differs: {name}")
            terminal["terminal_sha256"] = hashlib.sha256(raw).hexdigest()
            print(json.dumps(terminal, sort_keys=True), flush=True)
            if terminal.get("status") != "complete":
                raise RuntimeError("V169 compile terminal failed; see closed artifacts")
            return
        ...
        time.sleep(20)
    ec2.terminate_instances(InstanceIds=[instance_id])
    raise TimeoutError("V169 compile cell exceeded wall cap")
...
def main() -> None:
    ...
    with open("/tmp/borsuk-v169-pq64-compile-launch.lock", "a+") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as exc:
            raise RuntimeError("V169 compile launcher is already running here") from exc
        launch(args.attempt)
