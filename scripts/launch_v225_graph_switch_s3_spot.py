#!/usr/bin/env python3
"""One immutable Spot attempt for an S3 graph generation update CAS."""
...
ARTIFACTS = {"switch.json", "switch.time", "build.log", "install.log", "run-closed.log"}


def bootstrap(commit, archive_sha, archive_key, prefix):
    return f"""#!/bin/bash
set -euo pipefail
systemd-run --unit=v225-hard-stop --on-active=7200s /usr/sbin/shutdown -h now
root=/mnt/v225-graph-switch
mkdir -p "$root" && cd "$root"
aws s3 cp 's3://{BUCKET}/{archive_key}' source.tar.gz --only-show-errors
printf '%s  source.tar.gz\\n' '{archive_sha}' | sha256sum -c -
mkdir repo && tar -xzf source.tar.gz -C repo
export BORSUK_V225_BUCKET='{BUCKET}'
export BORSUK_V225_PREFIX='{prefix}'
export BORSUK_V225_SOURCE_COMMIT='{commit}'
export BORSUK_V225_ARCHIVE_SHA='{archive_sha}'
exec bash repo/scripts/run_v225_graph_switch_s3.sh
"""
...
def launch(attempt):
    ...
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as source:
        if not {"scripts/run_v225_graph_switch_s3.sh",
                "crates/borsuk/examples/v225_graph_switch_s3.rs",
                "docs/research/v225-graph-switch-s3-prereg.md"}.issubset(source.getnames()):
            raise ValueError("source archive lacks V225 gate inputs")
    archive_sha = hashlib.sha256(archive).hexdigest()
    archive_key = f"research/v225-graph-switch-s3/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v225-graph-switch-s3/{commit}/runs/{attempt}"
    ...
    put_if_absent(prefix + "/reservation.json", json.dumps({
        "schema": "borsuk-v225-graph-switch-s3-reservation-v1",
        "source_commit": commit, "source_archive_sha256": archive_sha,
        "hardware": "c7i.4xlarge Spot eu-central-1c",
        "gate": "old reader ID42/gen7;new reader ID99/gen8;stale S3 CAS rejects",
    ...
    try:
        ...
        request["TagSpecifications"][0]["Tags"][0]["Value"] = "borsuk-v225-graph-switch-s3"
        ...
        if (terminal.get("schema") != "borsuk-v225-graph-switch-s3-terminal-v1"
        ...
        result = json.loads(s3.get_object(Bucket=BUCKET,
            Key=prefix + "/artifacts/switch.json")["Body"].read())
        gate_pass = (result.get("schema") == "borsuk-v225-graph-switch-s3-v1"
                     and result.get("old_generation") == 7
                     and result.get("new_generation") == 8
        ...
        closeout = {"schema": "borsuk-v225-graph-switch-s3-closeout-v1",
                    "source_commit": commit, "source_archive_sha256": archive_sha,
                    "instance_id": instance_id, "terminal_sha256": terminal_sha,
                    "switch": result, "gate_pass": gate_pass,
                    "spot_quote_usd_per_hour": spot_price,
        ...
        if not gate_pass:
            raise RuntimeError("V225 switch gate failed")
    finally:
