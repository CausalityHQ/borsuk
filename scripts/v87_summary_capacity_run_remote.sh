...
publish_terminal() {
  ...
  if [ "$code" -eq 0 ]; then
    for name in dependency.log hashes.log hashes-check.log instance.json run.log time.log result.json; do
      if [ ! -s "$name" ] || ! aws s3 cp "$name" "$V87_OUTPUT_URI/$name" --only-show-errors; then
        code=98
    ...
    for name in dependency.log hashes.log hashes-check.log instance.json run.log time.log result.json; do
      if [ -n "${V87_OUTPUT_URI:-}" ] && [ -f "$name" ]; then
        aws s3 cp "$name" "$V87_OUTPUT_URI/$name" --only-show-errors || true
      fi
    done
  fi
  printf '{"exit_code":%d,"phase":"%s","source_commit":"%s"}\n' \
    "$code" "$phase" "${V87_SOURCE_COMMIT:-unknown}" >/tmp/v87-summary-capacity-terminal.json
  if [ -n "${V87_OUTPUT_URI:-}" ] && ! aws s3 cp /tmp/v87-summary-capacity-terminal.json \
    "$V87_OUTPUT_URI/terminal.json" --only-show-errors; then
    code=99
  fi
  unlink /tmp/v87-summary-capacity-terminal.json
  sync
  shutdown -h now || true
  exit "$code"
}
...
phase=inputs
aws s3 cp "$V87_SCREEN_URI" scripts/v87_summary_capacity_screen.py \
  --only-show-errors || exit 92
aws s3 cp "$V87_V86_URI" scripts/v86_coarse_to_fine_screen.py \
  --only-show-errors || exit 92
aws s3 cp "$V87_SHARED_URI" scripts/v85_shared_overlay_screen.py \
  --only-show-errors || exit 92
printf '%s  scripts/v87_summary_capacity_screen.py\n%s  scripts/v86_coarse_to_fine_screen.py\n%s  scripts/v85_shared_overlay_screen.py\n' \
  "$V87_SCREEN_SHA256" "$V87_V86_SHA256" "$V87_SHARED_SHA256" >hashes.log
...
/usr/bin/time -v -o time.log timeout --signal=TERM --kill-after=30s 1200s \
  .venv/bin/python -m scripts.v87_summary_capacity_screen \
