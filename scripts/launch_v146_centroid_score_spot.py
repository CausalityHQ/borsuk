#!/usr/bin/env python3
"""Launch and monitor one frozen V146 centroid-scoring Spot attempt."""
...
REGION = "eu-central-1"
TAG = "borsuk-v146-centroid-score"
RUNNER = "scripts/run_v146_centroid_score_remote.sh"
INPUTS = (
    ...
    ("research/v145-page-parity/e0e8e01f89be2c40438e09f325d9c1f72da1ca76/runs/v145-20260924T110000Z/a0001/terminal.json", 3_349),
    ("research/v145-page-parity/e0e8e01f89be2c40438e09f325d9c1f72da1ca76/runs/v145-20260924T110000Z/a0001/artifacts/deep.scores.bin", 1_564_000),
    ("research/v145-page-parity/e0e8e01f89be2c40438e09f325d9c1f72da1ca76/runs/v145-20260924T110000Z/a0001/artifacts/relaion.scores.bin", 15_628_000),
)
...
def validate(plan: Plan) -> None:
    ...
    if (
        ...
        or not output_key.startswith(
            f"research/v146-centroid-score/{plan.source_commit}/runs/"
        )
        ...
        raise ValueError("immutable V146 plan differs")
...
def user_data(plan: Plan) -> str:
    ...
    exports = "\n".join(
        (
            f"export V146_SOURCE_COMMIT={plan.source_commit}",
            f"export V146_ARCHIVE_SHA256={plan.archive_sha256}",
            f"export V146_OUTPUT_PREFIX={shlex.quote(plan.output_prefix)}",
            f"export V146_WALL_SECONDS={plan.wall_seconds}",
        )
    )
    return f"""#!/bin/bash
set -euo pipefail
systemd-run --unit=v146-hard-stop --on-active={plan.wall_seconds}s /usr/sbin/shutdown -h now
...
PY
    aws s3 cp terminal.json "$V146_OUTPUT_PREFIX/terminal.json" --only-show-errors || true
    shutdown -h now || true
  fi
}}
...
def launch_spec(plan: Plan) -> dict:
    token = "v146-" + hashlib.sha256(plan.output_prefix.encode()).hexdigest()[:48]
    return {
        "ClientToken": token,
...
def check_source_and_inputs(s3: object, plan: Plan) -> None:
    ...
    with tarfile.open(fileobj=archive, mode="r:gz") as source:
        if (RUNNER not in source.getnames()
                or "scripts/v140_budgeted_page_rank.py" not in source.getnames()
                or "scripts/v114_score_rust/src/bin/v146_centroid_score.rs" not in source.getnames()
                ...
                or "scripts/v139_centroid_page_screen.py" not in source.getnames()
                or "docs/research/inputs/v139-relaion-primary.jsonl" not in source.getnames()):
            raise ValueError("source archive lacks V146 runner or evaluator")
...
def reserve(plan: Plan) -> None:
    receipt = json.dumps({
        "schema": "borsuk-v146-centroid-score-reservation-v1",
...
def launch_and_monitor(plan: Plan) -> dict:
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V146 worker already active")
    ...
    try:
        while time.monotonic() < deadline:
            if not missing(s3, prefix + "/terminal.json"):
                ...
                if (terminal.get("schema") != "borsuk-v146-centroid-score-spot-v1"
                ...
                required = {"install.log", "core-check-resources.txt",
                            "score-build-resources.txt", "download.log",
                            "deep-score-resources.txt", "deep.centroids.bin",
                            "deep.score.jsonl", "deep.score.summary.json",
                            "deep.rust-scores.bin", "deep.cgroup-memory.txt",
                            "decision.json", "worker.log"}
                if terminal.get("status") == "complete":
                    decision = json.loads(s3.get_object(
                        Bucket=BUCKET, Key=prefix + "/artifacts/decision.json")["Body"].read())
                    if (decision.get("schema") != "borsuk-v146-decision-v1"
                            or decision.get("deep_completed") is not True
                            or decision.get("verdict") not in {
                                "pass", "reject-deep-parity", "reject-relaion-parity",
                                "reject-cpu"}):
                        raise ValueError("V146 decision differs")
                    if decision["relaion_completed"] is True:
                        required |= {"relaion-score-resources.txt",
                                     "relaion.centroids.bin", "relaion.rust-scores.bin",
                                     "relaion.score.jsonl", "relaion.score.summary.json",
                                     "relaion.cgroup-memory.txt"}
                    elif decision != {"schema": "borsuk-v146-decision-v1",
                                       "deep_completed": True,
                                       "relaion_completed": False,
                                       "verdict": "reject-deep-parity"}:
                        raise ValueError("V146 early-stop decision differs")
                    artifacts = terminal.get("artifacts", {})
                    if not required.issubset(artifacts):
                        raise ValueError("V146 complete terminal lacks required artifacts")
                ...
                for name, metadata in artifacts.items():
                    ...
                    if length != metadata["bytes"] or digest.hexdigest() != metadata["sha256"]:
                        raise ValueError(f"V146 artifact differs: {name}")
                terminal["authenticated_artifact_count"] = len(artifacts)
                print(json.dumps(terminal, sort_keys=True), flush=True)
                return terminal
            state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
            if state == "terminated":
                raise RuntimeError("V146 instance terminated without terminal marker")
            time.sleep(15)
        raise TimeoutError(f"V146 terminal missing after worker deadline: {instance_id}")
...
def main() -> None:
    ...
    if (terminal.get("status") != "complete" or terminal.get("exit_code") != 0
        ...
        raise RuntimeError("V146 terminal reports a failed attempt")
