#!/usr/bin/env python3
"""Independent replay of the closed V202 exact-witness Spot cell."""
...
from scripts.launch_v202_two_cap_cover_spot import BUCKET, INPUTS, REGION
from scripts.unconstrained_priced_interval import unconstrained_priced_cover

PREFIX = ("research/v202-two-cap-cover/"
          "576479788947be9c3d9df26a4b65b794a1079fbd/runs/a0001/")
TERMINAL_SHA = "35e57bb75b854b7750c4c7e23794b3f3015f7007729135356c4d87139c2bf777"
SOURCE = "576479788947be9c3d9df26a4b65b794a1079fbd"
INSTANCE = "i-0516b444461a3a38e"
...
def check() -> dict:
    ...
    if len(rows) != len(weights) or len(rows) != len(plans) or len(rows) != 1000:
        raise ValueError("V202 cohort length differs")
    ...
    for ordinal, (row, source, plan) in enumerate(zip(rows, weights, plans, strict=True)):
        if (row["ordinal"] != ordinal or source["ordinal"] != ordinal
            ...
            raise ValueError(f"V202 ordinal differs: {ordinal}")
        tier = row["tier"]
        if tier not in counts:
            raise ValueError(f"V202 tier differs: {ordinal}")
        counts[tier] += 1
        ...
        if (tier == "linear") != linear_admitted:
            raise ValueError(f"V202 linear admission differs: {ordinal}")
        if tier in ("linear", "unit", "get", "hard"):
            ...
            if (not reference["feasible"] or row["intervals"] != reference["intervals"]
                ...
                raise ValueError(f"V202 exact physical witness differs: {ordinal}")
            if tier == "linear" and (row["intervals"] != [list(pair) for pair in linear.intervals]
                                     or row["mass"] != linear.mass):
                raise ValueError(f"V202 Rust/Python linear cover differs: {ordinal}")
    ...
    if any(bench[key] != value for key, value in expected_counts.items()):
        raise ValueError("V202 tier counts differ")
    for name, times in (("linear_ns", linear_times), ("unit_ns", unit_times),
        ...
        if bench[name] != expected:
            raise ValueError(f"V202 percentile differs: {name}")
    if bench.get("peak_process_rss_bytes", 0) <= 0:
        raise ValueError("V202 RSS missing")
