"""Create-only Spot worker for the 1M page-local two-bit code-wave projection."""
...
def two_bit_code_wave_worker_script(plan: object) -> str:
    """Reuse the sealed source map, then separate query-plan and truth phases."""
    script = page_oracle_worker_script(plan)
    script = _once(script, "/mnt/native-one-million-page-oracle", "/mnt/native-two-bit-code-wave")
    script = _once(
        script, "borsuk-one-million-opq8-page-oracle-terminal-v1",
        "borsuk-one-million-two-bit-code-wave-terminal-v1",
    )
    downloads = "\n".join(
...
run_capped /usr/bin/time -v -o plan-resources.txt timeout --foreground @WALL@ unshare --net --fork setpriv --reuid=nobody --regid=nobody --clear-groups env -i PATH="$PATH" PYTHONPATH="$root/repo" OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 "$root/.venv/bin/python" -m scripts.native_one_million_two_bit_code_projection_cell plan --root "$root" --out "$root/planning"
mv planning/two-bit-code-wave-plans.json two-bit-code-wave-plans.json
mv planning/two-bit-code-wave-plan-seal.json two-bit-code-wave-plan-seal.json
rmdir planning
check_resources plan-resources.txt
chmod 0444 two-bit-code-wave-plans.json two-bit-code-wave-plan-seal.json
for name in two-bit-code-wave-plans.json two-bit-code-wave-plan-seal.json plan-resources.txt; do publish_artifact "$name"; done
""".replace("@WALL@", str(plan.wall_seconds))
    ...
    script = _once(
        ...
        "scripts.native_one_million_two_bit_code_projection_cell evaluate",
    )
    for old, new in (
        ("mv evaluation/evidence.json evidence.json", "mv evaluation/two-bit-code-wave-evidence.json two-bit-code-wave-evidence.json"),
        ("mv evaluation/result.json result.json", "mv evaluation/two-bit-code-wave-result.json two-bit-code-wave-result.json"),
        ("for name in evidence.json result.json evaluate-resources.txt; do publish_artifact \"$name\"; done",
         "for name in two-bit-code-wave-evidence.json two-bit-code-wave-result.json evaluate-resources.txt; do publish_artifact \"$name\"; done"),
        ("scripts.validate_native_one_million_page_oracle",
         "scripts.validate_native_one_million_two_bit_code_projection"),
        ("for name in validation.json validate-resources.txt; do publish_artifact \"$name\"; done",
         "for name in two-bit-code-wave-validation.json validate-resources.txt; do publish_artifact \"$name\"; done"),
    ...
    if (
        ...
        or "scripts.native_one_million_two_bit_code_projection_cell plan" not in script
        or "phase=plan\n" not in script
        or "phase=evaluate\n" not in script
    ):
