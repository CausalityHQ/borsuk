"""Independent replay of closed 100k sign96 range plans and truth masks."""
...
from scripts.native_rotated_sign96 import BATCH_ROWS, score_records
from scripts.native_rotated_sign96_groups import open_sign96_groups
...
def _metrics(samples: list[dict[str, object]], plans: list[dict[str, object]]) -> dict[str, object]:
    ...
    metrics["paired_gt100"] = {
        "sign96_better": sum(sample["sign96"]["hits_at_100"] > sample["source"]["hits_at_100"] for sample in samples),
        "source_better": sum(sample["sign96"]["hits_at_100"] < sample["source"]["hits_at_100"] for sample in samples),
        "tied": sum(sample["sign96"]["hits_at_100"] == sample["source"]["hits_at_100"] for sample in samples),
    }
    return metrics
...
def validate_closed(
    ...
    if (
        ...
        raise ValueError("sign96 validation source seal differs")
    reader = open_sign96_groups(
        ...
        seal_sha256=source_seal["sign_seal_sha256"], rotation_seed=ROTATION_SEED,
    )
    if reader.page_row_counts != counts:
        raise ValueError("sign96 validation code layout differs")
    plans = _sealed_plans(root, query_count)
    evidence, evidence_body = _read_canonical(out / "sign96-evidence.json")
    result, result_body = _read_canonical(out / "sign96-result.json")
    if (
        ...
        raise ValueError("sign96 validation evidence identity differs")
    _, truth = _read_queries_truth(root, FROZEN_INPUTS)
    if len(truth) != query_count:
        raise ValueError("sign96 validation truth cohort differs")
    ...
    if len(page_lengths["base"]) != len(counts):
        raise ValueError("sign96 validation page layout differs")
    queries = _read_geometric_queries(
        root / "queries.parquet", FROZEN_INPUTS.queries, 768,
    )
    if len(queries) != query_count:
        raise ValueError("sign96 validation query cohort differs")
    ...
    for query, sample in zip(queries, plans["samples"], strict=True):
        ...
        sign_scores = score_records(
            query, reader.mean, records[positions], rotation_seed=ROTATION_SEED,
        )
        ...
        if (
            sample["sign96"]["priority_pages"] != _independent_page_priority(
                sign_scores, pages, positions,
            )
            or sample["source"]["priority_pages"] != _independent_page_priority(
                source_scores, pages, positions,
            )
        ):
            raise ValueError("sign96 validation row-score priority differs")
    ...
    for ordinal, (plan, neighbors) in enumerate(zip(plans["samples"], truth, strict=True)):
        if (
            ...
            raise ValueError("sign96 validation truth owners differ")
    ...
    for sample, plan in zip(recounted, plans["samples"], strict=True):
        signed, exact = plan["sign96"], plan["source"]
    ...
    if (
        ...
        raise ValueError("sign96 validation masks or metrics differ")
    validation = {
    ...
    (out / "sign96-validation.json").write_bytes(_canonical(validation))
    return validation
