#!/usr/bin/env python3
"""Launch and authenticate one frozen V152 fresh returned-quality Spot attempt."""
...
REGION = "eu-central-1"
TAG = "borsuk-v152-cached-page-graph"
RUNNER = "scripts/run_v152_cached_page_graph_remote.sh"
...
def validate(plan: Plan) -> None:
    ...
    if (
        ...
        or not output_key.startswith(
            f"research/v152-cached-page-graph/{plan.source_commit}/runs/"
        )
        ...
        raise ValueError("immutable V152 plan differs")
...
def user_data(plan: Plan) -> str:
    ...
    exports = "\n".join(
        (
            f"export V152_SOURCE_COMMIT={plan.source_commit}",
            f"export V152_ARCHIVE_SHA256={plan.archive_sha256}",
            f"export V152_OUTPUT_PREFIX={shlex.quote(plan.output_prefix)}",
            f"export V152_WALL_SECONDS={plan.wall_seconds}",
        )
    )
    return f"""#!/bin/bash
set -euo pipefail
systemd-run --unit=v152-hard-stop --on-active={plan.wall_seconds}s /usr/sbin/shutdown -h now
root=/mnt/v152-cached-page-graph
...
import json,os
print(json.dumps({{"schema":"borsuk-v152-cached-page-graph-spot-v1",
  "source_commit":os.environ["V152_SOURCE_COMMIT"],
  "source_archive_sha256":os.environ["V152_ARCHIVE_SHA256"],
  "instance_id":os.environ["INSTANCE_ID"],"exit_code":int(os.environ["EXIT_CODE"]),
  "phase":"bootstrap","status":"failed","artifacts":{{}}}},
  sort_keys=True,separators=(',',':')))
PY
    aws s3 cp terminal.json "$V152_OUTPUT_PREFIX/terminal.json" --only-show-errors || true
    shutdown -h now || true
  fi
}}
...
def launch_spec(plan: Plan) -> dict:
    token = "v152-" + hashlib.sha256(plan.output_prefix.encode()).hexdigest()[:48]
    return {
        "ClientToken": token,
...
def check_source_and_inputs(s3: object, plan: Plan) -> None:
    ...
    with tarfile.open(fileobj=archive, mode="r:gz") as source:
        if (RUNNER not in source.getnames()
                or "crates/borsuk/src/bin/v152_cached_page_graph.rs" not in source.getnames()
                ...
                or "scripts/v152_prepare_fresh.py" not in source.getnames()
                or "scripts/v152_returned_quality.py" not in source.getnames()
                or "scripts/v152_returned_audit.py" not in source.getnames()
                or "scripts/v152_independent_recount.py" not in source.getnames()
                or "docs/research/v152-cached-page-graph-prereg.md" not in source.getnames()):
            raise ValueError("source archive lacks V152 runner or evaluator")
...
def reserve(plan: Plan) -> None:
    receipt = json.dumps({
        "schema": "borsuk-v152-cached-page-graph-reservation-v1",
...
def recount_science(s3: object, prefix: str, decision: dict,
    ...
    from scripts.v152_independent_recount import recount
...
def launch_and_monitor(plan: Plan) -> dict:
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V152 worker already active")
    ...
    try:
        while time.monotonic() < deadline:
            if not missing(s3, prefix + "/terminal.json"):
                ...
                if (terminal.get("schema") != "borsuk-v152-cached-page-graph-spot-v1"
                    ...
                    raise ValueError("V152 terminal identity differs")
                ...
                if terminal.get("status") == "complete":
                    ...
                    if (decision.get("schema") != "borsuk-v152-decision-v1"
                            or decision.get("verdict") not in {"pass", "reject"}):
                        raise ValueError("V152 decision differs")
                    artifacts = terminal.get("artifacts", {})
                    if not required.issubset(artifacts):
                        raise ValueError("V152 complete terminal lacks required artifacts")
                ...
                for name, metadata in artifacts.items():
                    ...
                    if length != metadata["bytes"] or digest.hexdigest() != metadata["sha256"]:
                        raise ValueError(f"V152 artifact differs: {name}")
            ...
            state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
            if state == "terminated":
                raise RuntimeError("V152 instance terminated without terminal marker")
            time.sleep(15)
        raise TimeoutError(f"V152 terminal missing after worker deadline: {instance_id}")
...
def main() -> None:
    ...
    if terminal.get("status") != "complete" or terminal.get("exit_code") != 0:
        raise RuntimeError("V152 terminal reports an infrastructure failure")
