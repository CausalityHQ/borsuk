#!/bin/bash
# One frozen, fail-fast, source-only centroid-page feasibility screen on Spot.
set -euo pipefail
...
deadline_epoch=$((started_epoch + V139_WALL_SECONDS))
monitor_pid=
imds_token=
imds() {
...
finish() {
  ...
  for path in install.log download.log deep.log relaion.log \
    ...
    if [ -f "$path" ] && ! timeout 900 aws s3 cp "$path" \
        "$V139_OUTPUT_PREFIX/artifacts/$path" --only-show-errors; then
      upload_failed=1
    fi
  done
...
PY
  aws s3 cp terminal.json "$V139_OUTPUT_PREFIX/terminal.json" --only-show-errors
  shutdown -h now || true
  exit "$code"
}
trap finish EXIT
...
phase=evaluate-deep
mkdir /sys/fs/cgroup/v139-evaluate
sync
echo 3 >/proc/sys/vm/drop_caches
run_science deep bash -c \
  'echo "$BASHPID" >/sys/fs/cgroup/v139-evaluate/cgroup.procs; exec "$@"' \
  v139 .venv/bin/python repo/scripts/v139_centroid_page_screen.py \
...
raise SystemExit(0 if summary['stop_centroid_policy'] else 1)
PY
then
  printf '%s\n' '{"schema":"borsuk-v139-decision-v1","deep_stop":true,"relaion_skipped":true}' >decision.json
else
...
print(json.dumps({'schema':'borsuk-v139-decision-v1','deep_stop':False,
  'relaion_skipped':False,'relaion_stop':relaion['stop_centroid_policy'],
  'centroid_policy_feasible':not relaion['stop_centroid_policy']},
  sort_keys=True,separators=(',',':')))
PY
fi
{
  cat /sys/fs/cgroup/v139-evaluate/memory.current
  cat /sys/fs/cgroup/v139-evaluate/memory.peak
  cat /sys/fs/cgroup/v139-evaluate/memory.stat
} >cgroup-memory.txt
phase=complete
exit 0
