...
output_prefix=$V104_OUTPUT_PREFIX
...
finish() {
  ...
  STARTED_EPOCH="$started_epoch" FINISHED_EPOCH="$finished_epoch" \
    ...
    SWAP_END_KIB="$swap_end_kib" SPOT_PRICE_MICROS="$V104_SPOT_PRICE_MICROS" \
    python3.12 - <<'PY'
import json, os
...
if status == "complete":
    for role, filename in (
        ...
        evidence[role] = Identity(
            uri=f'{os.environ["V104_OUTPUT_PREFIX"]}/evidence/{filename}',
...
terminal = Terminal(
    ...
    source_commit=os.environ["V104_SOURCE_COMMIT"],
    critique_result_sha256=os.environ["V104_CRITIQUE_SHA256"],
    instance_id=os.environ["INSTANCE_ID"],
    exit_code=int(os.environ["EXIT_CODE"]),
    spot_price_usd_per_hour_micros=int(os.environ["V104_SPOT_PRICE_MICROS"]),
    evidence=evidence,
)
...
phase=source
[ -f source.tar.gz ] || aws s3 cp "$V104_SOURCE_ARCHIVE_URI" source.tar.gz --only-show-errors || exit 92
[ "$(stat -c%s source.tar.gz)" = "$V104_SOURCE_ARCHIVE_BYTES" ] || exit 93
...
for role in source queries truth generation base delta; do
  upper=${role^^}
  uri_name=V104_${upper}_URI
  sha_name=V104_${upper}_SHA256
  bytes_name=V104_${upper}_BYTES
  case "$role" in
...
maximum = float(os.environ["V104_PREFLIGHT_MAX_SECONDS"])
passed = (
...
phase=producer
/usr/bin/time -v -o producer.time timeout "$V104_WALL_SECONDS" .venv/bin/python - <<'PY' >producer.log 2>&1 || exit 97
...
from scripts.v104_exact_retention_ladder import (
    ...
    canonical_v104_result_bytes, evaluate_v104,
)
...
identities = {
    role: ObjectIdentity(
        uri=os.environ[f"V104_{role.upper()}_URI"],
        sha256=os.environ[f"V104_{role.upper()}_SHA256"],
        bytes=int(os.environ[f"V104_{role.upper()}_BYTES"]),
    )
    for role in roles
}
registered_inputs = load_screen_inputs(
    **{role: Path(names[role]) for role in roles}, identities=identities,
    dimensions=768, neighbors=100, query_count=int(os.environ["V104_REGISTERED_QUERY_COUNT"]), seed=7216,
)
query_count = int(os.environ["V104_QUERY_COUNT"])
...
if os.environ["V104_RETENTION_PAGES"] != ",".join(
    str(pages) for pages in REGISTERED_RETENTION_PAGES
):
    raise ValueError("V104 retention ladder differs")
config = V104Config(
    pages_per_root=int(os.environ["V104_PAGES_PER_ROOT"]),
    maximum_root_groups=int(os.environ["V104_MAXIMUM_ROOT_GROUPS"]),
    maximum_exposed_pages=int(os.environ["V104_MAXIMUM_EXPOSED_PAGES"]),
    arms=tuple(
        RetentionArmConfig(pages, pages * 256)
        for pages in REGISTERED_RETENTION_PAGES
    ),
    shortlist_rows=int(os.environ["V104_SHORTLIST_ROWS"]),
    maximum_gets=int(os.environ["V104_MAXIMUM_GETS"]),
    maximum_bytes=int(os.environ["V104_MAXIMUM_BYTES"]),
)
Path("result.json").write_bytes(canonical_v104_result_bytes(evaluate_v104(inputs, authority, config)))
PY
...
from scripts.v104_exact_retention_ladder_rescore import canonical_v104_rescore_bytes, rescore_v104_result
...
authority = ScreenAuthority(
    source_commit=os.environ["V104_SOURCE_COMMIT"], critique_result_sha256=os.environ["V104_CRITIQUE_SHA256"],
...
registered_inputs = load_screen_inputs(
    ...
    query_count=int(os.environ["V104_REGISTERED_QUERY_COUNT"]), seed=7216,
)
query_count = int(os.environ["V104_QUERY_COUNT"])
inputs = dataclasses.replace(
    registered_inputs,
...
if os.environ["V104_RETENTION_PAGES"] != ",".join(
    ...
    raise ValueError("V104 retention ladder differs")
config = V104Config(
    pages_per_root=int(os.environ["V104_PAGES_PER_ROOT"]), maximum_root_groups=int(os.environ["V104_MAXIMUM_ROOT_GROUPS"]),
    maximum_exposed_pages=int(os.environ["V104_MAXIMUM_EXPOSED_PAGES"]),
    arms=tuple(RetentionArmConfig(pages, pages * 256) for pages in REGISTERED_RETENTION_PAGES),
    shortlist_rows=int(os.environ["V104_SHORTLIST_ROWS"]),
    maximum_gets=int(os.environ["V104_MAXIMUM_GETS"]), maximum_bytes=int(os.environ["V104_MAXIMUM_BYTES"]),
)
...
Path("rescore.json").write_bytes(canonical_v104_rescore_bytes(summary))
PY
