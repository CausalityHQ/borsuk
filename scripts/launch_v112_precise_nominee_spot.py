#!/usr/bin/env python3
"""Launch and authenticate one immutable ReLAION-1M precise nominee ceiling on Spot."""
...
def build_plan(source_commit: str, archive_sha256: str, archive_bytes: int,
    ...
    archive = ObjectIdentity(
        f"s3://{BUCKET}/research/v112-precise-nominee/{source_commit}/source/source.tar.gz",
    ...
    return BoundedReaderSpotPlan(
        ...
        output_prefix=(f"s3://{BUCKET}/research/v112-precise-nominee/{source_commit}"
...
def worker_script(plan: BoundedReaderSpotPlan) -> str:
    values = {
        "V112_SOURCE_COMMIT": plan.source_commit,
        "V112_ATTEMPT": plan.attempt,
        "V112_OUTPUT_PREFIX": plan.output_prefix,
        "V112_ARCHIVE_URI": plan.source_archive.uri,
        "V112_ARCHIVE_SHA256": plan.source_archive.sha256,
        "V112_ARCHIVE_BYTES": plan.source_archive.bytes,
        "V112_WALL_SECONDS": 1800,
    }
    for role in INPUTS:
        identity = INPUTS[role]
        values[f"V112_{role}_URI"] = identity.uri
        values[f"V112_{role}_SHA256"] = identity.sha256
        values[f"V112_{role}_BYTES"] = identity.bytes
    exports = "\n".join(
...
aws s3 cp "$V112_ARCHIVE_URI" source.tar.gz --only-show-errors
[ "$(stat -c%s source.tar.gz)" = "$V112_ARCHIVE_BYTES" ]
printf '%s  source.tar.gz\\n' "$V112_ARCHIVE_SHA256" | sha256sum -c -
mkdir repo
tar -xzf source.tar.gz -C repo
exec bash repo/scripts/run_v112_precise_nominee_remote.sh
"""
    script = "#!/bin/bash\n" + exports + "\n" + body
...
def build_launch_specs(plan: BoundedReaderSpotPlan) -> list[dict[str, object]]:
    ...
    for spec in specs:
        ...
        digest = hashlib.sha256(
            f"v112:{plan.source_commit}:{subnet}:a{plan.attempt:04d}".encode()
        ).hexdigest()[:40]
        spec["ClientToken"] = "v112-" + digest
        spec["UserData"] = encoded
        tags = spec["TagSpecifications"][0]["Tags"]
        for tag in tags:
            if tag["Key"] == "Name":
                tag["Value"] = "borsuk-v112-precise-nominee"
            elif tag["Key"] == "BorsukAttempt":
                tag["Value"] = f"a{plan.attempt:04d}"
    return specs


def _claim(plan: BoundedReaderSpotPlan, instance_id: str | None) -> bytes:
    value = {"schema": "borsuk-v112-precise-nominee-claim-v1", "source_commit": plan.source_commit,
        ...
        while True:
            try:
                ...
                if time.monotonic() >= deadline:
                    raise TimeoutError("V112 Spot terminal deadline exceeded") from error
                time.sleep(15)
                continue
            terminal = json.loads(body)
            if (terminal.get("schema") != "borsuk-v112-precise-nominee-terminal-v1"
                ...
                raise ValueError("V112 terminal identity differs")
            return terminal
    finally:
...
def readback(s3: object, terminal: dict[str, object]) -> dict[str, object]:
    ...
    if not isinstance(artifacts, dict) or not required.issubset(artifacts):
        raise ValueError("V112 terminal artifact roster differs")
    bodies = {}
    for name, identity in artifacts.items():
        ...
        if len(body) != identity["bytes"] or hashlib.sha256(body).hexdigest() != identity["sha256"]:
            raise ValueError("V112 artifact readback differs")
        bodies[name] = body
    decision = json.loads(bodies["decision.json"])
    prefix = json.loads(bodies["prefix-reduction.json"])
    if (decision.get("schema") != "borsuk-v112-decision-v1"
        ...
        raise ValueError("V112 prefix binding differs")
    control = prefix.get("historical_prefix_200")
    capped = prefix.get("capped")
    precise = prefix.get("precise")
    if not isinstance(control, dict) or not isinstance(capped, dict) or not isinstance(precise, dict):
        raise ValueError("V112 prefix reduction arms differ")
    matched = (control.get("matches_v77_v78_control") is True
               and control.get("hits") == 19832
               and capped.get("hits") == 19739)
    good = (type(precise.get("recall100_ppm")) is int
            and type(precise.get("p05_hits")) is int
            and type(precise.get("cap_violations")) is int
            and precise["cap_violations"] == 0
            and precise["recall100_ppm"] >= 990000
            and precise["p05_hits"] >= 90)
    expected_decision = ("advance-full" if matched and good else
                         "harness-mismatch" if not matched else "stop-precise-ceiling")
    if decision.get("prefix_decision") != expected_decision:
        raise ValueError("V112 prefix decision differs from reduction")
    if decision["prefix_decision"] == "advance-full":
        if not {"full.log", "full-evidence.jsonl", "full-reduction.json",
            ...
            raise ValueError("V112 full artifact roster differs")
        ...
        if (full.get("evidence_sha256") != artifacts["full-evidence.jsonl"]["sha256"]
            ...
            raise ValueError("V112 full binding differs")
    elif decision["prefix_decision"] not in ("harness-mismatch", "stop-precise-ceiling"):
        raise ValueError("V112 decision differs")
    return terminal
