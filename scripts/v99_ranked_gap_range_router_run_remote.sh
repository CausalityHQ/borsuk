...
output_prefix=$V99_OUTPUT_PREFIX
...
finish() {
  ...
  STARTED_EPOCH="$started_epoch" FINISHED_EPOCH="$finished_epoch" \
    ...
    SWAP_END_KIB="$swap_end_kib" SPOT_PRICE_MICROS="$V99_SPOT_PRICE_MICROS" \
    python3.12 - <<'PY'
import json, os
...
if status == "complete":
    for role, filename in (
        ...
        evidence[role] = Identity(
            uri=f'{os.environ["V99_OUTPUT_PREFIX"]}/evidence/{filename}',
...
terminal = Terminal(
    ...
    source_commit=os.environ["V99_SOURCE_COMMIT"],
    critique_result_sha256=os.environ["V99_CRITIQUE_SHA256"],
    instance_id=os.environ["INSTANCE_ID"],
    exit_code=int(os.environ["EXIT_CODE"]),
    spot_price_usd_per_hour_micros=int(os.environ["V99_SPOT_PRICE_MICROS"]),
    evidence=evidence,
)
...
phase=source
[ -f source.tar.gz ] || aws s3 cp "$V99_SOURCE_ARCHIVE_URI" source.tar.gz --only-show-errors || exit 92
[ "$(stat -c%s source.tar.gz)" = "$V99_SOURCE_ARCHIVE_BYTES" ] || exit 93
...
for role in source queries truth generation base delta; do
  upper=${role^^}
  uri_name=V99_${upper}_URI
  sha_name=V99_${upper}_SHA256
  bytes_name=V99_${upper}_BYTES
  case "$role" in
...
from scripts.v99_ranked_gap_range_router import RankedGapConfig, canonical_v99_result_bytes, evaluate_v99
...
identities = {
    role: ObjectIdentity(
        uri=os.environ[f"V99_{role.upper()}_URI"],
        sha256=os.environ[f"V99_{role.upper()}_SHA256"],
        bytes=int(os.environ[f"V99_{role.upper()}_BYTES"]),
    )
    for role in roles
}
inputs = load_screen_inputs(
    **{role: Path(names[role]) for role in roles}, identities=identities,
    dimensions=768, neighbors=100, query_count=int(os.environ["V99_QUERY_COUNT"]), seed=7216,
)
...
config = RankedGapConfig(
    pages_per_root=int(os.environ["V99_PAGES_PER_ROOT"]),
    maximum_root_groups=int(os.environ["V99_MAXIMUM_ROOT_GROUPS"]),
    maximum_exposed_pages=int(os.environ["V99_MAXIMUM_EXPOSED_PAGES"]),
    retained_pages=int(os.environ["V99_RETAINED_PAGES"]),
    maximum_scanned_rows=int(os.environ["V99_MAXIMUM_SCANNED_ROWS"]),
    shortlist_rows=int(os.environ["V99_SHORTLIST_ROWS"]),
    maximum_gets=int(os.environ["V99_MAXIMUM_GETS"]),
    maximum_bytes=int(os.environ["V99_MAXIMUM_BYTES"]),
)
Path("result.json").write_bytes(canonical_v99_result_bytes(evaluate_v99(inputs, authority, config)))
PY
...
from scripts.v99_ranked_gap_range_router_rescore import canonical_v99_rescore_bytes, rescore_v99_result
...
authority = ScreenAuthority(
    source_commit=os.environ["V99_SOURCE_COMMIT"], critique_result_sha256=os.environ["V99_CRITIQUE_SHA256"],
    page_map_sha256=Path("page_map.sha256").read_text().strip(), dimensions=768, seed=7216, identities=identities,
)
config = RankedGapConfig(
    pages_per_root=int(os.environ["V99_PAGES_PER_ROOT"]), maximum_root_groups=int(os.environ["V99_MAXIMUM_ROOT_GROUPS"]),
    maximum_exposed_pages=int(os.environ["V99_MAXIMUM_EXPOSED_PAGES"]), retained_pages=int(os.environ["V99_RETAINED_PAGES"]),
    maximum_scanned_rows=int(os.environ["V99_MAXIMUM_SCANNED_ROWS"]), shortlist_rows=int(os.environ["V99_SHORTLIST_ROWS"]),
    maximum_gets=int(os.environ["V99_MAXIMUM_GETS"]), maximum_bytes=int(os.environ["V99_MAXIMUM_BYTES"]),
)
summary = rescore_v99_result(Path("result.json"), os.environ["RESULT_SHA"], expected_authority=authority, expected_config=config)
Path("rescore.json").write_bytes(canonical_v99_rescore_bytes(summary))
PY
