#!/usr/bin/env python3
"""One preregistered cosine-PQ graph/FP16 100k gate on Spot."""
...
SCHEMA = "borsuk-v215-pq-cosine-graph-100k-spot-v1"
TAG = "borsuk-v215-pq-cosine-graph-100k"
WALL_SECONDS = 7_200
...
if len(INPUTS) != 8:
    raise RuntimeError("V215 frozen input roster differs")
...
def user_data(commit: str, archive_sha: str, archive_key: str, prefix: str) -> str:
    template = r'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v215-hard-stop --on-active=@@WALL@@s /usr/sbin/shutdown -h now
root=/mnt/v215-pq-cosine-graph-100k
...
"$CARGO_HOME/bin/cargo" build --release --locked -p borsuk \
  --bin v215_serve_pq_cosine_graph_100k \
  --jobs 6 >"$root/build.log" 2>&1
cd "$root"
phase=serve
/usr/bin/time -v -o serving-resources.txt "$CARGO_TARGET_DIR/release/v215_serve_pq_cosine_graph_100k" \
...
phase=truth
@@LATE_DOWNLOADS@@
.venv/bin/python -m scripts.v215_score_pq_cosine_graph_100k \
...
'''
    ...
    if "@@" in template:
        raise ValueError("unresolved V215 worker placeholder")
    return template
...
def launch(attempt: str) -> None:
    ...
    archive_key = f"research/v215-pq-cosine-graph-100k/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v215-pq-cosine-graph-100k/{commit}/runs/{attempt}"
    ...
    receipt = ec2.run_instances(
        ClientToken="v215-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
        ...
        while time.monotonic() < deadline:
            try:
                ...
                if (terminal.get("schema") != SCHEMA or terminal.get("instance_id") != instance_id
                    ...
                    raise ValueError("V215 terminal identity differs")
                terminal["terminal_sha256"] = hashlib.sha256(raw).hexdigest()
                print(json.dumps(terminal, sort_keys=True), flush=True)
                if terminal.get("status") != "complete":
                    raise RuntimeError("V215 failed; inspect only closed terminal artifacts")
                if set(terminal.get("artifacts", {})) != set(ARTIFACTS):
                    raise ValueError("V215 artifact roster differs")
                for name, identity in terminal["artifacts"].items():
                    ...
                    if size != identity["bytes"] or value.hexdigest() != identity["sha256"]:
                        raise ValueError(f"V215 artifact readback differs: {name}")
            ...
            if state in {"terminated", "shutting-down"}:
                raise RuntimeError("V215 worker stopped before terminal")
            time.sleep(20)
        raise TimeoutError("V215 Spot cell exceeded wall cap")
    finally:
        try:
            state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
...
if __name__ == "__main__":
    ...
    with open("/tmp/borsuk-v215-pq-cosine-graph-launch.lock", "a+") as lock:
