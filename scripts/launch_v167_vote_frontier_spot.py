#!/usr/bin/env python3
"""Launch one immutable GT-blind V167 vote frontier gate on Causality Spot."""
...
SCHEMA = "borsuk-v167-vote-frontier-spot-v1"
TAG = "borsuk-v167-vote-frontier"
WALL_SECONDS = 14_400
...
ARTIFACTS = (
    "out/rosters.jsonl", "out/proxy-labels.jsonl",
...
def user_data(commit: str, archive_sha: str, archive_key: str, prefix: str) -> str:
    return f'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v167-hard-stop --on-active={WALL_SECONDS}s /usr/sbin/shutdown -h now
root=/mnt/v167-vote-frontier
...
/usr/bin/time -v -o prepare-resources.txt .venv/bin/python -m \
  scripts.v167_vote_frontier_run prepare --output out \
...
/usr/bin/time -v -o plan-resources.txt .venv/bin/python -m \
  scripts.v167_vote_frontier_run plan --output out
phase=evaluate
/usr/bin/time -v -o evaluate-resources.txt .venv/bin/python -m \
  scripts.v167_vote_frontier_run evaluate --output out
phase=check
/usr/bin/time -v -o check-resources.txt .venv/bin/python -m \
  scripts.v167_vote_frontier_run check --output out \
...
def _launch() -> None:
    ...
    if subprocess.run(["git", "merge-base", "--is-ancestor", "HEAD",
        ...
        raise ValueError("V167 source commit is not pushed to origin/main")
    archive = archive_source(commit)
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as tar:
        required = {"scripts/v166_surrogate_probe.py",
                    "scripts/v166_surrogate_ranking_run.py",
                    "scripts/v167_min_cost_vote_frontier.py",
                    "scripts/v167_vote_frontier_run.py",
                    "docs/research/v167-min-cost-vote-frontier-decision.md",
                    "docs/research/v167-vote-frontier-prereg.md"}
        if not required.issubset(tar.getnames()):
            raise ValueError("source archive lacks V167 gate")
    archive_sha = hashlib.sha256(archive).hexdigest()
    archive_key = f"research/v167-vote-frontier/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v167-vote-frontier/{commit}/runs/{attempt}"
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V167 worker already active")
    ...
    receipt = ec2.run_instances(
        ClientToken="v167-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
    ...
    while time.monotonic() < deadline:
        if not missing(s3, prefix + "/terminal.json"):
            ...
            for name, identity in terminal.get("artifacts", {}).items():
                ...
                if size != identity["bytes"] or digest.hexdigest() != identity["sha256"]:
                    raise ValueError(f"V167 S3 artifact read-back differs: {name}")
            terminal["terminal_sha256"] = hashlib.sha256(raw).hexdigest()
            print(json.dumps(terminal, sort_keys=True), flush=True)
            if terminal.get("status") != "complete":
                raise RuntimeError("V167 terminal reports failed; see closed artifacts")
            return
        ...
        if state in {"terminated", "shutting-down"}:
            ...
            if time.monotonic() - started >= WALL_SECONDS - 60:
                raise TimeoutError("V167 worker reached wall cap before terminal")
            raise RuntimeError("Spot worker stopped before terminal; restart full cell")
        time.sleep(20)
    ec2.terminate_instances(InstanceIds=[instance_id])
    raise TimeoutError("V167 cell exceeded wall cap")


def main() -> None:
    # All V167 controllers on this devbox share one process-lifetime lock.
    # The EC2 tag check below also rejects an already running worker.
    with open("/tmp/borsuk-v167-vote-frontier-launch.lock", "a+") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as exc:
            raise RuntimeError("V167 launcher is already running here") from exc
        _launch()
