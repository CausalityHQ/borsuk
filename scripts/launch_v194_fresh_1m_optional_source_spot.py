#!/usr/bin/env python3
"""Launch one immutable source-only V194 priced interval gate on Spot."""
...
SCHEMA = "borsuk-v194-fresh-1m-optional-source-spot-v1"
TAG = "borsuk-v194-fresh-1m-optional-source"
WALL_SECONDS = 7_200
...
ARTIFACTS = (
    ...
    "out/raw.jsonl", "out/summary.json",
...
def user_data(commit: str, archive_sha: str, archive_key: str, prefix: str) -> str:
    return f'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v194-hard-stop --on-active={WALL_SECONDS}s /usr/sbin/shutdown -h now
root=/mnt/v194-fresh-1m-optional-source
...
/usr/bin/time -v -o prepare-resources.txt .venv/bin/python -m \
  scripts.v194_fresh_1m_optional_source prepare --output out \
  ...
  --v189-features v189-features.jsonl --v189-fit-labels v189-fit-labels.jsonl
# All 512 GT-blind features are durable before source truth.
for name in features.jsonl prepare-seal.json; do
  aws s3api put-object --bucket '{BUCKET}' --key '{prefix}/sealed/'"$name" \
    --body "out/$name" --if-none-match '*' --no-cli-pager >/dev/null
done
seal_sha=$(sha256sum out/prepare-seal.json | cut -d ' ' -f1)
phase=plan
/usr/bin/time -v -o plan-resources.txt .venv/bin/python -m \
  scripts.v194_fresh_1m_optional_source plan --output out \
  --source source.parquet --old-layout old-layout.npy \
  --old-sq8 old-sq8.bin --router router --order order.npy \
  --v164-terminal v164-terminal.json --v192-result v192-result.json \
  --v189-features v189-features.jsonl --v189-fit-labels v189-fit-labels.jsonl \
  --prepare-sha256 "$seal_sha"
# All physical plans are durable before source truth.
for name in plans.jsonl planner-runtime.json plan-seal.json; do
  aws s3api put-object --bucket '{BUCKET}' --key '{prefix}/sealed/'"$name" \
    --body "out/$name" --if-none-match '*' --no-cli-pager >/dev/null
done
# Re-read the immutable S3 copies before opening source GT100.
for name in features.jsonl prepare-seal.json plans.jsonl planner-runtime.json plan-seal.json; do
  local_sha=$(sha256sum "out/$name" | cut -d ' ' -f1)
  remote_sha=$(aws s3 cp "s3://{BUCKET}/{prefix}/sealed/$name" - --only-show-errors | sha256sum | cut -d ' ' -f1)
  [ "$local_sha" = "$remote_sha" ]
done
plan_sha=$(sha256sum out/plan-seal.json | cut -d ' ' -f1)
phase=evaluate
/usr/bin/time -v -o evaluate-resources.txt .venv/bin/python -m \
  scripts.v194_fresh_1m_optional_source evaluate --output out \
  --source source.parquet --old-layout old-layout.npy \
  --old-sq8 old-sq8.bin --router router --order order.npy \
  --v164-terminal v164-terminal.json --v192-result v192-result.json \
  --v189-features v189-features.jsonl --v189-fit-labels v189-fit-labels.jsonl \
...
def _launch() -> None:
    ...
    if subprocess.run(["git", "merge-base", "--is-ancestor", "HEAD",
        ...
        raise ValueError("V194 source commit is not pushed to origin/main")
    ...
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as tar:
        required = {"scripts/v166_surrogate_probe.py",
                    ...
                    "scripts/v194_fresh_1m_optional_source.py",
                    "scripts/hard_priced_interval.py",
                    "scripts/optional_rank_utility.py",
                    "docs/research/v194-fresh-1m-optional-source-prereg.md"}
        if not required.issubset(tar.getnames()):
            raise ValueError("source archive lacks V194 gate")
    archive_sha = hashlib.sha256(archive).hexdigest()
    archive_key = f"research/v194-fresh-1m-optional-source/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v194-fresh-1m-optional-source/{commit}/runs/{attempt}"
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V194 worker already active")
    ...
    receipt = ec2.run_instances(
        ClientToken="v194-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
        ...
        while time.monotonic() < deadline:
            if not missing(s3, prefix + "/terminal.json"):
                ...
                for name, identity in terminal.get("artifacts", {}).items():
                    ...
                    if size != identity["bytes"] or digest.hexdigest() != identity["sha256"]:
                        raise ValueError(f"V194 S3 artifact read-back differs: {name}")
                require_complete_terminal(terminal)
                labels_time = s3.head_object(
                    Bucket=BUCKET, Key=f"{prefix}/artifacts/out/raw.jsonl"
                )["LastModified"]
                for name in ("features.jsonl", "prepare-seal.json",
                    ...
                    if (size != expected["bytes"]
                        ...
                        raise ValueError(f"V194 pre-truth S3 seal differs: {name}")
            ...
            if state in {"terminated", "shutting-down"}:
                ...
                if time.monotonic() - started >= WALL_SECONDS - 60:
                    raise TimeoutError("V194 worker reached wall cap before terminal")
                raise RuntimeError("Spot worker stopped before terminal; restart full cell")
            time.sleep(20)
        ec2.terminate_instances(InstanceIds=[instance_id])
        raise TimeoutError("V194 cell exceeded wall cap")
...
def main() -> None:
    # All V194 controllers on this devbox share one process-lifetime lock.
    # The EC2 tag check below also rejects an already running worker.
    with open("/tmp/borsuk-v194-fresh-1m-optional-source-launch.lock", "a+") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as exc:
            raise RuntimeError("V194 launcher is already running here") from exc
        _launch()
