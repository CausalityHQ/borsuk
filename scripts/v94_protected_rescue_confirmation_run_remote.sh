...
if [ "${1:-}" = "--describe" ]; then
  printf '%s\n' '{"blas_threads":16,"confirmation_end_exclusive":456,"confirmation_start":328,"instance_type":"c7i.8xlarge","max_wall_seconds":1800,"queries":128,"query_parallelism":1,"rayon_work_stealing":false,"spot_only":true,"total_arms":2,"worker_model":"fixed-16-thread-blas"}'
  exit 0
fi
...
root=/mnt/v94-protected-rescue-confirmation
phase=bootstrap
...
publish_terminal() {
  ...
  if [ "$code" -eq 0 ]; then
    for name in dependency.log hashes.log hashes-check.log instance.json run.log time.log result.json; do
      if [ ! -s "$name" ] || ! aws s3 cp "$name" "$V94_OUTPUT_URI/$name" --only-show-errors; then
        code=98
    ...
    for name in dependency.log hashes.log hashes-check.log instance.json run.log time.log result.json; do
      if [ -n "${V94_OUTPUT_URI:-}" ] && [ -f "$name" ]; then
        aws s3 cp "$name" "$V94_OUTPUT_URI/$name" --only-show-errors || true
      fi
    done
  fi
  printf '{"exit_code":%d,"phase":"%s","source_commit":"%s"}\n' \
    "$code" "$phase" "${V94_SOURCE_COMMIT:-unknown}" >/tmp/v94-terminal.json
  if [ -n "${V94_OUTPUT_URI:-}" ] && ! aws s3 cp /tmp/v94-terminal.json \
    "$V94_OUTPUT_URI/terminal.json" --only-show-errors; then
    code=99
  fi
  unlink /tmp/v94-terminal.json
  sync
  shutdown -h now || true
  exit "$code"
}
...
shutdown -h +40 >/dev/null 2>&1 || exit 89

for variable in \
  V94_SCREEN_URI V94_SCREEN_SHA256 V94_V93_URI V94_V93_SHA256 \
  V94_V92_URI V94_V92_SHA256 V94_V90_URI V94_V90_SHA256 \
  V94_V88_URI V94_V88_SHA256 V94_V87_URI V94_V87_SHA256 \
  V94_V86_URI V94_V86_SHA256 V94_SHARED_URI V94_SHARED_SHA256 \
  V94_OUTPUT_URI V94_SOURCE_COMMIT; do
  eval "value=\${$variable:-}"
  [ -n "$value" ] || exit 86
done
...
phase=inputs
aws s3 cp "$V94_SCREEN_URI" scripts/v94_protected_rescue_confirmation.py --only-show-errors || exit 92
aws s3 cp "$V94_V93_URI" scripts/v93_protected_rescue_screen.py --only-show-errors || exit 92
aws s3 cp "$V94_V92_URI" scripts/v92_wave2_nominated_rows_screen.py --only-show-errors || exit 92
aws s3 cp "$V94_V90_URI" scripts/v90_residual_row_sketch_screen.py --only-show-errors || exit 92
aws s3 cp "$V94_V88_URI" scripts/v88_planner_objective_screen.py --only-show-errors || exit 92
aws s3 cp "$V94_V87_URI" scripts/v87_summary_capacity_screen.py --only-show-errors || exit 92
aws s3 cp "$V94_V86_URI" scripts/v86_coarse_to_fine_screen.py --only-show-errors || exit 92
aws s3 cp "$V94_SHARED_URI" scripts/v85_shared_overlay_screen.py --only-show-errors || exit 92
printf '%s  scripts/v94_protected_rescue_confirmation.py\n%s  scripts/v93_protected_rescue_screen.py\n%s  scripts/v92_wave2_nominated_rows_screen.py\n%s  scripts/v90_residual_row_sketch_screen.py\n%s  scripts/v88_planner_objective_screen.py\n%s  scripts/v87_summary_capacity_screen.py\n%s  scripts/v86_coarse_to_fine_screen.py\n%s  scripts/v85_shared_overlay_screen.py\n' \
  "$V94_SCREEN_SHA256" "$V94_V93_SHA256" "$V94_V92_SHA256" \
  "$V94_V90_SHA256" "$V94_V88_SHA256" "$V94_V87_SHA256" \
  "$V94_V86_SHA256" "$V94_SHARED_SHA256" >hashes.log
...
/usr/bin/time -v -o time.log timeout --signal=TERM --kill-after=30s 1800s \
  .venv/bin/python -m scripts.v94_protected_rescue_confirmation \
