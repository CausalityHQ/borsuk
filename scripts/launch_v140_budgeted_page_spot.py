#!/usr/bin/env python3
"""Launch and monitor one frozen V140 budgeted-page feasibility Spot attempt."""
...
REGION = "eu-central-1"
TAG = "borsuk-v140-budgeted-page"
RUNNER = "scripts/run_v140_budgeted_page_remote.sh"
INPUTS = (
...
def validate(plan: Plan) -> None:
    ...
    if (
        ...
        or not output_key.startswith(
            f"research/v140-budgeted-page/{plan.source_commit}/runs/"
        )
        ...
        raise ValueError("immutable V140 plan differs")
...
def user_data(plan: Plan) -> str:
    ...
    exports = "\n".join(
        (
            f"export V140_SOURCE_COMMIT={plan.source_commit}",
            f"export V140_ARCHIVE_SHA256={plan.archive_sha256}",
            f"export V140_OUTPUT_PREFIX={shlex.quote(plan.output_prefix)}",
            f"export V140_WALL_SECONDS={plan.wall_seconds}",
        )
    )
    return f"""#!/bin/bash
set -euo pipefail
systemd-run --unit=v140-hard-stop --on-active={plan.wall_seconds}s /usr/sbin/shutdown -h now
root=/mnt/v140-budgeted-page
...
import json,os
print(json.dumps({{"schema":"borsuk-v140-budgeted-page-feasibility-spot-v1",
  "source_commit":os.environ["V140_SOURCE_COMMIT"],
  "source_archive_sha256":os.environ["V140_ARCHIVE_SHA256"],
  "instance_id":os.environ["INSTANCE_ID"],"exit_code":int(os.environ["EXIT_CODE"]),
  "phase":"bootstrap","status":"failed","artifacts":{{}}}},
  sort_keys=True,separators=(',',':')))
PY
    aws s3 cp terminal.json "$V140_OUTPUT_PREFIX/terminal.json" --only-show-errors || true
    shutdown -h now || true
  fi
}}
...
def launch_spec(plan: Plan) -> dict:
    token = "v140-" + hashlib.sha256(plan.output_prefix.encode()).hexdigest()[:48]
    return {
        "ClientToken": token,
...
def check_source_and_inputs(s3: object, plan: Plan) -> None:
    ...
    with tarfile.open(fileobj=archive, mode="r:gz") as source:
        if (RUNNER not in source.getnames()
            ...
            raise ValueError("source archive lacks V140 runner or evaluator")
...
def reserve(plan: Plan) -> None:
    receipt = json.dumps({
        "schema": "borsuk-v140-budgeted-page-reservation-v1",
...
def launch_and_monitor(plan: Plan) -> dict:
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V140 worker already active")
    ...
    try:
        while time.monotonic() < deadline:
            if not missing(s3, prefix + "/terminal.json"):
                ...
                if (terminal.get("schema") != "borsuk-v140-budgeted-page-feasibility-spot-v1"
                ...
                required = {"install.log", "download.log", "deep-resources.txt",
                            ...
                            "relaion-resources.txt", "relaion.raw.jsonl",
                            "relaion.summary.json", "cgroup-memory.txt", "worker.log"}
                if terminal.get("status") == "complete":
                    decision = json.loads(s3.get_object(
                        Bucket=BUCKET, Key=prefix + "/artifacts/decision.json")["Body"].read())
                    if decision != {"schema": "borsuk-v140-decision-v1",
                                    "deep_completed": True, "relaion_completed": True}:
                        raise ValueError("V140 decision differs")
                    artifacts = terminal.get("artifacts", {})
                    if not required.issubset(artifacts):
                        raise ValueError("V140 complete terminal lacks required artifacts")
                    for name, metadata in artifacts.items():
                        ...
                        if length != metadata["bytes"] or digest.hexdigest() != metadata["sha256"]:
                            raise ValueError(f"V140 artifact differs: {name}")
                    terminal["authenticated_artifact_count"] = len(artifacts)
                print(json.dumps(terminal, sort_keys=True), flush=True)
                return terminal
            state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
            if state == "terminated":
                raise RuntimeError("V140 instance terminated without terminal marker")
            time.sleep(15)
        raise TimeoutError(f"V140 terminal missing after worker deadline: {instance_id}")
...
def main() -> None:
    ...
    if terminal.get("status") != "complete" or terminal.get("exit_code") != 0:
        raise RuntimeError("V140 terminal reports a failed attempt")
