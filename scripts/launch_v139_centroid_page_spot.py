#!/usr/bin/env python3
"""Launch and monitor one frozen V139 centroid-page feasibility Spot attempt."""
...
REGION = "eu-central-1"
TAG = "borsuk-v139-centroid-page"
RUNNER = "scripts/run_v139_centroid_page_remote.sh"
INPUTS = (
...
def validate(plan: Plan) -> None:
    ...
    if (
        ...
        or not output_key.startswith(
            f"research/v139-centroid-page/{plan.source_commit}/runs/"
        )
        ...
        raise ValueError("immutable V139 plan differs")
...
def user_data(plan: Plan) -> str:
    ...
    exports = "\n".join(
        (
            f"export V139_SOURCE_COMMIT={plan.source_commit}",
            f"export V139_ARCHIVE_SHA256={plan.archive_sha256}",
            f"export V139_OUTPUT_PREFIX={shlex.quote(plan.output_prefix)}",
            f"export V139_WALL_SECONDS={plan.wall_seconds}",
        )
    )
    return f"""#!/bin/bash
set -euo pipefail
...
import json,os
print(json.dumps({{"schema":"borsuk-v139-centroid-page-feasibility-spot-v1",
  "source_commit":os.environ["V139_SOURCE_COMMIT"],
  "source_archive_sha256":os.environ["V139_ARCHIVE_SHA256"],
  "instance_id":os.environ["INSTANCE_ID"],"exit_code":int(os.environ["EXIT_CODE"]),
  "phase":"bootstrap","status":"failed","artifacts":{{}}}},
  sort_keys=True,separators=(',',':')))
PY
    aws s3 cp terminal.json "$V139_OUTPUT_PREFIX/terminal.json" --only-show-errors || true
    shutdown -h now || true
  fi
}}
...
def launch_spec(plan: Plan) -> dict:
    token = "v139-" + hashlib.sha256(plan.output_prefix.encode()).hexdigest()[:48]
    return {
        "ClientToken": token,
...
def check_source_and_inputs(s3: object, plan: Plan) -> None:
    ...
    with tarfile.open(fileobj=archive, mode="r:gz") as source:
        if (RUNNER not in source.getnames()
            ...
            raise ValueError("source archive lacks V139 runner or evaluator")
...
def reserve(plan: Plan) -> None:
    receipt = json.dumps({
        "schema": "borsuk-v139-centroid-page-reservation-v1",
...
def launch_and_monitor(plan: Plan) -> dict:
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V139 worker already active")
...
def main() -> None:
    ...
    if terminal.get("status") != "complete" or terminal.get("exit_code") != 0:
        raise RuntimeError("V139 terminal reports a failed attempt")
