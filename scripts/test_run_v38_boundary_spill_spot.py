    ...
    def test_v38_ceiling_result_recomputes_certificates_aggregates_and_disposition(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            certificates = [
                {
                    ...
                    "feasible_hits": 100,
                    "query_ordinal": query,
                    "selected_postings": list(range(21)),
                    "solver_visits": 0,
                }
            ...
            mutations = [
                ...
                lambda value: value["certificates"][0].update(selected_postings=list(range(20))),
...
class V38SpotMonitorTests(unittest.TestCase):
    ...
    def test_v38_worker_script_has_one_sandbox_and_explicit_cleanup(self):
        ...
        outer_guard = script.index('if test "${V38_IN_SLICE:-0}" != 1; then')
        install = script.index("dnf install -y python3.12")
        self.assertLess(outer_guard, install)
        self.assertLess(install, script.index('systemctl start "$slice_unit"'))
