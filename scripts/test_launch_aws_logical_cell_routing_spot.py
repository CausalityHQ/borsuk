...
class LaunchAwsLogicalCellRoutingSpotTest(unittest.TestCase):
    def test_dry_run_describes_one_ephemeral_spot_worker_without_aws_mutation(
        ...
        self.assertEqual(plan.get("rust_toolchain"), "1.91.0")
        self.assertIs(plan.get("provisions_build_dependencies"), True)
        self.assertIs(plan.get("preserves_campaign_log_on_failure"), True)
