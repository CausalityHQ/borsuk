#!/usr/bin/env python3
"""Launch one immutable row-score router decision on Causality Spot."""
...
def build_plan(**values: object) -> SpotLayoutPlan:
    ...
    if type(attempt) is not int or not 1 <= attempt <= 99:
        raise ValueError("row-score attempt differs")
    actual_prefix = values.get("output_prefix")
    if type(actual_prefix) is not str:
        raise ValueError("row-score output prefix differs")
    ...
    if plan.output_prefix.rstrip("/") != expected:
        raise ValueError("row-score immutable output prefix differs")
    return plan
...
PY
  ...
  rm -f source.parquet membership.parquet queries.parquet truth.parquet tree.parquet pages.parquet
...
phase=construct
run_capped timeout @WALL@ unshare --net --fork env -i PATH="$PATH" PYTHONPATH="$root/repo" OPENBLAS_NUM_THREADS=32 OMP_NUM_THREADS=32 /usr/bin/time -v -o construct-resources.txt "$root/.venv/bin/python" -m scripts.native_row_score_cell construct --root "$root" --output-prefix "$output"
phase=seal
chmod 0444 source.parquet membership.parquet books.bin codes.bin seal.json sealed.json
aws s3 cp books.bin "$output/artifacts/books.bin" --only-show-errors
aws s3 cp codes.bin "$output/artifacts/codes.bin" --only-show-errors
...
run_capped /usr/bin/time -v -o evaluate-resources.txt timeout @WALL@ setpriv --reuid=nobody --regid=nobody --clear-groups env PYTHONPATH="$root/repo" OPENBLAS_NUM_THREADS=32 OMP_NUM_THREADS=32 "$root/.venv/bin/python" -m scripts.native_row_score_cell evaluate --root "$root" --out "$root/evaluation" --output-prefix "$output"
...
phase=validate
run_capped /usr/bin/time -v -o validate-resources.txt timeout @WALL@ env PYTHONPATH="$root/repo" OPENBLAS_NUM_THREADS=32 OMP_NUM_THREADS=32 "$root/.venv/bin/python" -m scripts.native_row_score_cell validate --root "$root" --out "$root" --output-prefix "$output" --source-commit @COMMIT@
...
"""
    ...
    if len(script.encode()) > 16_384:
        raise ValueError("row-score Spot user data exceeds EC2 limit")
    return script
...
def build_launch_specs(plan: SpotLayoutPlan) -> list[dict[str, object]]:
    ...
    for spec in specs:
        ...
        token = hashlib.sha256(
            f"row-score:{plan.source_commit}:{zone}:a{plan.attempt:04d}".encode()
        ).hexdigest()[:32]
        spec["ClientToken"] = "native-row-score-" + token
        spec["UserData"] = user_data
        spec["TagSpecifications"][0]["Tags"][0]["Value"] = (
            "borsuk-native-row-score"
        )
...
def _validate_terminal_bytes(
    ...
    try:
        ...
        raise ValueError("row-score terminal JSON differs") from error
    canonical = (
    ...
    if (
        ...
        raise ValueError("row-score terminal canonical bytes differ")
    if (
        terminal["schema"] != "borsuk-row-score-terminal-v1"
        ...
        raise ValueError("row-score terminal authority differs")
    complete = (
    ...
    if not complete:
        if (
            ...
            raise ValueError("row-score failed terminal differs")
        return terminal
    names = {
        "books": "books.bin",
        "codes": "codes.bin",
    ...
    if type(artifacts) is not dict or set(artifacts) != set(names):
        raise ValueError("row-score terminal artifact roster differs")
    for role, name in names.items():
        ...
        if (
            ...
            raise ValueError("row-score terminal artifact identity differs")
    return terminal
...
def launch_and_monitor(plan: SpotLayoutPlan) -> dict[str, object]:
    ...
    for name in ("reservation.json", "terminal.json"):
        try:
            ...
            if code in {"404", "NoSuchKey", "NotFound"}:
                continue
            raise
        raise ValueError("row-score immutable attempt already exists")
    reservation = {
        "schema": "borsuk-row-score-reservation-v1",
    ...
    if instance_id is None:
        raise RuntimeError(
            "row-score Spot capacity unavailable; no instance launched"
        )
        ...
        while True:
            try:
                ...
                if state in {"terminated", "shutting-down", "stopped"}:
                    raise RuntimeError(
                        f"row-score instance {instance_id} ended without terminal"
                    ) from error
                if time.monotonic() >= deadline:
                    raise TimeoutError(
                        "row-score terminal deadline exceeded"
                    ) from error
                time.sleep(15)
                continue
