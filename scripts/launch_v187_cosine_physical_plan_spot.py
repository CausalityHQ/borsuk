#!/usr/bin/env python3
"""Launch one immutable source-only V187 cosine physical-plan gate on Spot."""
...
SCHEMA = "borsuk-v187-cosine-physical-plan-spot-v1"
TAG = "borsuk-v187-cosine-physical-plan"
WALL_SECONDS = 3_600
...
def user_data(commit: str, archive_sha: str, archive_key: str, prefix: str) -> str:
    return f'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v187-hard-stop --on-active={WALL_SECONDS}s /usr/sbin/shutdown -h now
root=/mnt/v187-cosine-physical-plan
...
/usr/bin/time -v -o prepare-resources.txt .venv/bin/python -m \
  scripts.v187_cosine_physical_plan prepare --output out \
...
# The complete GT-blind rank list is durable before planning or source truth.
...
/usr/bin/time -v -o plan-resources.txt .venv/bin/python -m \
  scripts.v187_cosine_physical_plan plan --output out \
  --source source.parquet --old-layout old-layout.npy \
  --old-sq8 old-sq8.bin --router router --order order.npy \
  --v164-terminal v164-terminal.json --prepare-sha256 "$seal_sha"
# The complete GT-blind physical plans are durable before exact source truth.
...
/usr/bin/time -v -o evaluate-resources.txt .venv/bin/python -m \
  scripts.v187_cosine_physical_plan evaluate --output out \
  --source source.parquet --old-layout old-layout.npy \
  --old-sq8 old-sq8.bin --router router --order order.npy \
...
def _launch() -> None:
    ...
    if subprocess.run(["git", "merge-base", "--is-ancestor", "HEAD",
        ...
        raise ValueError("V187 source commit is not pushed to origin/main")
    ...
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as tar:
        required = {"scripts/v166_surrogate_probe.py",
                    ...
                    "scripts/v187_cosine_physical_plan.py",
                    "docs/research/v187-cosine-physical-plan-prereg.md"}
        if not required.issubset(tar.getnames()):
            raise ValueError("source archive lacks V187 gate")
    archive_sha = hashlib.sha256(archive).hexdigest()
    archive_key = f"research/v187-cosine-physical-plan/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v187-cosine-physical-plan/{commit}/runs/{attempt}"
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V187 worker already active")
    ...
    receipt = ec2.run_instances(
        ClientToken="v187-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
        ...
        while time.monotonic() < deadline:
            if not missing(s3, prefix + "/terminal.json"):
                ...
                for name, identity in terminal.get("artifacts", {}).items():
                    ...
                    if size != identity["bytes"] or digest.hexdigest() != identity["sha256"]:
                        raise ValueError(f"V187 S3 artifact read-back differs: {name}")
                ...
                for name in ("features.jsonl", "prepare-seal.json",
                    ...
                    sealed_key = f"{prefix}/sealed/{name}"
                    sealed = s3.get_object(Bucket=BUCKET, Key=sealed_key)
                    digest = hashlib.sha256()
                    size = 0
                    ...
                    if (size != expected["bytes"]
                        ...
                        raise ValueError(f"V187 pre-truth S3 seal differs: {name}")
                terminal["terminal_sha256"] = hashlib.sha256(raw).hexdigest()
                print(json.dumps(terminal, sort_keys=True), flush=True)
                if terminal.get("status") != "complete":
                    raise RuntimeError("V187 terminal reports failed; see closed artifacts")
                return
            ...
            if state in {"terminated", "shutting-down"}:
                ...
                if time.monotonic() - started >= WALL_SECONDS - 60:
                    raise TimeoutError("V187 worker reached wall cap before terminal")
                raise RuntimeError("Spot worker stopped before terminal; restart full cell")
            time.sleep(20)
        ec2.terminate_instances(InstanceIds=[instance_id])
        raise TimeoutError("V187 cell exceeded wall cap")

    finally:
        # The worker also has a one-hour systemd hard stop.
        try:
            state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
...
def main() -> None:
    # All V187 controllers on this devbox share one process-lifetime lock.
    # The EC2 tag check below also rejects an already running worker.
    with open("/tmp/borsuk-v187-cosine-physical-plan-launch.lock", "a+") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as exc:
            raise RuntimeError("V187 launcher is already running here") from exc
        _launch()
