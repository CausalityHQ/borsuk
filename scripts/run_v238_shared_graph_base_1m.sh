...
bucket=$BORSUK_V238_BUCKET
prefix=$BORSUK_V238_PREFIX
phase=bootstrap
started=$(date +%s)
watcher_pid=
...
finish() {
  ...
  for name in reuse.json reuse.time build.log install.log run-closed.log; do
...
names=['reuse.json','reuse.time','build.log','install.log','run-closed.log']
artifacts={}
for name in names:
    path=Path(name)
    if path.is_file():
...
print(json.dumps({'schema':'borsuk-v238-shared-graph-base-1m-terminal-v1',
    'source_commit':os.environ['BORSUK_V238_SOURCE_COMMIT'],
    'source_archive_sha256':os.environ['BORSUK_V238_ARCHIVE_SHA'],
...
"$CARGO_HOME/bin/cargo" build --release --locked -p borsuk \
  --example v238_shared_graph_base_1m --jobs 6 >"$root/build.log" 2>&1
cd "$root"
phase=hydrate_and_share
/usr/bin/time -v -o reuse.time \
  "$CARGO_TARGET_DIR/release/examples/v238_shared_graph_base_1m" \
  "$BORSUK_V238_COLLECTION_URI" cache reuse.json
phase=complete
