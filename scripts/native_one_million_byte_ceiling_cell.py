#!/usr/bin/env python3
"""Phase-separated ReLAION-1M score-ranked byte-only diagnostic."""
...
def run_evaluate(root: Path, out: Path, *, query_count: int = 1000) -> None:
    ...
    evaluate_range_selector(
        ...
        DEVELOPMENT_IDENTITIES, query_count=query_count, byte_only=True,
    )
...
def run_validate(root: Path, out: Path, *, query_count: int = 1000) -> None:
    ...
    validate_range_selector(
        ...
        query_count=query_count, byte_only=True,
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.phase == "construct":
        ...
        if args.out is None:
            raise ValueError("byte-ceiling evaluation output required")
        run_evaluate(args.root, args.out)
    else:
        if args.out is None:
            raise ValueError("byte-ceiling validation output required")
