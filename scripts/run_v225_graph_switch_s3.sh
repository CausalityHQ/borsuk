...
bucket=$BORSUK_V225_BUCKET
prefix=$BORSUK_V225_PREFIX
phase=bootstrap
started=$(date +%s)
watcher_pid=
...
finish() {
  ...
  for name in switch.json switch.time build.log install.log run-closed.log; do
...
names=['switch.json','switch.time','build.log','install.log','run-closed.log']
artifacts={}
for name in names:
    path=Path(name)
    if path.is_file():
...
print(json.dumps({'schema':'borsuk-v225-graph-switch-s3-terminal-v1',
    'source_commit':os.environ['BORSUK_V225_SOURCE_COMMIT'],
    'source_archive_sha256':os.environ['BORSUK_V225_ARCHIVE_SHA'],
...
"$CARGO_HOME/bin/cargo" build --release --locked -p borsuk \
  --example v225_graph_switch_s3 --jobs 6 >"$root/build.log" 2>&1
cd "$root"
phase=switch
/usr/bin/time -v -o switch.time \
  "$CARGO_TARGET_DIR/release/examples/v225_graph_switch_s3" \
  "s3://$bucket/$prefix/published" work >switch.json
phase=complete
