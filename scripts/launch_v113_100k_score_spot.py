#!/usr/bin/env python3
"""Register and monitor one query-blind V113 ReLAION-100k Spot attempt."""
...
from scripts.launch_native_geometric_layout_spot import (
    DEFAULT_TARGETS, FROZEN_QUERIES, FROZEN_SOURCE,
)

SCHEMA = "borsuk-v113-100k-spot-v1"
...
def validate_plan(plan: Plan) -> None:
    if (
        ...
        raise ValueError("V113 immutable Spot plan differs")
...
mkdir -p "$root"
cd "$root"
export V113_SOURCE_COMMIT={_q(plan.source_commit)}
export V113_OUTPUT_PREFIX={_q(plan.output_prefix.rstrip('/'))}
export V113_SOURCE_URI={_q(FROZEN_SOURCE.uri)}
export V113_SOURCE_SHA256={_q(FROZEN_SOURCE.sha256)}
export V113_SOURCE_BYTES={FROZEN_SOURCE.encoded_bytes}
export V113_QUERY_URI={_q(FROZEN_QUERIES.uri)}
export V113_QUERY_SHA256={_q(FROZEN_QUERIES.sha256)}
export V113_QUERY_BYTES={FROZEN_QUERIES.encoded_bytes}
export V113_WALL_SECONDS={plan.wall_seconds}
bootstrap_failure() {{
  code=$?
  trap - EXIT
  if [ "$code" -ne 0 ]; then
...
import json, os
print(json.dumps({{"schema":"borsuk-v113-100k-spot-v1","attempt":1,
...
PY
    aws s3 cp terminal.json "$V113_OUTPUT_PREFIX/terminal.json" --only-show-errors || true
    shutdown -h now || true
  fi
}}
...
trap - EXIT
exec bash repo/scripts/run_v113_100k_score_remote.sh
"""


def launch_spec(plan: Plan, zone: str, subnet: str) -> dict[str, object]:
    token = "v113-" + hashlib.sha256(
    ...
    return {
        ...
        "TagSpecifications": [{"ResourceType": "instance", "Tags": [
            {"Key": "Name", "Value": "borsuk-v113-100k-score"},
...
def _location(uri: str) -> tuple[str, str]:
    ...
    if not bucket or not slash or not key:
        raise ValueError("V113 S3 location differs")
...
def _reserve(s3: object, plan: Plan, bucket: str, prefix: str) -> None:
    if not _missing(s3, bucket, f"{prefix}/reservation.json") or not _missing(
        ...
        raise ValueError("V113 immutable attempt already registered")
    ...
    event_id = "borsuk-v113-reservation-if-none-match"
...
def launch_and_monitor(plan: Plan) -> dict[str, object]:
    ...
    if instance_id is None:
        raise RuntimeError("V113 Spot capacity unavailable; reservation retained")
        ...
        while time.monotonic() < deadline:
            if not _missing(s3, bucket, f"{prefix}/terminal.json"):
                ...
                if (
                    ...
                    raise ValueError("V113 terminal identity differs")
                return terminal
            state = ec2.describe_instances(InstanceIds=[instance_id])[
                "Reservations"
            ][0]["Instances"][0]["State"]["Name"]
            if state in {"stopped", "shutting-down", "terminated"}:
                raise RuntimeError(f"V113 Spot {instance_id} ended without terminal")
            time.sleep(20)
        raise TimeoutError(f"V113 Spot {instance_id} terminal deadline exceeded")
    finally:
