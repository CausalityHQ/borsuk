...
def run(attempt: str) -> None:
    ...
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as source:
        if not {"scripts/run_v223_authenticated_graph_http.sh",
                ...
                "docs/research/v230-mutation-graph-http-1m-prereg.md"}.issubset(source.getnames()):
    ...
    archive_key = f"research/v230-mutation-graph-http-1m/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v230-mutation-graph-http-1m/{commit}/runs/{attempt}"
    ...
    put_if_absent(prefix + "/reservation.json", json.dumps({
        ...
        "gate": "both passes no split GT100 loss vs V219, p05>=98, p95<=41.640ms, p99<=45.128ms, QPS>=200, RSS<=3GiB, no vector GET",
    ...
    try:
        for role in ("server", "client"):
            ...
            request["TagSpecifications"][0]["Tags"][0]["Value"] = "borsuk-v230-mutation-graph-http"
        ...
        closeout = {"schema": SCHEMA + "-closeout", "source_commit": commit,
                    ...
                    "v223_baseline_summaries": baseline,
                    "v230_summaries": summaries(s3, prefix, client)}
