#!/usr/bin/env python3
"""Launch one immutable page-centered group router decision on Causality Spot."""
...
PRIOR_EVIDENCE = FrozenInput(
    role="residual-row-score-evidence",
    uri=(
    ...
    sha256="d91c726afb584826c24a3fd1c0d804cd8d71d95c4967cf16f3cdceb924c53f1f",
    encoded_bytes=1_903_426,
    rows=1_000,
)
...
def build_plan(**values: object) -> SpotLayoutPlan:
    ...
    if type(attempt) is not int or not 1 <= attempt <= 99:
        raise ValueError("page-centered group attempt differs")
    actual_prefix = values.get("output_prefix")
    if type(actual_prefix) is not str:
        raise ValueError("page-centered group output prefix differs")
    ...
    if plan.output_prefix.rstrip("/") != expected:
        raise ValueError("page-centered group immutable output prefix differs")
    return plan
...
def worker_script(plan: SpotLayoutPlan) -> str:
    ...
    script = """#!/bin/bash
set -euo pipefail
root=/mnt/native-page-centered-groups
output=@OUTPUT@
phase=bootstrap
status=failed
started=$(date +%s)
...
phase=construct
run_capped timeout @WALL@ unshare --net --fork env -i PATH="$PATH" PYTHONPATH="$root/repo" OPENBLAS_NUM_THREADS=32 OMP_NUM_THREADS=32 /usr/bin/time -v -o construct-resources.txt "$root/.venv/bin/python" -m scripts.native_page_centered_group_cell construct --root "$root" --output-prefix "$output"
phase=seal
chmod 0444 source.parquet membership.parquet books.bin groups.bin seal.json sealed.json
aws s3 cp books.bin "$output/artifacts/books.bin" --only-show-errors
...
"""
    ...
    if len(script.encode()) > 16_384:
        raise ValueError("page-centered group Spot user data exceeds EC2 limit")
    return script
...
def build_launch_specs(plan: SpotLayoutPlan) -> list[dict[str, object]]:
    ...
    for spec in specs:
        ...
        token = hashlib.sha256(
            f"page-centered group:{plan.source_commit}:{zone}:a{plan.attempt:04d}".encode()
        ).hexdigest()[:32]
        spec["ClientToken"] = "native-page-centered-groups-" + token
        spec["UserData"] = user_data
        spec["TagSpecifications"][0]["Tags"][0]["Value"] = (
            "borsuk-native-page-centered-groups"
        )
...
def _validate_terminal_bytes(
    ...
    try:
        ...
        raise ValueError("page-centered group terminal JSON differs") from error
    canonical = (
    ...
    if (
        ...
        raise ValueError("page-centered group terminal canonical bytes differ")
    if (
        terminal["schema"] != "borsuk-page-centered-group-terminal-v1"
        ...
        raise ValueError("page-centered group terminal authority differs")
    if terminal["source_archive"] != dataclasses.asdict(plan.source_archive):
        raise ValueError("page-centered group source archive differs")
    complete = (
    ...
    if not complete:
        if (
            ...
            raise ValueError("page-centered group failed terminal differs")
        return terminal
    names = {
        "books": "books.bin",
        "groups": "groups.bin",
    ...
    if type(artifacts) is not dict or set(artifacts) != set(names):
        raise ValueError("page-centered group terminal artifact roster differs")
    for role, name in names.items():
        ...
        if (
            ...
            raise ValueError("page-centered group terminal artifact identity differs")
    return terminal
...
def launch_and_monitor(plan: SpotLayoutPlan) -> dict[str, object]:
    ...
    if instance_id is None:
        raise RuntimeError(
            "page-centered group Spot capacity unavailable; no instance launched"
        )
        ...
        while True:
            try:
                ...
                if state in {"terminated", "shutting-down", "stopped"}:
                    raise RuntimeError(
                        f"page-centered group instance {instance_id} ended without terminal"
                    ) from error
                if time.monotonic() >= deadline:
                    raise TimeoutError(
                        "page-centered group terminal deadline exceeded"
                    ) from error
                time.sleep(15)
                continue
