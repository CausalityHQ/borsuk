...
deadline_epoch=$((started_epoch + V144_WALL_SECONDS))
monitor_pid=
imds_token=
imds() {
...
finish() {
  ...
  for path in install.log core-check.log core-check-resources.txt \
    ...
    if [ -f "$path" ] && ! timeout 900 aws s3 cp "$path" \
        "$V144_OUTPUT_PREFIX/artifacts/$path" --only-show-errors; then
      upload_failed=1
    fi
  done
...
PY
  aws s3 cp terminal.json "$V144_OUTPUT_PREFIX/terminal.json" --only-show-errors
  shutdown -h now || true
  exit "$code"
}
trap finish EXIT
...
run_science parity-build "$CARGO_HOME/bin/cargo" build --release --locked -j 8 \
  --manifest-path repo/scripts/v114_score_rust/Cargo.toml --bin v144_page_parity
phase=download-deep
: >download.log
...
phase=evaluate-deep
mkdir /sys/fs/cgroup/v144-evaluate
sync
echo 3 >/proc/sys/vm/drop_caches
run_science deep bash -c \
  'echo "$BASHPID" >/sys/fs/cgroup/v144-evaluate/cgroup.procs; exec "$@"' \
  v144 .venv/bin/python repo/scripts/v140_budgeted_page_rank.py \
...
phase=parity-deep
run_science deep-parity "$CARGO_TARGET_DIR/release/v144_page_parity" \
...
run_science relaion-parity "$CARGO_TARGET_DIR/release/v144_page_parity" \
...
PY
printf '%s\n' '{"schema":"borsuk-v144-decision-v1","deep_completed":true,"relaion_completed":true}' >decision.json
{
  cat /sys/fs/cgroup/v144-evaluate/memory.current
  cat /sys/fs/cgroup/v144-evaluate/memory.peak
  cat /sys/fs/cgroup/v144-evaluate/memory.stat
} >cgroup-memory.txt
phase=complete
exit 0
