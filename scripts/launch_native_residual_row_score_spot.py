#!/usr/bin/env python3
"""Launch one immutable residual row-score router decision on Causality Spot."""
...
def build_plan(**values: object) -> SpotLayoutPlan:
    ...
    if type(attempt) is not int or not 1 <= attempt <= 99:
        raise ValueError("residual row-score attempt differs")
    actual_prefix = values.get("output_prefix")
    if type(actual_prefix) is not str:
        raise ValueError("residual row-score output prefix differs")
    ...
    if plan.output_prefix.rstrip("/") != expected:
        raise ValueError("residual row-score immutable output prefix differs")
    return plan
...
def worker_script(plan: SpotLayoutPlan) -> str:
    ...
    script = """#!/bin/bash
set -euo pipefail
root=/mnt/native-residual-row-score
output=@OUTPUT@
phase=bootstrap
status=failed
started=$(date +%s)
...
phase=construct
run_capped timeout @WALL@ unshare --net --fork env -i PATH="$PATH" PYTHONPATH="$root/repo" OPENBLAS_NUM_THREADS=32 OMP_NUM_THREADS=32 /usr/bin/time -v -o construct-resources.txt "$root/.venv/bin/python" -m scripts.native_residual_row_score_cell construct --root "$root" --output-prefix "$output"
phase=seal
...
"""
    ...
    if len(script.encode()) > 16_384:
        raise ValueError("residual row-score Spot user data exceeds EC2 limit")
    return script
...
def build_launch_specs(plan: SpotLayoutPlan) -> list[dict[str, object]]:
    ...
    for spec in specs:
        ...
        token = hashlib.sha256(
            f"residual row-score:{plan.source_commit}:{zone}:a{plan.attempt:04d}".encode()
        ).hexdigest()[:32]
        spec["ClientToken"] = "native-residual-row-score-" + token
        spec["UserData"] = user_data
        spec["TagSpecifications"][0]["Tags"][0]["Value"] = (
            "borsuk-native-residual-row-score"
        )
...
def _validate_terminal_bytes(
    ...
    try:
        ...
        raise ValueError("residual row-score terminal JSON differs") from error
    canonical = (
    ...
    if (
        ...
        raise ValueError("residual row-score terminal canonical bytes differ")
    if (
        terminal["schema"] != "borsuk-residual-row-score-terminal-v1"
    ...
    if not complete:
        if (
            ...
            raise ValueError("residual row-score failed terminal differs")
        return terminal
    names = {
        "books": "books.bin",
        "codes": "codes.bin",
    ...
    if type(artifacts) is not dict or set(artifacts) != set(names):
        raise ValueError("residual row-score terminal artifact roster differs")
    for role, name in names.items():
        ...
        if (
            ...
            raise ValueError("residual row-score terminal artifact identity differs")
    return terminal
...
def launch_and_monitor(plan: SpotLayoutPlan) -> dict[str, object]:
    ...
    for name in ("reservation.json", "terminal.json"):
        try:
            ...
            if code in {"404", "NoSuchKey", "NotFound"}:
                continue
            raise
        raise ValueError("residual row-score immutable attempt already exists")
    reservation = {
    ...
    if instance_id is None:
        raise RuntimeError(
            "residual row-score Spot capacity unavailable; no instance launched"
        )
        ...
        while True:
            try:
                ...
                if state in {"terminated", "shutting-down", "stopped"}:
                    raise RuntimeError(
                        f"residual row-score instance {instance_id} ended without terminal"
                    ) from error
                if time.monotonic() >= deadline:
                    raise TimeoutError(
                        "residual row-score terminal deadline exceeded"
                    ) from error
                time.sleep(15)
                continue
