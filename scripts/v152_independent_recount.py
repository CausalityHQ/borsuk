"""Independent postterminal V152 plan, work, and returned-quality recount."""
...
V151_QUERIES_SHA = "5374e44595486f1f2cb6b94ceb420fd5fed0c77284a90de976c7ed7a8552cbff"
V151_ROUTING_SHA = "a35584d4391bd1f46ce9bdf80062e5483132a9ad9f9ddb7678cc1aae72dbcea4"
V151_TRUTH_SHA = "038379c4bdd7aca71201f043b620ddaecccf2fbf6283b27db8918a56efc9e803"
...
def rows(data: bytes) -> list[dict]:
    ...
    if len(result) != 1000:
        raise ValueError("V152 record count differs")
    return result
...
def recount(s3: object, prefix: str, decision: dict,
    ...
    if (manifest["schema"] != "borsuk-v152-fresh-inputs-v1"
        ...
        raise ValueError("V152 fresh input seal differs")
    if (manifest["queries_sha256"] != V151_QUERIES_SHA
            or manifest["routing_sha256"] != V151_ROUTING_SHA
            or manifest["truth_sha256"] != V151_TRUTH_SHA):
        raise ValueError("V152 development cohort differs from terminal-closed V151")
    ...
    if plan_summary["schema"] != "borsuk-v152-cached-page-graph-plan-v1":
        raise ValueError("V152 plan summary schema differs")
    ...
    if (replay_seal["schema"] != "borsuk-v152-returned-seal-v1"
        ...
        raise ValueError("V152 returned-ID seal differs")
    if (audit["schema"] != "borsuk-v152-returned-audit-v1"
            ...
            or audit["centroid_sha256"] != CENTROID_SHA
            or audit["plans_sha256"] != digest(authenticated["science.jsonl"])
            or audit["flat_checked_queries"] != 1000
            or not 0 <= audit["max_flat_score_difference"] <= 0.001
            or audit["checked_arm_queries"] != 4000
            or audit["truth_opened"] is not False):
        raise ValueError("V152 independent returned audit differs")
    ...
    if (digest(graph) != GRAPH_SHA or graph[:8] != b"BORSUKG1"
        ...
        raise ValueError("V152 graph artifact differs")
    ...
    if (len(centroid_blob) != 600032 or digest(centroid_blob) != CENTROID_SHA
        ...
        raise ValueError("V152 centroid source differs")
    ...
    arm_values = {arm: {field: [] for field in ("source", "sq8", "physical", "bytes", "gets", "union")}
                  for arm in ("flat", "v150", "v151", "v152")}
    replay_cpu = {arm: {"sq8_ns": [], "source_ns": []}
                  for arm in ("flat", "v150", "v151", "v152")}
    cpu = {arm: [] for arm in ("flat", "v150", "v151", "v152")}
    candidate_phases = {field: [] for field in ("search_ns", "score_ns", "plan_ns")}
    ...
    new_unit_work = []
    prior_work, prior_exact, prior_union, prior_total, prior_shortfall = [], [], [], [], []
    for ordinal, (query, route, plan, replay, proof) in enumerate(
        ...
        if any(record.get("query_ordinal") != ordinal
            ...
            raise ValueError("V152 query identity differs")
        ...
        candidates = {"flat": dict(enumerate(flat_scores)),
                      ...
                      "v152": dict(plan["v152"]["scored_pages"])}
        for arm in ("v150", "v151", "v152"):
            ...
            if (len(scored_pages) != len(candidates[arm])
                ...
                raise ValueError(f"V152 {arm} scored page roster differs")
        for arm in arm_plans:
            ...
            if (observed["selected_pages"] != expected["selected_pages"]
                ...
                raise ValueError(f"V152 {arm} planner replay differs")
        ...
        for arm in ("v150", "v151", "v152"):
            ...
            if (len(evaluated) != len(set(evaluated))
                ...
                raise ValueError(f"V152 {arm} work differs")
            if arm in ("v151", "v152"):
                ...
                if (not expected_seeds.issubset(evaluated)
                    ...
                    raise ValueError("V152 page-diverse seeds or result differ")
                scored = set(candidates[arm])
                if scored != primary_pages | {page for page, _ in record["provisional_pages"]}:
                    raise ValueError("V152 exact page candidates differ")
                observed_minima = {}
                ...
                if ([page for page, _ in expected_provisional]
                    ...
                    raise ValueError("V152 provisional page ranking differs")
            if arm == "v152":
                reported_squared = record["cached_graph_squared"]
                if (len(reported_squared) != len(evaluated)
                        or any(abs(score - graph_squared(unit)) > 1e-7
                               for unit, score in zip(evaluated, reported_squared))):
                    raise ValueError("V152 cached graph distance differs")
            for page, score in record["scored_pages"]:
                units = range(page * 8, min((page + 1) * 8, 3125))
                expected_score = (f32(math.sqrt(min(graph_squared(unit) for unit in units)))
                                  if arm == "v152" else
                                  min(unit_distance(unit) for unit in units))
                if abs(score - expected_score) > (1e-7 if arm == "v152" else 0.001):
                    raise ValueError("V152 independent centroid page score differs")
                max_score_diff = max(max_score_diff, abs(score - flat_scores[page]))
        if (plan["v151"]["evaluated_units"] != plan["v152"]["evaluated_units"]
                or plan["v151"]["provisional_pages"] != plan["v152"]["provisional_pages"]
                or [page for page, _ in plan["v151"]["scored_pages"]]
                != [page for page, _ in plan["v152"]["scored_pages"]]):
            raise ValueError("V152 changed frozen V151 graph traversal or candidate roster")
        cpu["flat"].append(plan["flat_elapsed_ns"])
        if plan["flat_score_ns"] + plan["flat_plan_ns"] != plan["flat_elapsed_ns"]:
            raise ValueError("V152 flat phase timing differs")
        cpu["v150"].append(plan["v150"]["elapsed_ns"])
        cpu["v151"].append(plan["v151"]["elapsed_ns"])
        cpu["v152"].append(plan["v152"]["elapsed_ns"])
        for arm in ("v150", "v151", "v152"):
            record = plan[arm]
            if sum(record[field] for field in ("search_ns", "score_ns", "plan_ns")) != record["elapsed_ns"]:
                raise ValueError(f"V152 {arm} phase timing differs")
        for field in candidate_phases:
            candidate_phases[field].append(plan["v152"][field])
        flat_bytes += arm_plans["flat"]["planned_bytes"]
        new_bytes += arm_plans["v152"]["planned_bytes"]
        flat_selected += len(arm_plans["flat"]["selected_pages"])
        new_capture += len(set(arm_plans["v152"]["selected_pages"])
                           & set(arm_plans["flat"]["selected_pages"]))
        new_work.append(plan["v152"]["unit_evaluations"])
        new_shortfall.append(arm_plans["v152"]["target_shortfall"])
        for arm in ("v150", "v151", "v152"):
            record = plan[arm]
            exact_units = {unit for page in candidates[arm]
                           for unit in range(page * 8, min((page + 1) * 8, 3125))}
            union = exact_units | set(record["evaluated_units"])
            missing = len(exact_units - set(record["evaluated_units"])) if arm == "v152" else len(exact_units)
            if (record["exact_page_unit_evaluations"] != len(exact_units)
                    or record["distinct_scored_units"] != len(union)
                    or record["newly_scored_units"] != missing
                    or record["total_unit_distance_computations"]
                    != missing + len(record["evaluated_units"])):
                raise ValueError(f"V152 {arm} unit work differs")
            if arm == "v152":
                exact_work.append(len(exact_units))
                union_work.append(len(union))
                total_work.append(missing + len(record["evaluated_units"]))
                new_unit_work.append(missing)
            if arm == "v151":
                prior_work.append(len(record["evaluated_units"]))
                prior_exact.append(len(exact_units))
                prior_union.append(len(union))
                prior_total.append(len(exact_units) + len(record["evaluated_units"]))
                prior_shortfall.append(arm_plans[arm]["target_shortfall"])
        truth_ids = gold[ordinal * 100:(ordinal + 1) * 100]
        if len(set(truth_ids)) != 100 or any(not 0 <= source < 100000 for source in truth_ids):
            raise ValueError("V152 GT row differs")
        ...
        for arm in arm_values:
            ...
            if (len(source_ids) != 100 or len(set(source_ids)) != 100
                ...
                raise ValueError("V152 sealed returned ID or route differs")
            ...
            if (sq8_hits > physical_hits
                ...
                raise ValueError("V152 returned quality recount differs")
            for field, value in (("source", source_hits), ("sq8", sq8_hits),
                                 ("physical", physical_hits),
                                 ("bytes", returned["planned_bytes"]),
                                 ("gets", returned["gets"]),
                                 ("union", returned["union_size"])):
                arm_values[arm][field].append(value)
            for field in ("sq8_ns", "source_ns"):
                value = returned[field]
                if not isinstance(value, int) or value < 0:
                    raise ValueError("V152 replay timing differs")
                replay_cpu[arm][field].append(value)
    p95 = {arm: sorted(values)[949] / 1e6 for arm, values in cpu.items()}
    parity_p95 = {f"{arm}_{parity}_p95_ms": sorted(values[parity::2])[474] / 1e6
                  for arm, values in (("flat", cpu["flat"]), ("v152", cpu["v152"]))
                  for parity in (0, 1)}
    parity_p95 = {key.replace("_0_", "_even_").replace("_1_", "_odd_"): value
                  for key, value in parity_p95.items()}
    if (plan_summary["all_primary_retained"] is not all_primary
            ...
            or plan_summary["v152_planned_bytes_total"] != new_bytes
            or plan_summary["flat_selected_pages_total"] != flat_selected
            or plan_summary["v152_selected_flat_page_capture"] != new_capture
            or plan_summary["v152_graph_work_p95"] != sorted(new_work)[949]
            or plan_summary["v152_exact_page_work_p95"] != sorted(exact_work)[949]
            or plan_summary["v152_distinct_union_work_p95"] != sorted(union_work)[949]
            or plan_summary["v152_total_unit_work_p95"] != sorted(total_work)[949]
            or plan_summary["v152_new_unit_work_p95"] != sorted(new_unit_work)[949]
            or plan_summary["v152_target_shortfall_queries"] != sum(
                value > 0 for value in new_shortfall)
            or plan_summary["v152_target_shortfall_p95"] != sorted(new_shortfall)[949]
            or plan_summary["v151_graph_work_p95"] != sorted(prior_work)[949]
            or plan_summary["v151_exact_page_work_p95"] != sorted(prior_exact)[949]
            or plan_summary["v151_distinct_union_work_p95"] != sorted(prior_union)[949]
            or plan_summary["v151_total_unit_work_p95"] != sorted(prior_total)[949]
            or plan_summary["v151_target_shortfall_queries"] != sum(value > 0 for value in prior_shortfall)
            or plan_summary["v151_target_shortfall_p95"] != sorted(prior_shortfall)[949]
            or any(abs(plan_summary[f"{arm}_p95_ms"] - value) > 1e-9
                   for arm, value in p95.items())
            or any(abs(plan_summary[key] - value) > 1e-9
                   for key, value in parity_p95.items())
            or any(abs(plan_summary[f"v152_{field.removesuffix('_ns')}_p95_ms"]
                       - sorted(values)[949] / 1e6) > 1e-9
                   for field, values in candidate_phases.items())):
        raise ValueError("V152 plan summary recount differs")
    for arm, fields in arm_values.items():
        ...
        if (actual["source_hits"] != sum(source_hits)
                ...
                or actual["source_recall_at_100"] != sum(fields["source"]) / 100000
                or actual["mean_gets"] != sum(fields["gets"]) / 1000
                or actual["mean_union_size"] != sum(fields["union"]) / 1000):
                raise ValueError("V152 returned summary recount differs")
        for field, label in (("sq8_ns", "sq8_p95_ms"),
                             ("source_ns", "source_p95_ms")):
            if abs(actual[label] - sorted(replay_cpu[arm][field])[949] / 1e6) > 1e-9:
                raise ValueError("V152 replay timing summary differs")
    flat = summary["arms"]["flat"]
    candidate = summary["arms"]["v152"]
    paired = [new - old for new, old in zip(arm_values["v152"]["source"],
    ...
    if summary["v152_vs_flat_paired_source_hits"] != expected_paired:
        raise ValueError("V152 paired source outcome recount differs")
    ...
    if (summary["passes_frozen_gate"] is not passed
        ...
        raise ValueError("V152 frozen decision recount differs")
