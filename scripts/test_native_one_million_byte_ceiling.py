"""Byte-only diagnostic phase boundary and Spot wiring."""
...
from scripts.native_one_million_byte_ceiling_cell import run_evaluate, run_validate
...
class OneMillionByteCeilingTests(unittest.TestCase):
    def test_small_replay_and_spot_script(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                patch("scripts.native_one_million_byte_ceiling_cell.SOURCE_IDENTITIES", source),
                patch("scripts.native_one_million_byte_ceiling_cell.DEVELOPMENT_IDENTITIES", development),
                patch("scripts.native_one_million_byte_ceiling_cell.check_prior_page_artifact"),
            ):
            ...
            self.assertEqual(result["schema"], "borsuk-one-million-byte-ceiling-result-v1")
        commit = "12" * 20
        plan = build_plan(
            ...
            output_prefix=(
                "s3://borsuk-bench-453182569524-euc1/research/native-one-million-byte-selector/"
        ...
        self.assertIn("scripts.native_one_million_byte_ceiling_cell construct", script)
        self.assertIn("borsuk-one-million-byte-selector-terminal-v1", script)
