#!/usr/bin/env python3
"""Export the V77 manifest so a native reader can serve V70's published index.
...
def main() -> None:
    ...
    # Coarse level: the page summaries V66 measured, PQ-compressed and restored,
    # padded so the final short page still owns a full block pair.
    summaries = decode_pq(
    ...
    with args.output.open("wb") as handle:
        ...
        for value in (ROWS, DIMENSIONS, PAGE_ROWS, pages, QUERIES, NEIGHBORS,
                      subspaces, width, ROUTER_BLOCKS_PER_PAGE):
    ...
    expected = (
        8 + 9 * 8
        + pages * ROUTER_BLOCKS_PER_PAGE * DIMENSIONS * 4
        + DIMENSIONS * 4 * 2
