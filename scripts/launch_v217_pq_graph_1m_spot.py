#!/usr/bin/env python3
"""Frozen ReLAION-1M source graph/PQ-cosine/FP16 Spot comparison."""
...
SCHEMA = "borsuk-v217-pq-cosine-graph-1m-spot-v1"
TAG = "borsuk-v217-pq-cosine-graph-1m"
WALL_SECONDS = 10_800
...
if {row[0] for row in INPUTS} != {"source.parquet", "old-layout.npy", "order.npy",
    ...
    raise RuntimeError("V217 frozen input roster differs")
LATE = (BASELINE,)
...
def user_data(commit: str, archive_sha: str, archive_key: str, prefix: str) -> str:
    template = r'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v217-hard-stop --on-active=@@WALL@@s /usr/sbin/shutdown -h now
root=/mnt/v217-pq-cosine-graph-1m
...
"$CARGO_HOME/bin/cargo" build --release --locked -p borsuk \
  --bin v217_build_pq_graph_1m --bin v217_serve_pq_graph_1m \
  --jobs 6 >"$root/build.log" 2>&1
cd "$root"
/usr/bin/time -v -o graph-build-resources.txt "$CARGO_TARGET_DIR/release/v217_build_pq_graph_1m" \
  prep.json plane.bin vectors.raw graph.bin build-summary.json
phase=serve
/usr/bin/time -v -o serving-resources.txt "$CARGO_TARGET_DIR/release/v217_serve_pq_graph_1m" \
...
phase=truth
@@LATE_DOWNLOADS@@
.venv/bin/python -m scripts.v217_score_graph_1m \
...
'''
    ...
    if "@@" in template:
        raise ValueError("unresolved V217 worker placeholder")
    return template
...
def launch(attempt: str) -> None:
    ...
    archive_key = f"research/v217-pq-cosine-graph-1m/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v217-pq-cosine-graph-1m/{commit}/runs/{attempt}"
    ...
    put_if_absent(prefix + "/reservation.json", json.dumps({
        ...
        "qualifying_100k_terminal_sha256": "2bbba15b52ee361ba5b5c4fdb9cc73637344b3ed684d8ee157f6513a924c92c6",
    ...
    receipt = ec2.run_instances(
        ClientToken="v217-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
        ...
        while time.monotonic() < deadline:
            try:
                ...
                if (terminal.get("schema") != SCHEMA or terminal.get("instance_id") != instance_id
                    ...
                    raise ValueError("V217 terminal identity differs")
                terminal["terminal_sha256"] = hashlib.sha256(raw).hexdigest()
                print(json.dumps(terminal, sort_keys=True), flush=True)
                if terminal.get("status") != "complete":
                    raise RuntimeError("V217 failed; inspect only closed terminal artifacts")
                if set(terminal.get("artifacts", {})) != set(ARTIFACTS):
                    raise ValueError("V217 artifact roster differs")
                for name, identity in terminal["artifacts"].items():
                    ...
                    if size != identity["bytes"] or value.hexdigest() != identity["sha256"]:
                        raise ValueError(f"V217 artifact readback differs: {name}")
            ...
            if state in {"terminated", "shutting-down"}:
                raise RuntimeError("V217 worker stopped before terminal")
            time.sleep(20)
        raise TimeoutError("V217 Spot cell exceeded wall cap")
    finally:
        try:
            state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
...
if __name__ == "__main__":
    ...
    with open("/tmp/borsuk-v217-pq-cosine-graph-launch.lock", "a+") as lock:
