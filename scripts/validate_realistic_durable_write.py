...
def read_environment(path: Path) -> dict[str, str]:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        require(
            bool(separator) and bool(key) and key not in values,
            f"invalid environment line {line!r}",
        )
        values[key] = value
    return values
...
def validate(
    ...
    *,
    ...
    require(
        copied_path.read_bytes() == frozen,
        "copied manifest differs from frozen manifest",
    )
    manifest = json.loads(frozen)
    identity = read_environment(root / "environment.txt")
    require(
        identity.get("manifest_sha256") == hashlib.sha256(frozen).hexdigest(),
        "manifest SHA-256 mismatch",
    )
    for name in ("source_sha256", "dataset_sha256"):
        require(
            re.fullmatch(r"[0-9a-f]{64}", identity.get(name, "")) is not None,
    ...
    if expected_dataset_sha is not None:
        require(
            identity["dataset_sha256"] == expected_dataset_sha,
    ...
    if terminal_cell is not None:
        require(
            terminal_cell in frozen_cells, "terminal cell is outside the frozen matrix"
        )
        cells = [terminal_cell]
    else:
    ...
    for dataset, repetition, batch_records in cells:
        require(
            re.fullmatch(r"[A-Za-z0-9._-]+", dataset) is not None, "unsafe dataset name"
        )
        ...
        require(
            not (cell / "CELL_FAILED").exists(), f"cell has a failure marker: {cell}"
        )
        require(
            (cell / "INSERT_VISIBILITY_COMPLETE").is_file(),
            f"missing visibility marker in {cell}",
        )
        exit_path = cell / "process_exit.txt"
        require(
            exit_path.is_file() and exit_path.read_text().strip() == "0",
            f"nonzero or missing process exit in {cell}",
        )

        costs = read_rows(cell / "bench_write_costs.csv")
        require(
            len(costs) == 1 and costs[0].get("op") == "insert",
            f"expected exactly one insert aggregate in {cell}",
        )
        cost = costs[0]
        require(
            integer(cost["configured_batch_records"], "configured batch")
            == batch_records,
            f"configured batch drift in {cell}",
        )
        require(
            integer(cost["ops"], "operations") == expected_ops,
            f"operation count drift in {cell}",
        )
        aggregate_batches = integer(cost["batches"], "batches")
        require(
            aggregate_batches >= minimum_batches,
            f"raw batch count below minimum in {cell}",
        )
        ...
        require(
            len(samples) == aggregate_batches and len(samples) >= minimum_batches,
            f"raw batch count mismatch in {cell}",
        )
        observed_records = 0
        request_totals = {
            name: 0 for name in ("gets", "puts", "deletes", "heads", "lists")
        }
        latencies: list[float] = []
        for batch_index, sample in enumerate(samples):
            require(sample.get("op") == "insert", f"non-insert sample in {cell}")
            require(
                integer(sample["batch_index"], "batch index") == batch_index,
                f"batch index drift in {cell}",
            )
            records = integer(sample["batch_records"], "batch records")
            require(
                0 < records <= batch_records,
                f"sample exceeds configured batch in {cell}",
            )
        ...
        require(
            observed_records == expected_ops, f"record reconciliation failed in {cell}"
        )
        for name, total in request_totals.items():
            require(
                total == integer(cost[name], f"aggregate {name}"),
                f"request reconciliation failed for {name} in {cell}",
            )
        raw_p95 = sorted(latencies)[max(math.ceil(len(latencies) * 0.95) - 1, 0)]
        require(
            abs(raw_p95 - p95) <= 0.001,
            f"raw write p95 differs from aggregate in {cell}",
        )

        resources = read_rows(cell / "resources.csv")
        require(len(resources) >= 2, f"resource telemetry is incomplete in {cell}")
        elapsed = [
            finite(row["elapsed_ms"], "resource elapsed time") for row in resources
        ]
        require(
            elapsed == sorted(elapsed), f"resource timeline is not monotonic in {cell}"
        )
        require(
            elapsed[0] <= wall_ms and elapsed[-1] >= wall_ms,
            f"resource timeline does not cover measured wall time in {cell}",
        )
        require(
            resources[-1].get("child_cpu_seconds", "").strip() != "",
            f"resource telemetry lacks exact process totals in {cell}",
        )
