#!/usr/bin/env python3
"""Seal V151 returned IDs without GT, then reduce against fresh sealed GT100."""
...
from scripts.v151_prepare_fresh import COUNT, FIRST, SOURCE_SHA, SQ8_SHA, sha256
...
def checked_inputs(args: argparse.Namespace) -> tuple[list[dict], list[dict], dict]:
    ...
    if (sealed["schema"] != "borsuk-v151-fresh-inputs-v1"
...
def replay(args: argparse.Namespace) -> None:
    ...
    args.replay_seal.write_text(json.dumps({
        "schema": "borsuk-v151-returned-seal-v1",
...
def reduce(args: argparse.Namespace) -> None:
    ...
    summary = {
        "schema": "borsuk-v151-returned-quality-v1",
    ...
    paired = [(new - old) for new, old in
              zip(values["v151"]["source"], values["flat"]["source"])]
    summary["v151_vs_flat_paired_source_hits"] = {
    ...
    summary["passes_frozen_gate"] = (
        ...
        and plan_summary["v151_p95_ms"] < plan_summary["flat_p95_ms"]
