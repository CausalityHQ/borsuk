#!/usr/bin/env python3
"""Run one immutable GT-free V171 1M primary feasibility cell on Spot."""
...
from scripts.v171_primary_feasibility import HASHES

SCHEMA = "borsuk-v171-primary-feasibility-spot-v1"
TAG = "borsuk-v171-primary-feasibility"
WALL_SECONDS = 3_600
IMAGE = "ami-06121aa3085b6f918"
V164 = ("research/v164-smooth-layout/"
        "488fc4702f6fd408385e532f67d0a110daf1ba33/runs/a0001/")
V154 = ("research/v154-relaion-graph/"
        "8f2c5ce4bffadc421fb4a48e4deccf989f803770/"
        "runs/v154-20260924T143744Z/a0002/")
...
URI = {
    "sealed": V116 + "rust-replay.jsonl",
    "old_layout": ("research/v63-algorithm-first/"
                   "layout-oracle-e2f6c2bad99c720b/a0001/artifacts/kmeans_8192-order.npy"),
    "order": V164 + "artifacts/order.npy",
    "v164_terminal": V164 + "terminal.json",
    "layout_seal": V164 + "artifacts/layout-seal.json",
    "v154_terminal": V154 + "terminal.json",
    "science": V154 + "artifacts/science.jsonl",
}
ARTIFACTS = ("raw.jsonl", "summary.json", "check.json",
             "evaluate-resources.txt", "check-resources.txt", "run.log")


def download_script() -> str:
    return "\n".join(
        f"aws s3 cp 's3://{BUCKET}/{URI[name]}' '{name}' --only-show-errors\n"
...
def user_data(commit: str, archive_sha: str, archive_key: str, prefix: str) -> str:
    arguments = " ".join(
        f"--{name.replace('_', '-')} {name}" for name in HASHES
    ) + " --raw raw.jsonl --summary summary.json"
    return f'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v171-primary-hard-stop --on-active={WALL_SECONDS}s /usr/sbin/shutdown -h now
root=/mnt/v171-primary-feasibility
...
.venv/bin/pip install -q numpy==1.26.4
export PYTHONPATH="$root/repo"
export OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 MKL_NUM_THREADS=1
phase=inputs
{download_script()}
phase=evaluate
/usr/bin/time -v .venv/bin/python repo/scripts/v171_primary_feasibility.py evaluate {arguments} >evaluate-resources.txt 2>&1
phase=check
/usr/bin/time -v .venv/bin/python repo/scripts/v171_primary_feasibility.py check {arguments} >check.json 2>check-resources.txt
phase=complete
'''
...
def main() -> None:
    ...
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as tar:
        required = {"scripts/v171_primary_feasibility.py",
                    "docs/research/v171-pq-field-1m-transfer-prereg.md"}
        if not required.issubset(tar.getnames()):
            raise ValueError("source archive lacks V171 feasibility gate")
    archive_sha = hashlib.sha256(archive).hexdigest()
    archive_key = f"research/v171-primary-feasibility/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v171-primary-feasibility/{commit}/runs/{attempt}"
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V171 feasibility worker already active")
    for name, (size, _) in HASHES.items():
        if s3.head_object(Bucket=BUCKET, Key=URI[name])["ContentLength"] != size:
            raise ValueError(f"V171 {name} frozen input length differs")
    ...
    receipt = ec2.run_instances(
        ClientToken="v171-primary-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
        ...
        if not missing(s3, prefix + "/terminal.json"):
            ...
            for name, identity in terminal.get("artifacts", {}).items():
                ...
                if size != identity["bytes"] or digest.hexdigest() != identity["sha256"]:
                    raise ValueError(f"V171 S3 artifact differs: {name}")
            terminal["terminal_sha256"] = hashlib.sha256(raw).hexdigest()
            print(json.dumps(terminal, sort_keys=True), flush=True)
            if terminal.get("status") != "complete":
                raise RuntimeError("V171 feasibility terminal failed; see closed artifacts")
            return
        state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
        if state in {"terminated", "shutting-down"}:
            raise RuntimeError("V171 Spot worker stopped before terminal; restart full cell")
        time.sleep(20)
    ec2.terminate_instances(InstanceIds=[instance_id])
    raise TimeoutError("V171 feasibility cell exceeded wall cap")
