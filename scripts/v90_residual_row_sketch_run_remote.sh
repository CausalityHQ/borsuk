...
if [ "${1:-}" = "--describe" ]; then
  printf '%s\n' '{"max_wall_seconds":1200,"queries_per_arm":32,"sketch_bytes_per_base_row":16,"spot_only":true,"total_arms":2}'
  exit 0
fi
...
root=/mnt/v90-residual-row-sketch
phase=bootstrap
...
publish_terminal() {
  ...
  if [ "$code" -eq 0 ]; then
    for name in dependency.log hashes.log hashes-check.log instance.json run.log time.log result.json; do
      if [ ! -s "$name" ] || ! aws s3 cp "$name" "$V90_OUTPUT_URI/$name" --only-show-errors; then
        code=98
    ...
    for name in dependency.log hashes.log hashes-check.log instance.json run.log time.log result.json; do
      if [ -n "${V90_OUTPUT_URI:-}" ] && [ -f "$name" ]; then
        aws s3 cp "$name" "$V90_OUTPUT_URI/$name" --only-show-errors || true
      fi
    done
  fi
  printf '{"exit_code":%d,"phase":"%s","source_commit":"%s"}\n' \
    "$code" "$phase" "${V90_SOURCE_COMMIT:-unknown}" >/tmp/v90-residual-row-sketch-terminal.json
  if [ -n "${V90_OUTPUT_URI:-}" ] && ! aws s3 cp /tmp/v90-residual-row-sketch-terminal.json \
    "$V90_OUTPUT_URI/terminal.json" --only-show-errors; then
    code=99
  fi
  unlink /tmp/v90-residual-row-sketch-terminal.json
  sync
  shutdown -h now || true
  exit "$code"
}
...
phase=inputs
aws s3 cp "$V90_SCREEN_URI" scripts/v90_residual_row_sketch_screen.py --only-show-errors || exit 92
aws s3 cp "$V90_V88_URI" scripts/v88_planner_objective_screen.py --only-show-errors || exit 92
aws s3 cp "$V90_V87_URI" scripts/v87_summary_capacity_screen.py --only-show-errors || exit 92
aws s3 cp "$V90_V86_URI" scripts/v86_coarse_to_fine_screen.py --only-show-errors || exit 92
aws s3 cp "$V90_SHARED_URI" scripts/v85_shared_overlay_screen.py --only-show-errors || exit 92
printf '%s  scripts/v90_residual_row_sketch_screen.py\n%s  scripts/v88_planner_objective_screen.py\n%s  scripts/v87_summary_capacity_screen.py\n%s  scripts/v86_coarse_to_fine_screen.py\n%s  scripts/v85_shared_overlay_screen.py\n' \
  "$V90_SCREEN_SHA256" "$V90_V88_SHA256" "$V90_V87_SHA256" "$V90_V86_SHA256" "$V90_SHARED_SHA256" >hashes.log
...
/usr/bin/time -v -o time.log timeout --signal=TERM --kill-after=30s 1200s \
  .venv/bin/python -m scripts.v90_residual_row_sketch_screen \
