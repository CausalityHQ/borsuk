#!/usr/bin/env python3
"""Launch one immutable source-only V190 priced interval gate on Spot."""
...
SCHEMA = "borsuk-v190-frozen-rank-replication-spot-v1"
TAG = "borsuk-v190-frozen-rank-replication"
WALL_SECONDS = 3_600
...
INPUTS = (
    ...
    ("frozen-model.json", "research/v189-predicted-interval-source/"
     "ae0b160e0b48c419a6bac1b6d9e5964f7fd4b63b/runs/a0002/"
     "artifacts/out/model.json", 57_622,
     "cef01b55c5a196db187dc303dd002f33b00e75c6083465cf8ff746ea3d9beb5c"),
)
ARTIFACTS = (
    "out/features.jsonl", "out/prepare-seal.json",
    "out/plans.jsonl", "out/planner-runtime.json", "out/plan-seal.json",
    "out/source-labels.jsonl", "out/summary.json",
    "prepare-resources.txt", "plan-resources.txt",
    "evaluate-resources.txt", "run-closed.log",
)
...
def user_data(commit: str, archive_sha: str, archive_key: str, prefix: str) -> str:
    return f'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v190-hard-stop --on-active={WALL_SECONDS}s /usr/sbin/shutdown -h now
root=/mnt/v190-frozen-rank-replication
...
/usr/bin/time -v -o prepare-resources.txt .venv/bin/python -m \
  scripts.v190_frozen_rank_replication prepare --output out \
  --source source.parquet --old-layout old-layout.npy \
  --old-sq8 old-sq8.bin --router router --order order.npy \
  --v164-terminal v164-terminal.json --frozen-model frozen-model.json
# All 256 GT-blind features are durable before source truth.
aws s3 cp out/features.jsonl "s3://{BUCKET}/{prefix}/sealed/features.jsonl" --only-show-errors
aws s3 cp out/prepare-seal.json "s3://{BUCKET}/{prefix}/sealed/prepare-seal.json" --only-show-errors
seal_sha=$(sha256sum out/prepare-seal.json | cut -d ' ' -f1)
phase=plan
/usr/bin/time -v -o plan-resources.txt .venv/bin/python -m \
  scripts.v190_frozen_rank_replication plan --output out \
  --source source.parquet --old-layout old-layout.npy \
  --old-sq8 old-sq8.bin --router router --order order.npy \
  --v164-terminal v164-terminal.json --frozen-model frozen-model.json \
  --prepare-sha256 "$seal_sha"
# Both physical plans are durable before source truth.
for name in plans.jsonl planner-runtime.json plan-seal.json; do
  aws s3 cp "out/$name" "s3://{BUCKET}/{prefix}/sealed/$name" --only-show-errors
done
plan_sha=$(sha256sum out/plan-seal.json | cut -d ' ' -f1)
phase=evaluate
/usr/bin/time -v -o evaluate-resources.txt .venv/bin/python -m \
  scripts.v190_frozen_rank_replication evaluate --output out \
  --source source.parquet --old-layout old-layout.npy \
  --old-sq8 old-sq8.bin --router router --order order.npy \
  --v164-terminal v164-terminal.json --frozen-model frozen-model.json \
...
def _launch() -> None:
    ...
    if subprocess.run(["git", "merge-base", "--is-ancestor", "HEAD",
        ...
        raise ValueError("V190 source commit is not pushed to origin/main")
    ...
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as tar:
        required = {"scripts/v166_surrogate_probe.py",
                    ...
                    "scripts/v190_frozen_rank_replication.py",
                    "docs/research/v190-frozen-rank-replication-prereg.md"}
        if not required.issubset(tar.getnames()):
            raise ValueError("source archive lacks V190 gate")
    archive_sha = hashlib.sha256(archive).hexdigest()
    archive_key = f"research/v190-frozen-rank-replication/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v190-frozen-rank-replication/{commit}/runs/{attempt}"
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V190 worker already active")
    ...
    receipt = ec2.run_instances(
        ClientToken="v190-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
        ...
        while time.monotonic() < deadline:
            if not missing(s3, prefix + "/terminal.json"):
                ...
                for name, identity in terminal.get("artifacts", {}).items():
                    ...
                    if size != identity["bytes"] or digest.hexdigest() != identity["sha256"]:
                        raise ValueError(f"V190 S3 artifact read-back differs: {name}")
                ...
                for name in ("features.jsonl", "prepare-seal.json",
                             "plans.jsonl", "planner-runtime.json",
                             "plan-seal.json"):
                    expected = terminal["artifacts"]["out/" + name]
                    sealed = s3.get_object(
                        Bucket=BUCKET, Key=f"{prefix}/sealed/{name}")
                    digest = hashlib.sha256()
                    size = 0
                    while chunk := sealed["Body"].read(1024 * 1024):
                        digest.update(chunk)
                        size += len(chunk)
                    if (size != expected["bytes"]
                            or digest.hexdigest() != expected["sha256"]
                            or sealed["LastModified"] > labels_time):
                        raise ValueError(f"V190 pre-truth S3 seal differs: {name}")
            ...
            if state in {"terminated", "shutting-down"}:
                ...
                if time.monotonic() - started >= WALL_SECONDS - 60:
                    raise TimeoutError("V190 worker reached wall cap before terminal")
                raise RuntimeError("Spot worker stopped before terminal; restart full cell")
            time.sleep(20)
        ec2.terminate_instances(InstanceIds=[instance_id])
        raise TimeoutError("V190 cell exceeded wall cap")
...
def main() -> None:
    # All V190 controllers on this devbox share one process-lifetime lock.
    # The EC2 tag check below also rejects an already running worker.
    with open("/tmp/borsuk-v190-frozen-rank-replication-launch.lock", "a+") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as exc:
            raise RuntimeError("V190 launcher is already running here") from exc
        _launch()
