#!/usr/bin/env python3
"""Launch and monitor one frozen V133 local SQ8 replay on Spot."""
...
TAG = "borsuk-v133-local-sq8-transport"
...
def validate(plan: Plan) -> None:
    ...
    if (
        ...
        or not output_key.startswith(
            f"research/v133-local-sq8/{plan.source_commit}/runs/"
        )
        ...
        raise ValueError("immutable V133 plan differs")
...
def user_data(plan: Plan) -> str:
    ...
    exports = "\n".join(
        (
            f"export V133_SOURCE_COMMIT={plan.source_commit}",
            f"export V133_ARCHIVE_SHA256={plan.archive_sha256}",
            f"export V133_OUTPUT_PREFIX={shlex.quote(plan.output_prefix)}",
            f"export V133_WALL_SECONDS={plan.wall_seconds}",
            f"export V133_GENERATION_SHA256={GENERATION_SHA}",
            f"export V133_GENERATION_TERMINAL_SHA256={GENERATION_TERMINAL_SHA}",
        )
    )
    return f"""#!/bin/bash
set -euo pipefail
root=/mnt/v133-local-sq8
...
import json,os
print(json.dumps({{"schema":"borsuk-v133-local-sq8-transport-spot-v1",
  "source_commit":os.environ["V133_SOURCE_COMMIT"],
  "source_archive_sha256":os.environ["V133_ARCHIVE_SHA256"],
  "generation_manifest_sha256":os.environ["V133_GENERATION_SHA256"],
  "generation_terminal_sha256":os.environ["V133_GENERATION_TERMINAL_SHA256"],
  "instance_id":os.environ["INSTANCE_ID"],"exit_code":int(os.environ["EXIT_CODE"]),
  "phase":"bootstrap","status":"failed","artifacts":{{}}}},
  sort_keys=True,separators=(',',':')))
PY
    aws s3 cp terminal.json "$V133_OUTPUT_PREFIX/terminal.json" --only-show-errors || true
    shutdown -h now || true
  fi
}}
...
trap - EXIT
exec bash repo/scripts/run_v133_local_sq8_transport_remote.sh
"""


def launch_spec(plan: Plan) -> dict:
    token = "v133-" + hashlib.sha256(plan.output_prefix.encode()).hexdigest()[:48]
    return {
        "ClientToken": token,
...
def reserve(plan: Plan) -> None:
    receipt = json.dumps(
        {
            "schema": "borsuk-v133-local-sq8-reservation-v1",
...
def launch_and_monitor(plan: Plan) -> dict:
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V133 worker already active")
    ...
    while time.monotonic() < deadline:
        if not missing(s3, prefix + "/terminal.json"):
            ...
            terminal = json.loads(raw)
            if (
                terminal.get("schema") != "borsuk-v133-local-sq8-transport-spot-v1"
                ...
                raise ValueError("V133 terminal identity differs")
        ...
        if state == "terminated":
            raise RuntimeError("V133 instance terminated without terminal marker")
        time.sleep(15)
    ec2.terminate_instances(InstanceIds=[instance_id])
    ec2.get_waiter("instance_terminated").wait(InstanceIds=[instance_id])
    raise TimeoutError(
        f"V133 terminal missing after worker deadline: {instance_id} terminated"
    )
...
def main() -> None:
    ...
    if terminal.get("status") != "complete" or terminal.get("exit_code") != 0:
        raise RuntimeError("V133 terminal reports a failed attempt")
