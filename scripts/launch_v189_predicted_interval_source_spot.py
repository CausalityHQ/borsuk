#!/usr/bin/env python3
"""Launch one immutable source-only V189 priced interval gate on Spot."""
...
SCHEMA = "borsuk-v189-predicted-interval-source-spot-v1"
TAG = "borsuk-v189-predicted-interval-source"
WALL_SECONDS = 3_600
...
ARTIFACTS = (
    ...
    "out/fit-labels.jsonl", "out/model.json", "out/plans.jsonl",
    "out/planner-runtime.json",
    "out/plan-seal.json",
    "out/source-labels.jsonl", "out/summary.json",
    "prepare-resources.txt", "fit-plan-resources.txt",
...
def user_data(commit: str, archive_sha: str, archive_key: str, prefix: str) -> str:
    return f'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v189-hard-stop --on-active={WALL_SECONDS}s /usr/sbin/shutdown -h now
root=/mnt/v189-predicted-interval-source
...
/usr/bin/time -v -o prepare-resources.txt .venv/bin/python -m \
  scripts.v189_predicted_interval_source prepare --output out \
...
# All 256 GT-blind features are durable before fit or holdout truth.
...
phase=fit-plan
/usr/bin/time -v -o fit-plan-resources.txt .venv/bin/python -m \
  scripts.v189_predicted_interval_source fit-plan --output out \
  --source source.parquet --old-layout old-layout.npy \
  --old-sq8 old-sq8.bin --router router --order order.npy \
  --v164-terminal v164-terminal.json --prepare-sha256 "$seal_sha"
# Fit labels, model and all holdout plans are durable before holdout truth.
for name in fit-labels.jsonl model.json plans.jsonl planner-runtime.json plan-seal.json; do
...
/usr/bin/time -v -o evaluate-resources.txt .venv/bin/python -m \
  scripts.v189_predicted_interval_source evaluate --output out \
  --source source.parquet --old-layout old-layout.npy \
  --old-sq8 old-sq8.bin --router router --order order.npy \
  --v164-terminal v164-terminal.json --plan-sha256 "$plan_sha"
phase=complete
'''
...
def _launch() -> None:
    ...
    if subprocess.run(["git", "merge-base", "--is-ancestor", "HEAD",
        ...
        raise ValueError("V189 source commit is not pushed to origin/main")
    ...
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as tar:
        required = {"scripts/v166_surrogate_probe.py",
                    ...
                    "docs/research/v189-predicted-priced-interval-prereg.md"}
        if not required.issubset(tar.getnames()):
            raise ValueError("source archive lacks V189 gate")
    archive_sha = hashlib.sha256(archive).hexdigest()
    archive_key = f"research/v189-predicted-interval-source/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v189-predicted-interval-source/{commit}/runs/{attempt}"
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V189 worker already active")
    ...
    receipt = ec2.run_instances(
        ClientToken="v189-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
    ...
    try:
        ...
        while time.monotonic() < deadline:
            if not missing(s3, prefix + "/terminal.json"):
                ...
                fit_labels_time = s3.head_object(
                    Bucket=BUCKET, Key=f"{prefix}/sealed/fit-labels.jsonl"
                )["LastModified"]
                for name in ("features.jsonl", "prepare-seal.json",
                             "fit-labels.jsonl", "model.json",
                    ...
                    if (size != expected["bytes"]
                            ...
                            or sealed["LastModified"] >
                            (fit_labels_time if name in
                             ("features.jsonl", "prepare-seal.json")
                             else holdout_labels_time)):
            ...
            if state in {"terminated", "shutting-down"}:
                ...
                if time.monotonic() - started >= WALL_SECONDS - 60:
                    raise TimeoutError("V189 worker reached wall cap before terminal")
                raise RuntimeError("Spot worker stopped before terminal; restart full cell")
            time.sleep(20)
        ec2.terminate_instances(InstanceIds=[instance_id])
        raise TimeoutError("V189 cell exceeded wall cap")
...
def main() -> None:
    # All V189 controllers on this devbox share one process-lifetime lock.
    # The EC2 tag check below also rejects an already running worker.
    with open("/tmp/borsuk-v189-predicted-interval-source-launch.lock", "a+") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as exc:
            raise RuntimeError("V189 launcher is already running here") from exc
        _launch()
