...
role=$BORSUK_V222_ROLE
bucket=$BORSUK_V222_BUCKET
prefix=$BORSUK_V222_PREFIX
phase=bootstrap
...
role=os.environ['BORSUK_V222_ROLE']
...
print(json.dumps({'schema':'borsuk-v222-external-graph-http-terminal-v1',
    'role':role,'source_commit':os.environ['BORSUK_V222_SOURCE_COMMIT'],
    'source_archive_sha256':os.environ['BORSUK_V222_ARCHIVE_SHA'],
    ...
    'server_instance_id':os.environ.get('BORSUK_V222_SERVER_ID',''),
...
import json,os
print(json.dumps({'schema':'borsuk-v222-external-graph-http-ready-v1',
    'instance_id':os.environ['INSTANCE_ID'],'private_ip':os.environ['PRIVATE_IP'],
    'source_commit':os.environ['BORSUK_V222_SOURCE_COMMIT']},sort_keys=True))
PY
...
assert v['source_commit']==os.environ['BORSUK_V222_SOURCE_COMMIT']
PY
...
PY
  ...
  for attempt in $(seq 1 60); do
    if curl -fsS "http://$BORSUK_V222_SERVER_IP:8080/health" >/dev/null 2>&1; then break; fi
    sleep 2
  done
  curl -fsS "http://$BORSUK_V222_SERVER_IP:8080/health"
  phase=measure
  for pass in first repeat; do
    ...
    /usr/bin/time -v -o "$pass.time" python3.12 -m scripts.v220_bench_graph_http \
      --host "$BORSUK_V222_SERVER_IP" --port 8080 --pass-label "$label" \
