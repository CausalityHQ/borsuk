#!/bin/bash
# One frozen V152 fresh returned-quality screen on Causality Spot.
set -euo pipefail
...
deadline_epoch=$((started_epoch + V152_WALL_SECONDS - 900))
monitor_pid=
imds_token=
imds() {
...
finish() {
  ...
  if [ -d /sys/fs/cgroup/v152-science ]; then
    { cat /sys/fs/cgroup/v152-science/memory.current;
      cat /sys/fs/cgroup/v152-science/memory.peak;
      cat /sys/fs/cgroup/v152-science/memory.stat; } >science.cgroup-memory.txt
  fi
  upload_failed=0
  for path in install.log core-check.log core-check-resources.txt \
    ...
    if [ -f "$path" ] && ! timeout 300 aws s3 cp "$path" \
        "$V152_OUTPUT_PREFIX/artifacts/$path" --only-show-errors; then
      upload_failed=1
    fi
  done
...
PY
  aws s3 cp terminal.json "$V152_OUTPUT_PREFIX/terminal.json" --only-show-errors
  shutdown -h now || true
  exit "$code"
}
trap finish EXIT
...
run_science gate-build "$CARGO_HOME/bin/cargo" build --release --locked -j 8 \
  --manifest-path repo/crates/borsuk/Cargo.toml --bin v152_cached_page_graph
phase=download
: >download.log
...
PYINNER
mkdir /sys/fs/cgroup/v152-science
phase=prepare
run_science prepare .venv/bin/python -m scripts.v152_prepare_fresh \
...
run_science science bash -c \
  'echo "$BASHPID" >/sys/fs/cgroup/v152-science/cgroup.procs; exec "$@"' \
  v152 "$CARGO_TARGET_DIR/release/v152_cached_page_graph" \
...
{ cat /sys/fs/cgroup/v152-science/memory.current;
  ...
  cat /sys/fs/cgroup/v152-science/memory.stat; } >science.cgroup-memory.txt
phase=replay
run_science replay .venv/bin/python -m scripts.v152_returned_quality replay \
...
phase=audit
run_science audit .venv/bin/python -m scripts.v152_returned_audit \
  --source deep/source.parquet --sq8 deep/built/sq8.bin \
  --centroids deep/centroids.bin --plans science.jsonl \
  --low deep/router/low.bin --step deep/router/step.bin \
  --queries fresh/queries.jsonl --routing fresh/routing.jsonl \
  --replay replay.jsonl --output returned.audit.json
phase=reduce
run_science reduce .venv/bin/python -m scripts.v152_returned_quality reduce \
...
if value.get('schema')!='borsuk-v152-returned-quality-v1':
    raise ValueError('V152 returned summary differs')
with open('decision.json','w') as output:
    json.dump({'schema':'borsuk-v152-decision-v1',
