"""Closed pq96 evidence is bound to readback bytes and resource receipts."""
...
from scripts.closeout_native_hundred_thousand_pq96 import (
    closeout_receipts,
    compare_frozen_source_control,
)
from scripts.native_hundred_thousand_pq96_worker import ARTIFACT_FILES
from scripts.test_launch_native_hundred_thousand_pq96_spot import Pq96SpotTest
...
class Pq96CloseoutTest(unittest.TestCase):
    def test_exact_source_control_matches_prior_screen(self) -> None:
        with TemporaryDirectory() as directory:
            root = Path(directory)
            samples = [
                {"query_ordinal": index, "selected_groups": [0],
                 "source": {"priority_pages": [["base", 0]]}}
                for index in range(1000)
            ]
            pq, sign = root / "pq.json", root / "sign.json"
            pq.write_bytes(_body({"samples": samples}))
            sign.write_bytes(_body({"samples": samples}))
            self.assertEqual(compare_frozen_source_control(pq, sign), 1000)
            samples[999]["source"]["priority_pages"] = [["base", 1]]
            pq.write_bytes(_body({"samples": samples}))
            with self.assertRaisesRegex(ValueError, "source control differs"):
                compare_frozen_source_control(pq, sign)

    def test_closed_readback_and_resource_gate(self) -> None:
        plan = Pq96SpotTest.plan()
        archive = b"archive"
        ...
        with TemporaryDirectory() as directory:
            ...
            (root / ARTIFACT_FILES["decision"]).write_bytes(_body({
                "schema": "borsuk-hundred-thousand-pq96-decision-v1",
            ...
            (root / "terminal.json").write_bytes(_body({
                "schema": "borsuk-hundred-thousand-pq96-terminal-v1",
