#!/usr/bin/env python3
"""V181 sealed source-only candidate-universe ceiling for relaid 1M SQ8."""
...
SCHEMA = "borsuk-v181-neighborhood-reach-v1"
FIRST = 896
COUNT = 256
FIT = 128
WIDTHS = (8, 16, 32)
...
def select_qualified_width(totals: dict[str, int],
                           p05: dict[str, int]) -> int | None:
    """Smallest preregistered width meeting both source holdout gates."""
    threshold = (TARGET_HITS_PER_100K * FIT * 100 + 99_999) // 100_000
    if (set(totals) != {str(width) for width in WIDTHS}
            or set(p05) != set(totals)
            or any(not 0 <= hits <= FIT * 100 for hits in totals.values())
            or any(not 0 <= count <= 100 for count in p05.values())):
        raise ValueError("V181 width qualification totals differ")
    return next((width for width in WIDTHS
                 if totals[str(width)] >= threshold and p05[str(width)] >= 98),
                None)
...
def expanded_units(base: list[int] | tuple[int, ...], width: int,
    ...
    if (width < 0 or unit_count <= 0 or not base
            ...
            or any(type(unit) is not int or not 0 <= unit < unit_count
                   for unit in base)):
        raise ValueError("V181 candidate-unit geometry differs")
...
def coverage(truth_rows: np.ndarray, inverse_new: np.ndarray,
    ...
    if (truth_rows.shape != (100,) or len(set(map(int, truth_rows))) != 100
        ...
        raise ValueError("V181 source truth geometry differs")
...
def _inputs(args: argparse.Namespace):
    if (sha256(args.source) != SOURCE_SHA or sha256(args.old_layout) != LAYOUT_SHA
        ...
        raise ValueError("V181 source/layout/SQ8 identity differs")
    ...
    if (manifest["source_sha256"] != SOURCE_SHA
        ...
        raise ValueError("V181 router authority differs")
    ...
    if not np.array_equal(sq8["id"], source_ids[old]):
        raise ValueError("V181 SQ8/source mapping differs")
    return old, inverse_new, source_ids, vectors, sq8, planes


def prepare(args: argparse.Namespace) -> None:
    if args.output.exists():
        raise ValueError("V181 output already exists")
    ...
    with (args.output / "rosters.jsonl").open("x") as output:
        for ordinal, stable_id in enumerate(chosen, start=FIRST):
            ...
            if nominees.shape != (512,) or len(np.unique(nominees)) != 512:
                raise ValueError("V181 nominee roster differs")
            ...
            if not set(mandatory).issubset(base):
                raise ValueError("V181 primary units outside nominees")
    ...
    (args.output / "prepare-seal.json").write_text(canonical({
        ...
        "split": "source-pseudoquery-hash-ranks-897-1152",
        "fit": "897-1024", "holdout": "1025-1152",
...
def evaluate(args: argparse.Namespace) -> None:
    ...
    if (seal.get("schema") != SCHEMA + "-prepare-seal"
        ...
        raise ValueError("V181 sealed roster differs")
    ...
    if (sha256(args.output / "prepare-seal.json") != args.prepare_sha256
        ...
        raise ValueError("V181 prepare seal identity differs")
    ...
    for start in range(0, ROWS, 8192):
        ...
        if not np.isfinite(norms).all() or (norms <= 0).any():
            raise ValueError("V181 source norm differs")
    ...
    per_query = {split: {str(width): [] for width in WIDTHS}
                 for split in ("fit", "holdout")}
    sizes = {split: {str(width): [] for width in WIDTHS}
             for split in ("fit", "holdout")}
    with (args.output / "source-labels.jsonl").open("x") as output:
        for index, row in enumerate(rows):
            ...
            if (type(source_row) is not int or not 0 <= source_row < ROWS
                ...
                raise ValueError("V181 source row identity differs")
            base = row["base_units"]
            ...
            if (not isinstance(base, list) or not isinstance(mandatory, list)
                ...
                raise ValueError("V181 unit roster differs")
            ...
            for width, count in counts.items():
                ...
                per_query[split][width].append(count)
                sizes[split][width].append(
                    len(expanded_units(base, int(width), unit_count)))
            output.write(canonical({
                "ordinal": row["ordinal"], "source_id": row["source_id"],
                "split": split, "coverage": counts,
                "truth_ids": [int(value) for value in source_ids[truth]],
            }))
    p05 = {split: {width: sorted(counts)[(FIT * 5 + 99) // 100 - 1]
                   for width, counts in cases.items()}
           for split, cases in per_query.items()}
    qualified = select_qualified_width(totals["holdout"], p05["holdout"])
    size_summary = {
        split: {width: {"min": min(counts),
                        "median": float(np.median(counts)),
                        "p95": float(np.percentile(counts, 95)),
                        "max": max(counts)}
                for width, counts in cases.items()}
        for split, cases in sizes.items()
    }
    (args.output / "summary.json").write_text(canonical({
        ...
        "query_count_per_split": FIT, "totals": totals, "p05": p05,
        "candidate_units": size_summary,
        "selected_width": qualified,
        "holdout_neighborhood_decision": (
            "advance-to-ranking-and-physical-gate" if qualified is not None
            else "kill-local-neighborhood-expansion"),
        "holdout_hit_threshold": (TARGET_HITS_PER_100K * FIT * 100
                                  + 99_999) // 100_000,
        "target_hits_per_100k": TARGET_HITS_PER_100K,
...
def main() -> None:
    ...
    if args.phase == "prepare":
        ...
        if len(args.prepare_sha256) != 64:
            raise ValueError("V181 evaluate requires externally sealed prepare SHA")
        evaluate(args)
