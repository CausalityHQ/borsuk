...
V244_PREFIX = ("research/v244-owner-commit-100k/"
               "ad367b68fa2fb47e96851fe29460a120535c64dc/runs/a0001/")
V244_TERMINAL_SHA = "71df3134b48f222542769f96ca1cad520676b4de166e68c7a05c466ed7559c44"
...
def user_data(commit: str, archive_sha: str, archive_key: str, prefix: str,
              schema: str = SCHEMA, batched: bool = False) -> str:
    template = r'''#!/bin/bash
set -euo pipefail
...
/usr/bin/time -v -o graph-build-resources.txt "$CARGO_TARGET_DIR/release/v219_build_reachable_graph_1m" \
  prep.json plane.bin vectors.raw graph.bin build-summary.json @@BUILD_ARGS@@
phase=serve
...
'''
    replacements = {
        ...
        "BUILD_ARGS": "--batched-threads 8" if batched else "",
...
def launch(attempt: str, batched: bool = False) -> None:
    ...
    campaign = "v245-owner-graph-1m" if batched else "v219-reachable-graph-1m"
    schema = "borsuk-v245-owner-graph-1m-spot-v1" if batched else SCHEMA
    qualifying_prefix = V244_PREFIX if batched else QUALIFYING_PREFIX
    qualifying_sha = V244_TERMINAL_SHA if batched else QUALIFYING_TERMINAL_SHA
    ...
    if (hashlib.sha256(qualifying_raw).hexdigest() != qualifying_sha
    ...
    put_if_absent(prefix + "/reservation.json", json.dumps({
        ...
        "qualifying_100k_terminal_sha256": qualifying_sha,
        ...
        "gate": ("ef4096 hits>=99614;dev>=25498;remaining>=74116;p05>=98;reachable=1000000;build<=900s;build-RSS<=6000000000B;loaded-p95<=25ms;p99<=30ms;QPS>=400;serve-RSS<=3GiB;0 vector GET"
                 if batched else "hits>=99605;p05>=98;loaded-p95<92.23ms;loaded-p99<137.87ms;loaded-QPS>=100;RSS<=3GiB;0 vector GET"),
        "baseline_scope": ("V219 same-panel in-process graph/FP16, distinct source revision and host"
                           if batched else "V199 p95/p99 contain S3+SQ8+FP16 but exclude route/plan; advancement ceilings only"),
    ...
    receipt = ec2.run_instances(
        ClientToken=("v245-" if batched else "v219-") + hashlib.sha256(prefix.encode()).hexdigest()[:48],
        ...
        TagSpecifications=[{"ResourceType": "instance", "Tags": [
            {"Key": "Name", "Value": "borsuk-" + campaign if batched else TAG}, {"Key": "BorsukAttempt", "Value": attempt}]}],
        UserData=base64.b64encode(user_data(commit, archive_sha, archive_key, prefix, schema, batched).encode()).decode(),
    )
...
if __name__ == "__main__":
    ...
    with open("/tmp/borsuk-v219-reachable-graph-launch.lock", "a+") as lock:
        ...
        launch(args.attempt, args.batched)
