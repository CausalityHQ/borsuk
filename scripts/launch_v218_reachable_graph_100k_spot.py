#!/usr/bin/env python3
"""Frozen reachable source graph/PQ/FP16 100k gate on Spot."""
...
SCHEMA = "borsuk-v218-reachable-graph-100k-spot-v1"
TAG = "borsuk-v218-reachable-graph-100k"
WALL_SECONDS = 7_200
...
if len(INPUTS) != 8:
    raise RuntimeError("V218 frozen input roster differs")
LATE = tuple(row for row in V158_INPUTS if row[0] == "truth.parquet") + (
    ("v215-raw.jsonl", SELECTING_PREFIX + "artifacts/raw.jsonl", 1_351_492,
     "40e24eb455b94714442a871033585373a587688c85091bdcf79ab2c6a34108c9"),
    ("v216-raw.jsonl", "research/v216-pq-cosine-heldout-100k/"
     "bf5db61a8f0f84543e57338be224e8636a41a170/runs/a0001/artifacts/raw.jsonl",
     813_163, "bbf2aac2d54e07556974dfb6f86d32161607e535d2bfc14bd9d9b7e52c5090f0"),
)
...
def user_data(commit: str, archive_sha: str, archive_key: str, prefix: str,
              schema: str, owned_f32: bool = False, batched: bool = False) -> str:
    template = r'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v218-hard-stop --on-active=@@WALL@@s /usr/sbin/shutdown -h now
root=/mnt/v218-reachable-graph-100k
...
phase=truth
@@LATE_DOWNLOADS@@
.venv/bin/python -m scripts.v218_score_reachable_graph_100k \
  --serving serving.json --raw raw.jsonl --v215 v215-raw.jsonl \
  --v216 v216-raw.jsonl --truth truth.parquet --output quality.json
phase=complete
'''
    replacements = {
  ...
  -- --exact >>"$root/build.log" 2>&1''' if batched else ""),
        "GRAPH_COMMANDS": ('''/usr/bin/time -v -o graph-one-resources.txt "$CARGO_TARGET_DIR/release/v218_build_reachable_graph_100k" \
...
def launch(attempt: str, owned_f32: bool = False, cached_prune: bool = False,
    ...
    if sum((owned_f32, cached_prune, batched)) > 1:
    ...
    put_if_absent(prefix + "/reservation.json", json.dumps({
        ...
        "arms": [[2048, 2048]],
        "gate": ("thread1=thread8 graph SHA;reachable=100000;min-indegree>=4;GT100>=99721;held>=74200;p05>=99;p95-visits<=24166;thread8-build<=70s;RSS<=600000000B"
                 if batched else
    ...
    receipt = ec2.run_instances(
        ClientToken=("v243-" if batched else "v242-" if cached_prune else "v241-" if owned_f32 else "v218-") + hashlib.sha256(prefix.encode()).hexdigest()[:48],
        ...
        TagSpecifications=[{"ResourceType": "instance", "Tags": [
            {"Key": "Name", "Value": "borsuk-" + campaign if batched else campaign if owned_f32 or cached_prune else TAG}, {"Key": "BorsukAttempt", "Value": attempt}]}],
        UserData=base64.b64encode(user_data(commit, archive_sha, archive_key, prefix, schema,
                                           owned_f32 or cached_prune or batched, batched).encode()).decode(),
    )
    ...
    try:
        ...
        while time.monotonic() < deadline:
            try:
                ...
                for name, identity in terminal["artifacts"].items():
                    ...
                    if size != identity["bytes"] or value.hexdigest() != identity["sha256"]:
                        raise ValueError(f"V218 artifact readback differs: {name}")
            ...
            time.sleep(20)
        raise TimeoutError("V218 Spot cell exceeded wall cap")
    finally:
        try:
            state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
