#!/usr/bin/env python3
"""Launch and monitor one frozen V144 page-parity feasibility Spot attempt."""
...
REGION = "eu-central-1"
TAG = "borsuk-v144-page-parity"
RUNNER = "scripts/run_v144_page_parity_remote.sh"
INPUTS = (
...
def validate(plan: Plan) -> None:
    ...
    if (
        ...
        or not output_key.startswith(
            f"research/v144-page-parity/{plan.source_commit}/runs/"
        )
        ...
        raise ValueError("immutable V144 plan differs")
...
def user_data(plan: Plan) -> str:
    ...
    exports = "\n".join(
        (
            f"export V144_SOURCE_COMMIT={plan.source_commit}",
            f"export V144_ARCHIVE_SHA256={plan.archive_sha256}",
            f"export V144_OUTPUT_PREFIX={shlex.quote(plan.output_prefix)}",
            f"export V144_WALL_SECONDS={plan.wall_seconds}",
        )
    )
    return f"""#!/bin/bash
set -euo pipefail
systemd-run --unit=v144-hard-stop --on-active={plan.wall_seconds}s /usr/sbin/shutdown -h now
root=/mnt/v144-page-parity
...
import json,os
print(json.dumps({{"schema":"borsuk-v144-page-parity-feasibility-spot-v1",
  "source_commit":os.environ["V144_SOURCE_COMMIT"],
  "source_archive_sha256":os.environ["V144_ARCHIVE_SHA256"],
  "instance_id":os.environ["INSTANCE_ID"],"exit_code":int(os.environ["EXIT_CODE"]),
  "phase":"bootstrap","status":"failed","artifacts":{{}}}},
  sort_keys=True,separators=(',',':')))
PY
    aws s3 cp terminal.json "$V144_OUTPUT_PREFIX/terminal.json" --only-show-errors || true
    shutdown -h now || true
  fi
}}
...
def launch_spec(plan: Plan) -> dict:
    token = "v144-" + hashlib.sha256(plan.output_prefix.encode()).hexdigest()[:48]
    return {
        "ClientToken": token,
...
def check_source_and_inputs(s3: object, plan: Plan) -> None:
    ...
    with tarfile.open(fileobj=archive, mode="r:gz") as source:
        if (RUNNER not in source.getnames()
                ...
                or "scripts/v114_score_rust/src/bin/v144_page_parity.rs" not in source.getnames()
            ...
            raise ValueError("source archive lacks V144 runner or evaluator")
...
def reserve(plan: Plan) -> None:
    receipt = json.dumps({
        "schema": "borsuk-v144-page-parity-reservation-v1",
...
def launch_and_monitor(plan: Plan) -> dict:
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V144 worker already active")
    ...
    try:
        while time.monotonic() < deadline:
            if not missing(s3, prefix + "/terminal.json"):
                ...
                if (terminal.get("schema") != "borsuk-v144-page-parity-feasibility-spot-v1"
                    ...
                    raise ValueError("V144 terminal identity differs")
                ...
                required = {"install.log", "core-check-resources.txt",
                            "parity-build-resources.txt", "download.log",
                            "deep-resources.txt", "deep.scores.bin", "deep.raw.jsonl",
                            "deep.summary.json", "deep-parity-resources.txt",
                            "deep.parity.jsonl", "deep.parity.summary.json",
                            "relaion-resources.txt", "relaion.scores.bin",
                            "relaion.raw.jsonl", "relaion.summary.json",
                            "relaion-parity-resources.txt", "relaion.parity.jsonl",
                            "relaion.parity.summary.json", "decision.json",
                ...
                if terminal.get("status") == "complete":
                    ...
                    if decision != {"schema": "borsuk-v144-decision-v1",
                                    "deep_completed": True, "relaion_completed": True}:
                        raise ValueError("V144 decision differs")
                    artifacts = terminal.get("artifacts", {})
                    if not required.issubset(artifacts):
                        raise ValueError("V144 complete terminal lacks required artifacts")
                    for name, metadata in artifacts.items():
                        ...
                        if length != metadata["bytes"] or digest.hexdigest() != metadata["sha256"]:
                            raise ValueError(f"V144 artifact differs: {name}")
                    terminal["authenticated_artifact_count"] = len(artifacts)
                print(json.dumps(terminal, sort_keys=True), flush=True)
                return terminal
            state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
            if state == "terminated":
                raise RuntimeError("V144 instance terminated without terminal marker")
            time.sleep(15)
        raise TimeoutError(f"V144 terminal missing after worker deadline: {instance_id}")
...
def main() -> None:
    ...
    if terminal.get("status") != "complete" or terminal.get("exit_code") != 0:
        raise RuntimeError("V144 terminal reports a failed attempt")
