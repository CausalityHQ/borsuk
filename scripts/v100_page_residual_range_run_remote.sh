...
output_prefix=$V100_OUTPUT_PREFIX
...
finish() {
  ...
  STARTED_EPOCH="$started_epoch" FINISHED_EPOCH="$finished_epoch" \
    ...
    SWAP_END_KIB="$swap_end_kib" SPOT_PRICE_MICROS="$V100_SPOT_PRICE_MICROS" \
    python3.12 - <<'PY'
import json, os
...
if status == "complete":
    for role, filename in (
        ...
        evidence[role] = Identity(
            uri=f'{os.environ["V100_OUTPUT_PREFIX"]}/evidence/{filename}',
...
terminal = Terminal(
    ...
    source_commit=os.environ["V100_SOURCE_COMMIT"],
    critique_result_sha256=os.environ["V100_CRITIQUE_SHA256"],
    instance_id=os.environ["INSTANCE_ID"],
    exit_code=int(os.environ["EXIT_CODE"]),
    spot_price_usd_per_hour_micros=int(os.environ["V100_SPOT_PRICE_MICROS"]),
    evidence=evidence,
)
...
phase=source
[ -f source.tar.gz ] || aws s3 cp "$V100_SOURCE_ARCHIVE_URI" source.tar.gz --only-show-errors || exit 92
[ "$(stat -c%s source.tar.gz)" = "$V100_SOURCE_ARCHIVE_BYTES" ] || exit 93
...
for role in source queries truth generation base delta; do
  upper=${role^^}
  uri_name=V100_${upper}_URI
  sha_name=V100_${upper}_SHA256
  bytes_name=V100_${upper}_BYTES
  case "$role" in
...
from scripts.v100_page_residual_range_screen import select_ranked_gap_ranges
...
maximum = float(os.environ["V100_PREFLIGHT_MAX_SECONDS"])
passed = (
    len(exact) == shortlist
...
phase=producer
/usr/bin/time -v -o producer.time timeout "$V100_WALL_SECONDS" .venv/bin/python - <<'PY' >producer.log 2>&1 || exit 97
import os
from pathlib import Path
from scripts.v97_row_width_screen import ObjectIdentity, ScreenAuthority, load_screen_inputs, page_map_sha256
from scripts.v100_page_residual_range_screen import RankedGapConfig, canonical_v100_result_bytes, evaluate_v100
...
identities = {
    role: ObjectIdentity(
        uri=os.environ[f"V100_{role.upper()}_URI"],
        sha256=os.environ[f"V100_{role.upper()}_SHA256"],
        bytes=int(os.environ[f"V100_{role.upper()}_BYTES"]),
    )
    for role in roles
}
inputs = load_screen_inputs(
    **{role: Path(names[role]) for role in roles}, identities=identities,
    dimensions=768, neighbors=100, query_count=int(os.environ["V100_QUERY_COUNT"]), seed=7216,
)
...
config = RankedGapConfig(
    pages_per_root=int(os.environ["V100_PAGES_PER_ROOT"]),
    maximum_root_groups=int(os.environ["V100_MAXIMUM_ROOT_GROUPS"]),
    maximum_exposed_pages=int(os.environ["V100_MAXIMUM_EXPOSED_PAGES"]),
    retained_pages=int(os.environ["V100_RETAINED_PAGES"]),
    maximum_scanned_rows=int(os.environ["V100_MAXIMUM_SCANNED_ROWS"]),
    shortlist_rows=int(os.environ["V100_SHORTLIST_ROWS"]),
    maximum_gets=int(os.environ["V100_MAXIMUM_GETS"]),
    maximum_bytes=int(os.environ["V100_MAXIMUM_BYTES"]),
)
Path("result.json").write_bytes(canonical_v100_result_bytes(evaluate_v100(inputs, authority, config)))
PY
...
from scripts.v100_page_residual_range_screen import RankedGapConfig
from scripts.v100_page_residual_range_rescore import canonical_v100_rescore_bytes, rescore_v100_result
...
authority = ScreenAuthority(
    source_commit=os.environ["V100_SOURCE_COMMIT"], critique_result_sha256=os.environ["V100_CRITIQUE_SHA256"],
    page_map_sha256=Path("page_map.sha256").read_text().strip(), dimensions=768, seed=7216, identities=identities,
)
config = RankedGapConfig(
    pages_per_root=int(os.environ["V100_PAGES_PER_ROOT"]), maximum_root_groups=int(os.environ["V100_MAXIMUM_ROOT_GROUPS"]),
    maximum_exposed_pages=int(os.environ["V100_MAXIMUM_EXPOSED_PAGES"]), retained_pages=int(os.environ["V100_RETAINED_PAGES"]),
    maximum_scanned_rows=int(os.environ["V100_MAXIMUM_SCANNED_ROWS"]), shortlist_rows=int(os.environ["V100_SHORTLIST_ROWS"]),
    maximum_gets=int(os.environ["V100_MAXIMUM_GETS"]), maximum_bytes=int(os.environ["V100_MAXIMUM_BYTES"]),
)
...
Path("rescore.json").write_bytes(canonical_v100_rescore_bytes(summary))
PY
