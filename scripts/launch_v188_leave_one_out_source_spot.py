#!/usr/bin/env python3
"""Launch one immutable source-only V188 leave-one-out gate on Spot."""
...
SCHEMA = "borsuk-v188-leave-one-out-source-spot-v1"
TAG = "borsuk-v188-leave-one-out-source"
WALL_SECONDS = 3_600
...
ARTIFACTS = (
    ...
    "prepare-resources.txt", "evaluate-resources.txt", "run-closed.log",
)
...
def user_data(commit: str, archive_sha: str, archive_key: str, prefix: str) -> str:
    return f'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v188-hard-stop --on-active={WALL_SECONDS}s /usr/sbin/shutdown -h now
root=/mnt/v188-leave-one-out-source
...
/usr/bin/time -v -o prepare-resources.txt .venv/bin/python -m \
  scripts.v188_leave_one_out_source prepare --output out \
...
# Both complete GT-blind rank lists are durable before exact source truth.
...
/usr/bin/time -v -o evaluate-resources.txt .venv/bin/python -m \
  scripts.v188_leave_one_out_source evaluate --output out \
  --source source.parquet --old-layout old-layout.npy \
  --old-sq8 old-sq8.bin --router router --order order.npy \
...
def _launch() -> None:
    ...
    if subprocess.run(["git", "merge-base", "--is-ancestor", "HEAD",
        ...
        raise ValueError("V188 source commit is not pushed to origin/main")
    ...
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as tar:
        required = {"scripts/v166_surrogate_probe.py",
                    ...
                    "scripts/v188_leave_one_out_source.py",
                    "docs/research/v188-leave-one-out-source-prereg.md"}
        if not required.issubset(tar.getnames()):
            raise ValueError("source archive lacks V188 gate")
    archive_sha = hashlib.sha256(archive).hexdigest()
    archive_key = f"research/v188-leave-one-out-source/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v188-leave-one-out-source/{commit}/runs/{attempt}"
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V188 worker already active")
    ...
    receipt = ec2.run_instances(
        ClientToken="v188-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
        ...
        while time.monotonic() < deadline:
            if not missing(s3, prefix + "/terminal.json"):
                ...
                for name, identity in terminal.get("artifacts", {}).items():
                    ...
                    if size != identity["bytes"] or digest.hexdigest() != identity["sha256"]:
                        raise ValueError(f"V188 S3 artifact read-back differs: {name}")
                ...
                for name in ("features.jsonl", "prepare-seal.json"):
                    ...
                    if (size != expected["bytes"]
                        ...
                        raise ValueError(f"V188 pre-truth S3 seal differs: {name}")
                terminal["terminal_sha256"] = hashlib.sha256(raw).hexdigest()
                print(json.dumps(terminal, sort_keys=True), flush=True)
                if terminal.get("status") != "complete":
                    raise RuntimeError("V188 terminal reports failed; see closed artifacts")
                return
            ...
            if state in {"terminated", "shutting-down"}:
                ...
                if time.monotonic() - started >= WALL_SECONDS - 60:
                    raise TimeoutError("V188 worker reached wall cap before terminal")
                raise RuntimeError("Spot worker stopped before terminal; restart full cell")
            time.sleep(20)
        ec2.terminate_instances(InstanceIds=[instance_id])
        raise TimeoutError("V188 cell exceeded wall cap")
...
def main() -> None:
    # All V188 controllers on this devbox share one process-lifetime lock.
    # The EC2 tag check below also rejects an already running worker.
    with open("/tmp/borsuk-v188-leave-one-out-source-launch.lock", "a+") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as exc:
            raise RuntimeError("V188 launcher is already running here") from exc
        _launch()
