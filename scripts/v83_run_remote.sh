...
root=/mnt/v83-reader-profile
prefix=s3://borsuk-bench-453182569524-euc1/research/v83-algorithm-first/reader-profile
output_uri="$prefix/a0002"
...
publish_terminal() {
  ...
  trap - EXIT
  set +e
  for name in build.log export.log run.log hashes.log perf-stat.log perf-report.txt perf.data.zst; do
  ...
  printf '{"exit_code":%d,"phase":"%s"}\n' "$code" "$phase" >/tmp/v83-terminal.json
  aws s3 cp /tmp/v83-terminal.json "$output_uri/terminal.json" --only-show-errors
  unlink /tmp/v83-terminal.json
  exit "$code"
}
...
dnf install -y -q gcc perf zstd >build.log 2>&1 || exit 88
...
21b4f913dd2c0bde9a3f42fe63f1055cdc980fa9ace4957dd0a022b36e3eb3ad  reader/src/main.rs
...
export BORSUK_V71_OUTPUT="$root/result.json"
perf stat \
...
perf report -i perf.data --stdio --no-children --sort comm,dso,symbol \
  --percent-limit 0.1 >perf-report.txt 2>&1 || exit 99
zstd -q -3 perf.data -o perf.data.zst || exit 100
