#!/usr/bin/env python3
"""Independent semantic reducer for V104 exact-retention evidence."""

from __future__ import annotations

import hashlib
import json
from dataclasses import asdict, dataclass
from types import SimpleNamespace
from typing import Literal

from scripts.v97_row_width_screen import ScreenAuthority, ScreenInputs
from scripts.v98_hierarchical_row_router import (
    _hierarchy_evidence,
    build_hierarchy,
    route_hierarchy,
)
from scripts.v99_ranked_gap_range_router_rescore import (
    RescoreAggregate,
    _range_aggregate,
    _range_sample,
)
from scripts.v104_exact_retention_ladder import (
    REGISTERED_RETENTION_PAGES,
    V104Config,
    _arm_fence,
)


@dataclass(frozen=True, slots=True)
class V104RescoreArm:
    """Independently reduced evidence for one retention arm."""

    retained_pages: int
    maximum_scanned_rows: int
    maximum_observed_scanned_rows: int
    aggregate: RescoreAggregate


@dataclass(frozen=True, slots=True)
class V104Rescore:
    """Authenticated independent V104 decision summary."""

    schema: str
    result_sha256: str
    query_count: int
    classification: str
    arms: tuple[V104RescoreArm, ...]
    winner_retained_pages: int | None
    status: Literal["verified"]


def _canonical(value: object) -> bytes:
    return (
        json.dumps(value, allow_nan=False, separators=(",", ":"), sort_keys=True).encode()
        + b"\n"
    )


def _normalized(value: object) -> object:
    return json.loads(json.dumps(value, allow_nan=False, sort_keys=True))


def _fail(label: str) -> None:
    raise ValueError(f"V104 {label} differs")


def rescore_v104_result(
    body: bytes,
    *,
    expected_inputs: ScreenInputs,
    expected_authority: ScreenAuthority,
    expected_config: V104Config,
    expected_result_sha256: str,
) -> V104Rescore:
    """Authenticate bytes and independently reduce every V104 arm."""

    if (
        len(expected_result_sha256) != 64
        or any(character not in "0123456789abcdef" for character in expected_result_sha256)
        or hashlib.sha256(body).hexdigest() != expected_result_sha256
    ):
        _fail("result identity")
    try:
        value = json.loads(body)
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise ValueError("V104 result JSON differs") from error
    if type(value) is not dict or _canonical(value) != body:
        _fail("canonical result")
    expected_keys = {
        "schema",
        "authority",
        "config",
        "query_count",
        "hierarchy",
        "classification",
        "arms",
        "winner_retained_pages",
    }
    if (
        set(value) != expected_keys
        or value["schema"] != "borsuk-v104-exact-retention-ladder-v1"
        or value["authority"] != _normalized(asdict(expected_authority))
        or value["config"] != _normalized(asdict(expected_config))
        or type(value["query_count"]) is not int
        or value["query_count"] != len(expected_inputs.queries)
    ):
        _fail("result authority")

    baseline_config = expected_config.baseline()
    hierarchy = build_hierarchy(expected_inputs, baseline_config)
    if value["hierarchy"] != _normalized(asdict(_hierarchy_evidence(hierarchy))):
        _fail("hierarchy authority")
    baseline_fences = tuple(
        route_hierarchy(query, hierarchy, baseline_config)
        for query in expected_inputs.queries
    )
    directory = {
        (key.object_role, key.ordinal): (page.offset, page.encoded_bytes)
        for key, page in expected_inputs.pages.items()
    }
    arms_value = value["arms"]
    if type(arms_value) is not list or len(arms_value) != len(expected_config.arms):
        _fail("arm cardinality")

    reduced: list[V104RescoreArm] = []
    for arm_value, registered in zip(
        arms_value, expected_config.arms, strict=True
    ):
        if type(arm_value) is not dict or set(arm_value) != {
            "retained_pages",
            "maximum_scanned_rows",
            "maximum_observed_scanned_rows",
            "aggregate",
            "samples",
        }:
            _fail("arm schema")
        if (
            arm_value["retained_pages"] != registered.retained_pages
            or arm_value["maximum_scanned_rows"] != registered.maximum_scanned_rows
            or type(arm_value["samples"]) is not list
            or len(arm_value["samples"]) != len(expected_inputs.queries)
        ):
            _fail("arm authority")
        cap = SimpleNamespace(
            maximum_root_groups=expected_config.maximum_root_groups,
            maximum_exposed_pages=expected_config.maximum_exposed_pages,
            maximum_scanned_rows=registered.maximum_scanned_rows,
            maximum_gets=expected_config.maximum_gets,
            maximum_bytes=expected_config.maximum_bytes,
        )
        samples = []
        for ordinal, (sample_value, baseline_fence) in enumerate(
            zip(arm_value["samples"], baseline_fences, strict=True)
        ):
            fence = _arm_fence(baseline_fence, expected_inputs, registered)
            truth = tuple(int(row_id) for row_id in expected_inputs.truth_ids[ordinal])
            truth_pages = tuple(
                (
                    expected_inputs.page_by_id[row_id].object_role,
                    expected_inputs.page_by_id[row_id].ordinal,
                )
                for row_id in truth
            )
            retained = tuple(
                (page.object_role, page.ordinal) for page in fence.retained_pages
            )
            try:
                sample = _range_sample(
                    sample_value,
                    query_ordinal=ordinal,
                    config=cap,
                    directory=directory,
                    expected_truth=(truth, truth_pages),
                    expected_fence=(
                        fence.root_evaluations,
                        fence.page_evaluations,
                        fence.scanned_rows,
                    ),
                    expected_retained_pages=retained,
                )
            except ValueError as error:
                message = str(error).replace("V99", "V104", 1)
                raise ValueError(message) from error
            samples.append(sample)
        sample_tuple = tuple(samples)
        aggregate = _range_aggregate(sample_tuple, cap)
        maximum_observed = max(sample.scanned_rows for sample in sample_tuple)
        if (
            arm_value["aggregate"] != _normalized(asdict(aggregate))
            or arm_value["maximum_observed_scanned_rows"] != maximum_observed
        ):
            _fail("arm aggregate")
        reduced.append(
            V104RescoreArm(
                retained_pages=registered.retained_pages,
                maximum_scanned_rows=registered.maximum_scanned_rows,
                maximum_observed_scanned_rows=maximum_observed,
                aggregate=aggregate,
            )
        )

    passing = [
        arm.retained_pages
        for arm in reduced
        if arm.aggregate.quality_gate_passed and arm.aggregate.resource_gate_passed
    ]
    winner = min(passing) if passing else None
    classification = (
        "exact-retention-survivor" if winner is not None else "exact-retention-rejected"
    )
    if (
        value["classification"] != classification
        or value["winner_retained_pages"] != winner
        or tuple(arm.retained_pages for arm in reduced)
        != REGISTERED_RETENTION_PAGES
    ):
        _fail("decision")
    return V104Rescore(
        schema="borsuk-v104-exact-retention-ladder-rescore-v1",
        result_sha256=expected_result_sha256,
        query_count=len(expected_inputs.queries),
        classification=classification,
        arms=tuple(reduced),
        winner_retained_pages=winner,
        status="verified",
    )


def canonical_v104_rescore_bytes(summary: V104Rescore) -> bytes:
    """Serialize one typed verified reducer receipt."""

    if (
        not isinstance(summary, V104Rescore)
        or summary.schema != "borsuk-v104-exact-retention-ladder-rescore-v1"
        or summary.status != "verified"
        or len(summary.result_sha256) != 64
    ):
        _fail("rescore summary")
    return _canonical(asdict(summary))
