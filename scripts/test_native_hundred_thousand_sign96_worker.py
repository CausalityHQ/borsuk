"""Spot user-data contract for the truth-separated sign96 gate."""
...
class Sign96WorkerTest(unittest.TestCase):
    def test_user_data_is_parseable_bounded_and_truth_separated(self) -> None:
        ...
        plan = SpotLayoutPlan(
            ...
            output_prefix=(
                "s3://bucket/research/native-sign96/"
        ...
        self.assertLess(
            script.index("-m scripts.native_hundred_thousand_sign96_cell construct"),
            script.index("aws s3 cp " + FROZEN_QUERIES.uri),
        )
        self.assertLess(
            script.index("-m scripts.native_hundred_thousand_sign96_cell plan"),
