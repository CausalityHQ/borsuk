...
case "${1:-}" in
  --describe)
    printf '%s\n' '{"baseline_rerun":false,"blas_threads":16,"instance_type":"c7i.8xlarge","max_wall_seconds":1200,"page_budget":32,"queries":1000,"query_parallelism":1,"rotation":"srht","rotation_seed_xor":1397901396,"shortlist_rows":2048,"split":"burned-development","spot_only":true}'
    exit 0
    ;;
  "") ;;
...
publish_terminal() {
  ...
  for name in install.log hashes.log srht.time srht.json summary.json; do
...
/usr/bin/time -v -o srht.time timeout 900 .venv/bin/python \
  ...
  --base base-000.arrow --delta delta-000.arrow --output srht.json \
  --dimensions 768 --neighbors 100 --queries-count 1000 --shortlist-rows 2048 \
  --gap-pages 0 --seed 7216 --rotation srht >/dev/null || exit 98

phase=validate
.venv/bin/python - <<'PY' >summary.json || exit 99
import hashlib
import json
from pathlib import Path
from scripts.v85_pq16_rescore_summary import summarize_paired_pq16_rotation

baseline_body = Path("baseline.json").read_bytes()
challenger_body = Path("srht.json").read_bytes()
