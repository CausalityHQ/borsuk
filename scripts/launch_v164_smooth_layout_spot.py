#!/usr/bin/env python3
"""Launch one frozen V164 1M transfer attempt on Causality Spot."""
...
SCHEMA = "borsuk-v164-smooth-layout-spot-v1"
TAG = "borsuk-v164-smooth-layout"
WALL_SECONDS = 10_800
...
def user_data(commit: str, archive_sha: str, archive_key: str, prefix: str) -> str:
    return f'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v164-hard-stop --on-active={WALL_SECONDS}s /usr/sbin/shutdown -h now
root=/mnt/v164-smooth-layout
...
/usr/bin/time -v -o score-resources.txt .venv/bin/python \\
  repo/scripts/v164_smooth_layout_1m.py score \\
  --source source.parquet --old-layout old-layout.npy --old-sq8 old-sq8.bin \\
...
/usr/bin/time -v -o reduce-resources.txt .venv/bin/python \\
  repo/scripts/v164_smooth_layout_1m.py reduce \\
...
/usr/bin/time -v -o check-resources.txt .venv/bin/python \\
  repo/scripts/v164_check_smooth_layout.py \\
  --source source.parquet --old-layout old-layout.npy --old-sq8 old-sq8.bin \\
  --manifest manifest.json --membership order.npy \\
...
def main() -> None:
    ...
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as tar:
        required = {"scripts/v164_smooth_layout_1m.py",
                    "scripts/v164_check_smooth_layout.py",
                    "docs/research/v164-smooth-layout-1m-transfer-prereg.md"}
        if not required.issubset(tar.getnames()):
            raise ValueError("source archive lacks V164 gate")
    archive_sha = hashlib.sha256(archive).hexdigest()
    archive_key = f"research/v164-smooth-layout/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v164-smooth-layout/{commit}/runs/{attempt}"
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V164 worker already active")
    ...
    receipt = ec2.run_instances(
        ClientToken="v164-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
        ...
        if not missing(s3, prefix + "/terminal.json"):
            ...
            for name, identity in terminal.get("artifacts", {}).items():
                ...
                if size != identity["bytes"] or digest.hexdigest() != identity["sha256"]:
                    raise ValueError(f"V164 S3 artifact read-back differs: {name}")
            terminal["terminal_sha256"] = hashlib.sha256(raw).hexdigest()
            print(json.dumps(terminal, sort_keys=True), flush=True)
            if terminal.get("status") != "complete":
                raise RuntimeError("V164 terminal reports failed; see closed artifacts")
            return
        state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
        if state in {"terminated", "shutting-down"}:
            if time.monotonic() - started >= WALL_SECONDS - 60:
                raise TimeoutError("V164 worker reached wall cap before terminal")
            raise RuntimeError("Spot worker stopped before terminal; restart full cell")
        time.sleep(20)
    ec2.terminate_instances(InstanceIds=[instance_id])
    raise TimeoutError("V164 cell exceeded wall cap")
