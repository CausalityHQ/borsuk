...
from scripts import v234_score_blocked_delta_100k as scorer


class BlockedDeltaScoreTest(unittest.TestCase):
    def test_exact_ids_and_loaded_speed_gate(self):
        ids = list(range(100))
        with tempfile.TemporaryDirectory() as folder:
            paths = {name: Path(folder) / name for name in
                     ("control_serving", "blocked_serving", "control_raw", "blocked_raw",
                      "control_loaded_raw", "blocked_loaded_raw", "baseline", "truth", "output")}
            paths["truth"].write_bytes(b"fixture")
            paths["baseline"].write_text("".join(json.dumps({
                "ordinal": i, "arms": {"2048-2048": {"returned_ids": ids}}}) + "\n"
                for i in range(1000)))
            for mode, latency in (("control", 20), ("blocked", 10)):
                ...
                paths[f"{mode}_serving"].write_text(json.dumps({
                    "schema": ("borsuk-v232-decoded-delta-100k-serving-v1" if mode == "control"
                               else "borsuk-v234-blocked-delta-100k-serving-v1"),
                    "mode": "decoded-10k" if mode == "control" else "blocked-10k",
            ...
            with (patch.object(scorer, "BASE_SHA", scorer.sha256(paths["baseline"])),
                ...
                bad = [json.loads(line) for line in paths["blocked_raw"].read_text().splitlines()]
                bad[0]["returned_ids"][-1] = 100
                paths["blocked_raw"].write_text("".join(json.dumps(row) + "\n" for row in bad))
                serving = json.loads(paths["blocked_serving"].read_text())
                serving["raw_sha256"] = scorer.sha256(paths["blocked_raw"])
                paths["blocked_serving"].write_text(json.dumps(serving))
                scorer.run(args)
                result = json.loads(paths["output"].read_text())
                self.assertEqual(result["exact_control_id_lists"], 999)
