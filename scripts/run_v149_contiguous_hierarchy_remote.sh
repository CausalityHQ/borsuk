#!/bin/bash
# One frozen GT-free V149 cost/correctness screen on Causality Spot.
set -euo pipefail
...
deadline_epoch=$((started_epoch + V149_WALL_SECONDS - 900))
monitor_pid=
imds_token=
imds() {
...
finish() {
  ...
  if [ -d /sys/fs/cgroup/v149-science ]; then
    { cat /sys/fs/cgroup/v149-science/memory.current;
      cat /sys/fs/cgroup/v149-science/memory.peak;
      cat /sys/fs/cgroup/v149-science/memory.stat; } >science.cgroup-memory.txt
  fi
  upload_failed=0
  for path in install.log core-check.log core-check-resources.txt \
      ...
      science.summary.json science.tree.bin science.cgroup-memory.txt \
      decision.json worker.log interrupt-stop.txt; do
    if [ -f "$path" ] && ! timeout 300 aws s3 cp "$path" \
        "$V149_OUTPUT_PREFIX/artifacts/$path" --only-show-errors; then
      upload_failed=1
    fi
  done
...
PY
  aws s3 cp terminal.json "$V149_OUTPUT_PREFIX/terminal.json" --only-show-errors
  shutdown -h now || true
  exit "$code"
}
trap finish EXIT
...
run_science gate-build "$CARGO_HOME/bin/cargo" build --release --locked -j 8 \
  --manifest-path repo/scripts/v114_score_rust/Cargo.toml --bin v149_contiguous_hierarchy
phase=download
: >download.log
...
PYINNER
phase=science
mkdir /sys/fs/cgroup/v149-science
sync
echo 3 >/proc/sys/vm/drop_caches
run_science science bash -c \
  'echo "$BASHPID" >/sys/fs/cgroup/v149-science/cgroup.procs; exec "$@"' \
  v149 "$CARGO_TARGET_DIR/release/v149_contiguous_hierarchy" \
  deep/queries.jsonl deep/primary.jsonl deep/centroids.bin \
  deep/flat-scores.bin deep/v140.raw.jsonl science.tree.bin \
  science.summary.json 100000
cp science.log science.jsonl
python3 - <<'PYINNER'
import json
value=json.load(open('science.summary.json'))
if value.get('schema')!='borsuk-v149-contiguous-hierarchy-v1':
    raise ValueError('V149 scientific summary differs')
with open('decision.json','w') as output:
    json.dump({'schema':'borsuk-v149-decision-v1','verdict':value['verdict']},output,sort_keys=True)
    output.write('\n')
PYINNER
phase=complete
exit 0
