...
output_prefix=$V106_OUTPUT_PREFIX
...
finish() {
  ...
  STARTED_EPOCH="$started_epoch" FINISHED_EPOCH="$finished_epoch" \
    ...
    SWAP_END_KIB="$swap_end_kib" SPOT_PRICE_MICROS="$V106_SPOT_PRICE_MICROS" \
    python3.12 - <<'PY'
import json, os
...
if status == "complete":
    for role, filename in (
        ...
        evidence[role] = Identity(
            uri=f'{os.environ["V106_OUTPUT_PREFIX"]}/evidence/{filename}',
...
terminal = Terminal(
    ...
    source_commit=os.environ["V106_SOURCE_COMMIT"],
    critique_result_sha256=os.environ["V106_CRITIQUE_SHA256"],
    instance_id=os.environ["INSTANCE_ID"],
    exit_code=int(os.environ["EXIT_CODE"]),
    spot_price_usd_per_hour_micros=int(os.environ["V106_SPOT_PRICE_MICROS"]),
    evidence=evidence,
)
...
phase=source
[ -f source.tar.gz ] || aws s3 cp "$V106_SOURCE_ARCHIVE_URI" source.tar.gz --only-show-errors || exit 92
[ "$(stat -c%s source.tar.gz)" = "$V106_SOURCE_ARCHIVE_BYTES" ] || exit 93
...
for role in source queries truth generation base delta; do
  upper=${role^^}
  uri_name=V106_${upper}_URI
  sha_name=V106_${upper}_SHA256
  bytes_name=V106_${upper}_BYTES
  case "$role" in
...
from scripts.v106_anisotropic_pq48 import PQ48X8, plan_refinement_fetch
...
for ordinal in range(1_024):
    ...
    pages[key] = RoutedPage(key, offset, 256 * 52)
    offset += 256 * 52
...
maximum = float(os.environ["V106_PREFLIGHT_MAX_SECONDS"])
passed = (
...
phase=producer
/usr/bin/time -v -o producer.time timeout "$V106_WALL_SECONDS" .venv/bin/python - <<'PY' >producer.log 2>&1 || exit 97
...
identities = {
    role: ObjectIdentity(
        uri=os.environ[f"V106_{role.upper()}_URI"],
        sha256=os.environ[f"V106_{role.upper()}_SHA256"],
        bytes=int(os.environ[f"V106_{role.upper()}_BYTES"]),
    )
    for role in roles
}
registered_inputs = load_screen_inputs(
    **{role: Path(names[role]) for role in roles}, identities=identities,
    dimensions=768, neighbors=100, query_count=int(os.environ["V106_REGISTERED_QUERY_COUNT"]), seed=7216,
)
query_count = int(os.environ["V106_QUERY_COUNT"])
...
config = RankedGapConfig(
    pages_per_root=int(os.environ["V106_PAGES_PER_ROOT"]),
    maximum_root_groups=int(os.environ["V106_MAXIMUM_ROOT_GROUPS"]),
    maximum_exposed_pages=int(os.environ["V106_MAXIMUM_EXPOSED_PAGES"]),
    retained_pages=int(os.environ["V106_RETAINED_PAGES"]),
    maximum_scanned_rows=int(os.environ["V106_MAXIMUM_SCANNED_ROWS"]),
    shortlist_rows=int(os.environ["V106_SHORTLIST_ROWS"]),
    maximum_gets=int(os.environ["V106_MAXIMUM_GETS"]),
    maximum_bytes=int(os.environ["V106_MAXIMUM_BYTES"]),
)
Path("result.json").write_bytes(canonical_v106_result_bytes(evaluate_v106(inputs, authority, config)))
PY
...
from scripts.v99_ranked_gap_range_router import RankedGapConfig
from scripts.v106_anisotropic_pq48_rescore import canonical_v106_rescore_bytes, rescore_v106_result
...
authority = ScreenAuthority(
    source_commit=os.environ["V106_SOURCE_COMMIT"], critique_result_sha256=os.environ["V106_CRITIQUE_SHA256"],
...
registered_inputs = load_screen_inputs(
    ...
    query_count=int(os.environ["V106_REGISTERED_QUERY_COUNT"]), seed=7216,
)
query_count = int(os.environ["V106_QUERY_COUNT"])
inputs = dataclasses.replace(
    registered_inputs,
...
config = RankedGapConfig(
    pages_per_root=int(os.environ["V106_PAGES_PER_ROOT"]), maximum_root_groups=int(os.environ["V106_MAXIMUM_ROOT_GROUPS"]),
    maximum_exposed_pages=int(os.environ["V106_MAXIMUM_EXPOSED_PAGES"]), retained_pages=int(os.environ["V106_RETAINED_PAGES"]),
    maximum_scanned_rows=int(os.environ["V106_MAXIMUM_SCANNED_ROWS"]), shortlist_rows=int(os.environ["V106_SHORTLIST_ROWS"]),
    maximum_gets=int(os.environ["V106_MAXIMUM_GETS"]), maximum_bytes=int(os.environ["V106_MAXIMUM_BYTES"]),
)
summary = rescore_v106_result(
...
Path("rescore.json").write_bytes(canonical_v106_rescore_bytes(summary))
PY
