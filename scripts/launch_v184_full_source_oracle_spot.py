#!/usr/bin/env python3
"""Launch one immutable V184 full-source physical resource oracle on Spot."""
...
SCHEMA = "borsuk-v184-full-source-oracle-spot-v1"
TAG = "borsuk-v184-full-source-oracle"
WALL_SECONDS = 14_400
...
ARTIFACTS = (
    "out/full-truth-units.jsonl", "out/frontiers.npy",
    "out/witness-plans.jsonl", "out/summary.json",
...
def user_data(commit: str, archive_sha: str, archive_key: str, prefix: str) -> str:
    return f'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v184-hard-stop --on-active={WALL_SECONDS}s /usr/sbin/shutdown -h now
root=/mnt/v184-full-source-oracle
...
/usr/bin/time -v -o oracle-resources.txt .venv/bin/python -m \
  scripts.v184_full_source_oracle --output out --v182 v182 \
...
def _launch() -> None:
    ...
    if subprocess.run(["git", "merge-base", "--is-ancestor", "HEAD",
        ...
        raise ValueError("V184 source commit is not pushed to origin/main")
    archive = archive_source(commit)
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as tar:
        required = {"scripts/sparse_cover_frontier.py",
                    "scripts/global_source_oracle.py",
                    "scripts/v184_full_source_oracle.py",
                    "docs/research/v184-full-source-oracle-prereg.md"}
        if not required.issubset(tar.getnames()):
            raise ValueError("source archive lacks V184 gate")
    archive_sha = hashlib.sha256(archive).hexdigest()
    archive_key = f"research/v184-full-source-oracle/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v184-full-source-oracle/{commit}/runs/{attempt}"
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V184 worker already active")
    ...
    receipt = ec2.run_instances(
        ClientToken="v184-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
    ...
    while time.monotonic() < deadline:
        if not missing(s3, prefix + "/terminal.json"):
            ...
            for name, identity in terminal.get("artifacts", {}).items():
                ...
                if size != identity["bytes"] or digest.hexdigest() != identity["sha256"]:
                    raise ValueError(f"V184 S3 artifact read-back differs: {name}")
            terminal["terminal_sha256"] = hashlib.sha256(raw).hexdigest()
            print(json.dumps(terminal, sort_keys=True), flush=True)
            if terminal.get("status") != "complete":
                raise RuntimeError("V184 terminal reports failed; see closed artifacts")
            return
        ...
        if state in {"terminated", "shutting-down"}:
            ...
            if time.monotonic() - started >= WALL_SECONDS - 60:
                raise TimeoutError("V184 worker reached wall cap before terminal")
            raise RuntimeError("Spot worker stopped before terminal; restart full cell")
        time.sleep(20)
    ec2.terminate_instances(InstanceIds=[instance_id])
    raise TimeoutError("V184 cell exceeded wall cap")


def main() -> None:
    # All V184 controllers on this devbox share one process-lifetime lock.
    # The EC2 tag check below also rejects an already running worker.
    with open("/tmp/borsuk-v184-full-source-oracle-launch.lock", "a+") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as exc:
            raise RuntimeError("V184 launcher is already running here") from exc
        _launch()
