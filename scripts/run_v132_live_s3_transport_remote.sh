#!/bin/bash
# One frozen V132 transport replay on a Causality Spot worker.
set -euo pipefail
...
deadline_epoch=$((started_epoch + V132_WALL_SECONDS))
monitor_pid=
imds_token=
imds() {
...
finish() {
  ...
  for path in install.log compile.log compile-resources.txt \
    ...
    if [ -f "$path" ] && ! timeout 900 aws s3 cp "$path" \
        "$V132_OUTPUT_PREFIX/artifacts/$path" --only-show-errors; then
      upload_failed=1
    fi
  done
...
PY
  aws s3 cp terminal.json "$V132_OUTPUT_PREFIX/terminal.json" --only-show-errors
  shutdown -h now || true
  exit "$code"
}
trap finish EXIT
...
run_science compile "$CARGO_HOME/bin/cargo" build --release --locked -j 8 \
  --manifest-path repo/crates/borsuk/Cargo.toml --example v132_live_s3_transport
phase=prepare
run_science prepare python3 repo/scripts/prepare_v132_live_inputs.py prepared
phase=evaluate
mkdir /sys/fs/cgroup/v132-serving
sync
echo 3 >/proc/sys/vm/drop_caches
run_science evaluate bash -c \
  'echo "$BASHPID" >/sys/fs/cgroup/v132-serving/cgroup.procs; exec "$@"' \
  v132 "$CARGO_TARGET_DIR/release/examples/v132_live_s3_transport" \
  prepared replay.jsonl summary.json
{
  cat /sys/fs/cgroup/v132-serving/memory.current
  cat /sys/fs/cgroup/v132-serving/memory.peak
  cat /sys/fs/cgroup/v132-serving/memory.stat
} >cgroup-memory.txt
phase=complete
exit 0
