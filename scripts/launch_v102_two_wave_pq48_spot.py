#!/usr/bin/env python3
"""Launch one immutable V102 two-wave-pq48 experiment on Causality Spot."""
...
class SpotTarget:
    ...
    def __post_init__(self) -> None:
        if not self.availability_zone.startswith(
            ...
            raise ValueError("V102 Spot target differs")
...
def build_plan(**values: object) -> V102SpotPlan:
    """Freeze and validate one claim-eligible V102 Spot plan."""

    try:
        plan = V102SpotPlan(**values)
    except (AttributeError, TypeError) as error:
        raise ValueError("V102 Spot plan differs") from error
    ...
    if (
        ...
        raise ValueError("V102 Spot plan differs")
    return plan
...
def worker_script(plan: V102SpotPlan) -> str:
    """Bind the exact plan to a size-bounded authenticated bootstrap."""

    exports = {
        "V102_ATTEMPT": plan.attempt,
        "V102_BOOTSTRAP_RESAMPLES": plan.bootstrap_resamples,
        "V102_CRITIQUE_SHA256": plan.critique_result_sha256,
        "V102_MAXIMUM_BYTES": plan.config.maximum_bytes,
        "V102_MAXIMUM_EXPOSED_PAGES": plan.config.maximum_exposed_pages,
        "V102_MAXIMUM_GETS": plan.config.maximum_gets,
        "V102_MAXIMUM_ROOT_GROUPS": plan.config.maximum_root_groups,
        "V102_MAXIMUM_SCANNED_ROWS": plan.config.maximum_scanned_rows,
        "V102_OUTPUT_PREFIX": plan.output_prefix.rstrip("/"),
        "V102_PAGES_PER_ROOT": plan.config.pages_per_root,
        "V102_PREFLIGHT_MAX_SECONDS": 5,
        "V102_QUERY_COUNT": plan.query_count,
        "V102_REGISTERED_QUERY_COUNT": 1_000,
        "V102_RETAINED_PAGES": plan.config.retained_pages,
        "V102_SHORTLIST_ROWS": plan.config.shortlist_rows,
        "V102_SOURCE_ARCHIVE_BYTES": plan.source_archive.bytes,
        "V102_SOURCE_ARCHIVE_SHA256": plan.source_archive.sha256,
        "V102_SOURCE_ARCHIVE_URI": plan.source_archive.uri,
        "V102_SOURCE_COMMIT": plan.source_commit,
        "V102_SPOT_PRICE_MICROS": plan.spot_price_usd_per_hour_micros,
        "V102_WALL_SECONDS": plan.wall_seconds,
    }
    for role, identity in sorted(plan.inputs.items()):
        prefix = f"V102_{role.upper()}"
...
aws s3 cp "$V102_SOURCE_ARCHIVE_URI" source.tar.gz --only-show-errors
[ "$(stat -c%s source.tar.gz)" = "$V102_SOURCE_ARCHIVE_BYTES" ]
printf '%s  source.tar.gz\n' "$V102_SOURCE_ARCHIVE_SHA256" >hashes.txt
sha256sum -c hashes.txt
mkdir repo
tar -xzf source.tar.gz -C repo
exec bash repo/scripts/v102_two_wave_pq48_run_remote.sh
"""
    script = f"#!/bin/bash\n{header}\n{bootstrap}"
    if len(script.encode()) > 16_384:
        raise ValueError("V102 user data exceeds EC2 limit")
    return script


def build_launch_specs(plan: V102SpotPlan) -> list[dict[str, object]]:
    ...
    for target in plan.targets:
        token = hashlib.sha256(
            f"v102:{plan.source_commit}:{target.availability_zone}:a0001".encode()
        ).hexdigest()[:48]
        specs.append(
            {
                "BlockDeviceMappings": [
...
def launch_one_spot(plan: V102SpotPlan, *, ec2_client: object) -> str:
    ...
    for request in build_launch_specs(plan):
        ...
        if (
            ...
            raise RuntimeError("V102 Spot launch response differs")
        return instances[0]["InstanceId"]
    raise RuntimeError(f"V102 Spot capacity unavailable in {failures} zones")
...
def _launch_receipt(plan: V102SpotPlan, instance_id: str | None) -> bytes:
    value = {
...
def claim_reserved_attempt(plan: V102SpotPlan, *, s3_client: object) -> None:
...
def claim_launched_attempt(
    plan: V102SpotPlan, *, s3_client: object, instance_id: str
) -> None:
    """Persist the exact launched instance under a create-only key."""

    if not instance_id.startswith("i-"):
        raise ValueError("V102 instance identity differs")
    bucket, prefix = _s3_location(plan.output_prefix)
    _atomic_put(
        s3_client,
        bucket=bucket,
...
def canonical_terminal_bytes(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("V102 terminal differs")
...
def validate_terminal_bytes(
    body: bytes, plan: V102SpotPlan, instance_id: str
) -> AttemptTerminal:
    ...
    try:
        ...
        raise ValueError("V102 terminal JSON differs") from error
    if (
        ...
        + b"\n"
        != body
        or value.get("schema") != "borsuk-v102-spot-terminal-v1"
        or type(value.get("evidence")) is not dict
    ):
        raise ValueError("V102 terminal canonical bytes differ")
    try:
        evidence = {
        ...
        raise ValueError("V102 terminal authority differs") from error
    if expected != body:
        raise ValueError("V102 terminal authority differs")
        ...
        while True:
            try:
                ...
                if monotonic() >= deadline:  # type: ignore[operator]
                    raise TimeoutError(
                        "V102 attempt exceeded terminal deadline"
                    ) from error
                sleep(poll_seconds)  # type: ignore[operator]
                continue
            body = response["Body"].read()
            if response.get("ContentLength") != len(body):
                raise ValueError("V102 terminal object length differs")
            return validate_terminal_bytes(body, plan, instance_id)
    finally:
        ec2_client.terminate_instances(InstanceIds=[instance_id])


def ensure_unstarted(plan: V102SpotPlan, *, s3_client: object) -> None:
    ...
    for name in ("reservation.json", "launch.json", "terminal.json"):
        try:
            ...
            if code in ("404", "NoSuchKey", "NotFound") or any(
                ...
                continue
            raise
        raise ValueError("V102 immutable attempt already exists")


def parse_args(argv: Sequence[str] | None = None) -> V102SpotPlan:
