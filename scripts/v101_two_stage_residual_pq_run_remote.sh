...
root=/mnt/v101-two-stage-residual-pq-g1
phase=bootstrap
interrupted=0
watcher_pid=
started_epoch=$(date +%s)
output_prefix=$V101_OUTPUT_PREFIX
...
finish() {
  ...
  STARTED_EPOCH="$started_epoch" FINISHED_EPOCH="$finished_epoch" \
    ...
    SWAP_END_KIB="$swap_end_kib" SPOT_PRICE_MICROS="$V101_SPOT_PRICE_MICROS" \
    python3.12 - <<'PY'
import json, os
...
if status == "complete":
    for role, filename in (
        ...
        evidence[role] = Identity(
            uri=f'{os.environ["V101_OUTPUT_PREFIX"]}/evidence/{filename}',
            sha256=hashlib.sha256(body).hexdigest(),
            bytes=len(body),
        )
terminal = Terminal(
    schema="borsuk-v101-spot-terminal-v1",
    attempt=1,
    status=status,
    claim_eligible=status == "complete",
    source_commit=os.environ["V101_SOURCE_COMMIT"],
    critique_result_sha256=os.environ["V101_CRITIQUE_SHA256"],
    instance_id=os.environ["INSTANCE_ID"],
    exit_code=int(os.environ["EXIT_CODE"]),
    spot_price_usd_per_hour_micros=int(os.environ["V101_SPOT_PRICE_MICROS"]),
    evidence=evidence,
)
...
phase=source
[ -f source.tar.gz ] || aws s3 cp "$V101_SOURCE_ARCHIVE_URI" source.tar.gz --only-show-errors || exit 92
[ "$(stat -c%s source.tar.gz)" = "$V101_SOURCE_ARCHIVE_BYTES" ] || exit 93
printf '%s  source.tar.gz\n' "$V101_SOURCE_ARCHIVE_SHA256" >hashes.txt
...
for role in source queries truth generation base delta; do
  upper=${role^^}
  uri_name=V101_${upper}_URI
  sha_name=V101_${upper}_SHA256
  bytes_name=V101_${upper}_BYTES
  case "$role" in
...
from scripts.v101_two_stage_residual_pq_screen import TwoStageResidualPqArtifact, two_stage_adc_scores

@dataclass(frozen=True, slots=True)
class Preflight:
    rows: int
    shortlist_rows: int
    two_stage_seconds: float
...
first_books = rng.normal(size=(8, 256, 12)).astype(np.float32)
second_books = rng.normal(size=(8, 256, 12)).astype(np.float32)
first_codes = rng.integers(0, 256, size=(rows, 8), dtype=np.uint8)
second_codes = rng.integers(0, 256, size=(rows, 8), dtype=np.uint8)
artifact = TwoStageResidualPqArtifact.from_arrays(
    first_books=first_books, second_books=second_books,
    first_codes=first_codes, second_codes=second_codes,
)
started = time.perf_counter()
scores = two_stage_adc_scores(query, artifact)
two_stage_seconds = time.perf_counter() - started
pages = {}
offset = 0
...
maximum = float(os.environ["V101_PREFLIGHT_MAX_SECONDS"])
passed = (
    scores.shape == (rows,)
    and np.isfinite(scores).all()
    and selection.gets <= 32
    and selection.bytes <= 16 * 1024**2
    and max(two_stage_seconds, planner_seconds) <= maximum
)
receipt = Preflight(
    rows=rows,
    shortlist_rows=shortlist,
    two_stage_seconds=two_stage_seconds,
...
phase=producer
/usr/bin/time -v -o producer.time timeout "$V101_WALL_SECONDS" .venv/bin/python - <<'PY' >producer.log 2>&1 || exit 97
import dataclasses, os
from pathlib import Path
from scripts.v97_row_width_screen import ObjectIdentity, ScreenAuthority, load_screen_inputs, page_map_sha256
from scripts.v101_two_stage_residual_pq_screen import RankedGapConfig, canonical_v101_result_bytes, evaluate_v101
...
identities = {
    role: ObjectIdentity(
        uri=os.environ[f"V101_{role.upper()}_URI"],
        sha256=os.environ[f"V101_{role.upper()}_SHA256"],
        bytes=int(os.environ[f"V101_{role.upper()}_BYTES"]),
    )
    for role in roles
}
registered_inputs = load_screen_inputs(
    **{role: Path(names[role]) for role in roles}, identities=identities,
    dimensions=768, neighbors=100, query_count=int(os.environ["V101_REGISTERED_QUERY_COUNT"]), seed=7216,
)
query_count = int(os.environ["V101_QUERY_COUNT"])
...
config = RankedGapConfig(
    pages_per_root=int(os.environ["V101_PAGES_PER_ROOT"]),
    maximum_root_groups=int(os.environ["V101_MAXIMUM_ROOT_GROUPS"]),
    maximum_exposed_pages=int(os.environ["V101_MAXIMUM_EXPOSED_PAGES"]),
    retained_pages=int(os.environ["V101_RETAINED_PAGES"]),
    maximum_scanned_rows=int(os.environ["V101_MAXIMUM_SCANNED_ROWS"]),
    shortlist_rows=int(os.environ["V101_SHORTLIST_ROWS"]),
    maximum_gets=int(os.environ["V101_MAXIMUM_GETS"]),
    maximum_bytes=int(os.environ["V101_MAXIMUM_BYTES"]),
)
Path("result.json").write_bytes(canonical_v101_result_bytes(evaluate_v101(inputs, authority, config)))
PY
...
from scripts.v101_two_stage_residual_pq_screen import RankedGapConfig
from scripts.v101_two_stage_residual_pq_rescore import canonical_v101_rescore_bytes, rescore_v101_result
...
authority = ScreenAuthority(
    source_commit=os.environ["V101_SOURCE_COMMIT"], critique_result_sha256=os.environ["V101_CRITIQUE_SHA256"],
    page_map_sha256=Path("page_map.sha256").read_text().strip(), dimensions=768, seed=7216, identities=identities,
)
config = RankedGapConfig(
    pages_per_root=int(os.environ["V101_PAGES_PER_ROOT"]), maximum_root_groups=int(os.environ["V101_MAXIMUM_ROOT_GROUPS"]),
    maximum_exposed_pages=int(os.environ["V101_MAXIMUM_EXPOSED_PAGES"]), retained_pages=int(os.environ["V101_RETAINED_PAGES"]),
    maximum_scanned_rows=int(os.environ["V101_MAXIMUM_SCANNED_ROWS"]), shortlist_rows=int(os.environ["V101_SHORTLIST_ROWS"]),
    maximum_gets=int(os.environ["V101_MAXIMUM_GETS"]), maximum_bytes=int(os.environ["V101_MAXIMUM_BYTES"]),
)
...
Path("rescore.json").write_bytes(canonical_v101_rescore_bytes(summary))
PY
