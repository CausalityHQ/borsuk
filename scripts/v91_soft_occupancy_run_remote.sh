...
if [ "${1:-}" = "--describe" ]; then
  printf '%s\n' '{"blas_threads":16,"max_wall_seconds":1200,"queries_per_arm":32,"query_parallelism":1,"rayon_work_stealing":false,"sketch_bytes_per_base_row":16,"spot_only":true,"total_arms":2,"worker_model":"fixed-16-thread-blas"}'
  exit 0
fi
...
publish_terminal() {
  ...
  if [ "$code" -eq 0 ]; then
    for name in dependency.log hashes.log hashes-check.log instance.json run.log time.log result.json; do
      if [ ! -s "$name" ] || ! aws s3 cp "$name" "$V91_OUTPUT_URI/$name" --only-show-errors; then
        code=98
    ...
    for name in dependency.log hashes.log hashes-check.log instance.json run.log time.log result.json; do
      if [ -n "${V91_OUTPUT_URI:-}" ] && [ -f "$name" ]; then
        aws s3 cp "$name" "$V91_OUTPUT_URI/$name" --only-show-errors || true
      fi
    done
  fi
  printf '{"exit_code":%d,"phase":"%s","source_commit":"%s"}\n' \
    "$code" "$phase" "${V91_SOURCE_COMMIT:-unknown}" >/tmp/v91-soft-occupancy-terminal.json
  if [ -n "${V91_OUTPUT_URI:-}" ] && ! aws s3 cp /tmp/v91-soft-occupancy-terminal.json \
    "$V91_OUTPUT_URI/terminal.json" --only-show-errors; then
    code=99
  fi
  unlink /tmp/v91-soft-occupancy-terminal.json
  sync
  shutdown -h now || true
  exit "$code"
}
...
for variable in \
  V91_SCREEN_URI V91_SCREEN_SHA256 V91_V90_URI V91_V90_SHA256 \
  V91_V88_URI V91_V88_SHA256 V91_V87_URI V91_V87_SHA256 \
  V91_V86_URI V91_V86_SHA256 V91_SHARED_URI V91_SHARED_SHA256 \
  V91_OUTPUT_URI V91_SOURCE_COMMIT; do
  eval "value=\${$variable:-}"
  [ -n "$value" ] || exit 86
done
...
phase=inputs
aws s3 cp "$V91_SCREEN_URI" scripts/v91_soft_occupancy_screen.py --only-show-errors || exit 92
aws s3 cp "$V91_V90_URI" scripts/v90_residual_row_sketch_screen.py --only-show-errors || exit 92
aws s3 cp "$V91_V88_URI" scripts/v88_planner_objective_screen.py --only-show-errors || exit 92
aws s3 cp "$V91_V87_URI" scripts/v87_summary_capacity_screen.py --only-show-errors || exit 92
aws s3 cp "$V91_V86_URI" scripts/v86_coarse_to_fine_screen.py --only-show-errors || exit 92
aws s3 cp "$V91_SHARED_URI" scripts/v85_shared_overlay_screen.py --only-show-errors || exit 92
printf '%s  scripts/v91_soft_occupancy_screen.py\n%s  scripts/v90_residual_row_sketch_screen.py\n%s  scripts/v88_planner_objective_screen.py\n%s  scripts/v87_summary_capacity_screen.py\n%s  scripts/v86_coarse_to_fine_screen.py\n%s  scripts/v85_shared_overlay_screen.py\n' \
  "$V91_SCREEN_SHA256" "$V91_V90_SHA256" "$V91_V88_SHA256" \
  "$V91_V87_SHA256" "$V91_V86_SHA256" "$V91_SHARED_SHA256" >hashes.log
...
/usr/bin/time -v -o time.log timeout --signal=TERM --kill-after=30s 1200s \
  .venv/bin/python -m scripts.v91_soft_occupancy_screen \
