...
SCHEMA = "borsuk-v205-dual-certificate-spot-v1"
TAG = "borsuk-v205-dual-certificate"
...
def user_data(commit: str, archive_sha: str, archive_key: str,
    ...
    template = r'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v205-hard-stop --on-active=@@WALL@@s /usr/sbin/shutdown -h now
root=/mnt/v205-dual-certificate
...
phase=build
cd repo
"$CARGO_HOME/bin/cargo" build --release --locked -p borsuk --bin v205_dual_certificate_preflight --jobs 6 >"$root/build.log" 2>&1
phase=unit_tests
"$CARGO_HOME/bin/cargo" test --release --locked -p borsuk --lib relaxed_priced_interval::tests --jobs 6 >"$root/test.log" 2>&1
cd "$root"
phase=bench
/usr/bin/time -v -o bench-resources.txt "$CARGO_TARGET_DIR/release/v205_dual_certificate_preflight" \
...
'''
    ...
    if "@@" in template:
        raise ValueError("unresolved V205 worker placeholder")
    return template
...
def launch(attempt: str) -> None:
    ...
    if subprocess.run(["git", "merge-base", "--is-ancestor", "HEAD", "origin/main"],
        ...
        raise ValueError("V205 source commit is not pushed to origin/main")
    archive = archive_source(commit)
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as tar:
        required = {"crates/borsuk/src/relaxed_priced_interval.rs",
                    "crates/borsuk/src/bin/v205_dual_certificate_preflight.rs",
                    "docs/research/v205-dual-certificate-prereg.md"}
        if not required.issubset(tar.getnames()):
            raise ValueError("V205 source archive lacks frozen gate")
    archive_sha = hashlib.sha256(archive).hexdigest()
    archive_key = f"research/v205-dual-certificate/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v205-dual-certificate/{commit}/runs/{attempt}"
    ...
    for _, key, size, _ in INPUTS:
        if s3.head_object(Bucket=BUCKET, Key=key)["ContentLength"] != size:
            raise ValueError("V205 frozen input length differs")
    ...
    receipt = ec2.run_instances(
        ClientToken="v205-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
        ...
        while time.monotonic() < deadline:
            try:
                ...
                if terminal.get("status") != "complete":
                    raise RuntimeError("V205 failed; inspect only closed terminal artifacts")
                if set(ARTIFACTS) != set(terminal.get("artifacts", {})):
                    raise ValueError("complete V205 artifact roster differs")
                for name, identity in terminal["artifacts"].items():
                    ...
                    if size != identity["bytes"] or digest.hexdigest() != identity["sha256"]:
                        raise ValueError(f"V205 S3 artifact read-back differs: {name}")
                ...
                if (bench.get("linear_exact") != 861
                        ...
                        or bench.get("certified_exact", -1) + bench.get("hard_exact", -1) != 51
                        or sum(bench.get(key, 0) for key in
                               ("linear_exact", "unit_exact", "get_exact", "certified_exact", "hard_exact")) != 1000):
                    raise ValueError("V205 hierarchy accounting differs")
            ...
            time.sleep(20)
        raise TimeoutError("V205 Spot cell exceeded wall cap")
    finally:
        try:
            state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
...
def main() -> None:
    ...
    with open("/tmp/borsuk-v205-dual-certificate-launch.lock","a+") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as error:
            raise RuntimeError("V205 launcher is already active") from error
        launch(args.attempt)
