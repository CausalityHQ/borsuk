#!/usr/bin/env python3
"""Launch and monitor one frozen V132 live S3 transport replay on Spot."""
...
TAG = "borsuk-v132-live-s3-transport"
...
def validate(plan: Plan) -> None:
    ...
    if (
        ...
        or not output_key.startswith(
            f"research/v132-live-s3/{plan.source_commit}/runs/"
        )
        ...
        raise ValueError("immutable V132 plan differs")
...
def user_data(plan: Plan) -> str:
    ...
    exports = "\n".join(
        (
            f"export V132_SOURCE_COMMIT={plan.source_commit}",
            f"export V132_ARCHIVE_SHA256={plan.archive_sha256}",
            f"export V132_OUTPUT_PREFIX={shlex.quote(plan.output_prefix)}",
            f"export V132_WALL_SECONDS={plan.wall_seconds}",
            f"export V132_GENERATION_SHA256={GENERATION_SHA}",
            f"export V132_GENERATION_TERMINAL_SHA256={GENERATION_TERMINAL_SHA}",
        )
    )
    return f"""#!/bin/bash
set -euo pipefail
root=/mnt/v132-live-s3
...
import json,os
print(json.dumps({{"schema":"borsuk-v132-live-s3-transport-spot-v1",
  "source_commit":os.environ["V132_SOURCE_COMMIT"],
  "source_archive_sha256":os.environ["V132_ARCHIVE_SHA256"],
  "generation_manifest_sha256":os.environ["V132_GENERATION_SHA256"],
  "generation_terminal_sha256":os.environ["V132_GENERATION_TERMINAL_SHA256"],
  "instance_id":os.environ["INSTANCE_ID"],"exit_code":int(os.environ["EXIT_CODE"]),
  "phase":"bootstrap","status":"failed","artifacts":{{}}}},
  sort_keys=True,separators=(',',':')))
PY
    aws s3 cp terminal.json "$V132_OUTPUT_PREFIX/terminal.json" --only-show-errors || true
    shutdown -h now || true
  fi
}}
...
trap - EXIT
exec bash repo/scripts/run_v132_live_s3_transport_remote.sh
"""


def launch_spec(plan: Plan) -> dict:
    token = "v132-" + hashlib.sha256(plan.output_prefix.encode()).hexdigest()[:48]
    return {
        "ClientToken": token,
...
def reserve(plan: Plan) -> None:
    receipt = json.dumps(
        {
            "schema": "borsuk-v132-live-s3-reservation-v1",
...
def launch_and_monitor(plan: Plan) -> dict:
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V132 worker already active")
    ...
    while time.monotonic() < deadline:
        if not missing(s3, prefix + "/terminal.json"):
            ...
            terminal = json.loads(raw)
            if (
                terminal.get("schema") != "borsuk-v132-live-s3-transport-spot-v1"
                ...
                raise ValueError("V132 terminal identity differs")
        ...
        if state == "terminated":
            raise RuntimeError("V132 instance terminated without terminal marker")
        time.sleep(15)
    ec2.terminate_instances(InstanceIds=[instance_id])
    ec2.get_waiter("instance_terminated").wait(InstanceIds=[instance_id])
    raise TimeoutError(
        f"V132 terminal missing after worker deadline: {instance_id} terminated"
    )
...
def main() -> None:
    ...
    if terminal.get("status") != "complete" or terminal.get("exit_code") != 0:
        raise RuntimeError("V132 terminal reports a failed attempt")
