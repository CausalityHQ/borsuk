#!/usr/bin/env python3
"""One immutable Causality Spot cell for composed V115 returned-quality parity."""
...
from scripts.validate_v115_returned_replay import (
    FROZEN_EVIDENCE_SHA256, FROZEN_REFERENCE_SHA256,
    FROZEN_ROSTERS_SHA256,
)
...
V115 = (f"s3://{BUCKET}/research/v115-source-router-parity/"
        ...
        "runs/v115-router-20260923T235000Z/a0001/artifacts")
SCHEMA = "borsuk-v115-returned-replay-spot-v1"
ARTIFACTS = {
    "SQ8": INPUTS["SQ8"],
    "TRUTH": INPUTS["TRUTH"],
    ...
    "REQUESTS": ObjectIdentity(V114 + "/requests-1000.jsonl",
        FROZEN_REQUESTS_SHA256, 18217189),
    "ROSTERS": ObjectIdentity(V115 + "/rust-nominees.jsonl",
        FROZEN_ROSTERS_SHA256, 3552632),
    "REFERENCE": ObjectIdentity(V114 + "/reference-1000.jsonl",
        FROZEN_REFERENCE_SHA256, 8535378),
    "EVIDENCE": ObjectIdentity(V114 + "/evidence-1000.jsonl",
        FROZEN_EVIDENCE_SHA256, 3179695),
}
...
def validate_plan(plan: Plan) -> None:
    valid_hex = lambda value, length: len(value) == length and all(
        char in "0123456789abcdef" for char in value)
    if (not valid_hex(plan.source_commit, 40)
            or not valid_hex(plan.archive_sha256, 64)
        ...
        raise ValueError("V115 returned replay immutable plan differs")
...
import json, os
print(json.dumps({{"schema":"{SCHEMA}","source_commit":os.environ["V115_SOURCE_COMMIT"],
...
trap - EXIT
exec bash repo/scripts/run_v115_returned_replay_remote.sh
"""


def launch_spec(plan: Plan, zone: str, subnet: str) -> dict[str, object]:
    token = "v115-replay-" + hashlib.sha256(
    ...
    return {
        ...
        "TagSpecifications": [{"ResourceType": "instance", "Tags": [
            {"Key": "Name", "Value": "borsuk-v115-returned-replay"},
...
def _reserve(s3: object, plan: Plan, bucket: str, prefix: str) -> None:
    if not _missing(s3, bucket, f"{prefix}/reservation.json") or not _missing(
        ...
        raise ValueError("V115 immutable replay attempt already registered")
    body = (json.dumps({"schema": SCHEMA, "source_commit": plan.source_commit,
                        "archive_sha256": plan.archive_sha256, "attempt": 1},
                       sort_keys=True, separators=(",", ":")) + "\n").encode()
    s3.put_object(Bucket=bucket, Key=f"{prefix}/reservation.json", Body=body,
...
def launch_and_monitor(plan: Plan) -> dict[str, object]:
    ...
    for target in DEFAULT_TARGETS:
        try:
            result = ec2.run_instances(**launch_spec(plan, target.availability_zone,
                                                     target.subnet_id))
    ...
    if instance_id is None:
        raise RuntimeError("V115 replay Spot capacity unavailable; reservation retained")
        ...
        while time.monotonic() < deadline:
            if not _missing(s3, bucket, f"{prefix}/terminal.json"):
                ...
                if (terminal.get("schema") != SCHEMA
                    ...
                    raise ValueError("V115 replay terminal identity differs")
                return terminal
            state = ec2.describe_instances(InstanceIds=[instance_id])[
                "Reservations"][0]["Instances"][0]["State"]["Name"]
            if state in {"stopped", "shutting-down", "terminated"}:
                raise RuntimeError(f"V115 replay Spot {instance_id} ended without terminal")
            time.sleep(20)
        raise TimeoutError(f"V115 replay Spot {instance_id} terminal deadline exceeded")
    finally:
