...
'''
  ...
  '20ff50e632cc575386b15d7fcd9c3842ef435388ed29ae8c30617158ee907dc5' map.u32 | sha256sum -c -'''
    for key, value in {"ARTIFACTS": " ".join(ARTIFACTS), "ARTIFACTS_PY": repr(ARTIFACTS),
                       "SCHEMA": EXACT_SCHEMA if exact_nav else DIVERSE_SCHEMA if diverse else PQ_SCHEMA if pq_topology else SCHEMA,
                       ...
                       "PQ_ARGS": "--diverse" if diverse or exact_nav else "--pq-topology books.bin codes.bin" if pq_topology else "",
                       "SERVE_ARGS": "--exact-nav" if exact_nav else "",
                       "EXACT_TEST": ('"$CARGO_HOME/bin/cargo" test --release --locked -p borsuk --lib '
                                      ...
                                      '-- --exact >>"$root/build.log" 2>&1' if exact_nav else ""),
                       "MATCHED_PREP": matched_prep if pq_topology or diverse or exact_nav else "",
                       "MATCHED_GRAPH": ("printf '%s  graph.bin\\n' '688941c7c61c89a39739909af14cb2b4a935a7a4967a168f9503ac34e170d0e7' | sha256sum -c -" if exact_nav else ""),
                       "MATCHED_REQUESTS": ("printf '%s  requests.jsonl\\n' '86d9406486a2bb27aa2e603f019e078dd3ecaed47f79ec685558ba3536433812' | sha256sum -c -" if pq_topology or diverse or exact_nav else ""),
                       "MATCHED_TRUTH": ("printf '%s  truth.u32\\n' '06cd59b31962d4190367b54d7abf24dd4e018d3c4ac8da0b2b528d21a5a7cbb8' | sha256sum -c -" if pq_topology or diverse or exact_nav else "")}.items():
...
def launch(attempt, pq_topology=False, diverse=False, exact_nav=False):
    if sum((pq_topology, diverse, exact_nav)) > 1:
    ...
    campaign = ("v251-cohere-fp16-navigation-100k" if exact_nav else
    ...
    if pq_topology or diverse or exact_nav:
    ...
    put_if_absent(prefix + "/reservation.json", json.dumps({
        ...
        "arms": [[512, 0], [1024, 0], [2048, 0]] if exact_nav else
                [[2048, 2048], [4096, 4096], [8192, 8192]],
        "pq_aligned_topology": pq_topology,
        "diverse_pruning": diverse or exact_nav,
        "exact_fp16_navigation": exact_nav,
        "paired_v248_terminal_sha256": PARENT_TERMINAL_SHA if pq_topology or diverse or exact_nav else None,
        "paired_v249_terminal_sha256": V249_TERMINAL_SHA if diverse or exact_nav else None,
        "paired_v250_terminal_sha256": V250_TERMINAL_SHA if exact_nav else None,
    ...
    receipt = ec2.run_instances(
        ClientToken=("v251-" if exact_nav else "v250-" if diverse else "v249-" if pq_topology else "v248-")
        ...
        UserData=base64.b64encode(worker(commit, archive_sha, archive_key, prefix,
                                          pq_topology, diverse, exact_nav).encode()).decode(),
    )
