...
class V23RaBitQSpotLauncherTests(unittest.TestCase):
    def test_cli_dispatches_exact_construction_plan(self) -> None:
        arguments = [
            "--run-id",
            "v23-rabitq-construction-fixture",
            "--source-commit",
            "1" * 40,
            "--source-archive-uri",
            "s3://fixture/source.tar.zst",
            "--source-archive-sha256",
            "2" * 64,
            "--source-archive-bytes",
            "8192",
            "--binary-uri",
            "s3://fixture/constructor",
            "--binary-sha256",
            "3" * 64,
            "--binary-bytes",
            "4096",
            "--manifest-uri",
            "s3://fixture/construction-manifest",
            "--manifest-sha256",
            "4" * 64,
            "--manifest-bytes",
            "2048",
            "--d2-report-uri",
            "s3://fixture/d2-report",
            "--d2-report-sha256",
            "5" * 64,
            "--d2-report-bytes",
            "1024",
            "--query-parquet-uri",
            "s3://fixture/query-parquet",
            ...
            "--output-prefix",
            "s3://fixture/terminal/construction/",
            "--execute-construction",
        ]
        sts = mock.Mock()
        sts.get_caller_identity.return_value = {
            "Account": launcher.EXPECTED_AWS_ACCOUNT
        }
        with (
            mock.patch.object(
                launcher, "_clients", return_value=(sts, mock.Mock(), mock.Mock())
            ),
            mock.patch.object(
                launcher, "run_spot", return_value="s3://fixture/terminal"
            ) as run,
    ...
    def test_construction_launch_is_phase_separated_and_opens_no_query_object(
        self,
    ) -> None:
    ...
    def test_idempotent_token_and_terminal_prefix_bind_every_immutable_input(
        self,
    ) -> None:
        values = dict(
            run_id="v23-rabitq-fixture",
    ...
    def test_instance_is_terminated_immediately_after_terminal_receipt(self) -> None:
        ...
        self.assertEqual(
            terminal, "s3://fixture/terminal/v23-rabitq-fixture/COMPLETE.json"
        )
    ...
    def test_worker_publishes_authenticated_log_before_terminal_marker(self) -> None:
        ...
        user_data = base64.b64decode(
            launcher.build_launch_spec(plan)["UserData"]
        ).decode()
        ...
        log_upload = user_data.index(
            "worker.log\" 's3://fixture/terminal/v23-rabitq-fixture/worker.log'"
        )
        failed_upload = user_data.index(
            "s3://fixture/terminal/v23-rabitq-fixture/FAILED.json"
        )
        complete_upload = user_data.index(
            "s3://fixture/terminal/v23-rabitq-fixture/COMPLETE.json"
        )
        self.assertLess(log_upload, failed_upload)
        self.assertLess(log_upload, complete_upload)

    def test_worker_provisions_pinned_python_before_direct_phase_execution(
        self,
    ) -> None:
        plan = launcher.build_launch_plan(
            run_id="v23-rabitq-fixture",
