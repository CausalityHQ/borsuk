#!/usr/bin/env python3
"""Independent closed-input recount for V164's 1M transfer decision."""
...
from scripts.v164_smooth_layout_1m import (
...
def check(source_path: Path, old_layout_path: Path, old_sq8_path: Path,
    ...
    if (sha256(requests_path) != REQUEST_SHA
        ...
        raise ValueError("V164 frozen identity differs")
    ...
    for start in range(0, ROWS, 16384):
        ...
        if not np.isfinite(norms).all() or (norms <= 0).any():
            raise ValueError("V164 checker source norm differs")
        geometry[start:stop] = vectors[start:stop] / norms[:, None]
    if not np.array_equal(order, _fit_order(geometry, cluster_count(ROWS))):
        raise ValueError("V164 source-only order reconstruction differs")
    del geometry
    ...
    for start in range(0, ROWS, 4096):
        ...
        if not np.array_equal(new[start:stop],
            ...
            raise ValueError("V164 relayout changed SQ8 row bytes")
    ...
    if (layout.get("schema") != SCHEMA + "-layout-seal"
        ...
        raise ValueError("V164 layout seal differs")
    ...
    if (plan_identity.get("schema") != SCHEMA + "-plan-seal"
        ...
        raise ValueError("V164 GT-blind plan/score seal differs")
    ...
    if len(positions) != ROWS or len(id_to_source) != ROWS:
        raise ValueError("V164 IDs are not unique")
    ...
    if low.shape != (DIMS,) or step.shape != (DIMS,):
        raise ValueError("V164 SQ8 quantizer geometry differs")
    ...
    for request, sealed, planned, scored, control, prior, raw in itertools.zip_longest(
        ...
        if any(row is None or row.get("query_ordinal") != count
            ...
            raise ValueError("V164 recount query order differs")
        ...
        if (planned["ranges"] != ranges or planned["bytes"] != amount
            ...
            raise ValueError(f"V164 plan differs at {count}")
        ...
        if (scored["sq8_top512_ids"] != returned
            ...
            raise ValueError(f"V164 SQ8 returned IDs differ at {count}")
        ...
        if (scored["source_top100_ids"] != exact.tolist()
            ...
            raise ValueError(f"V164 exact-source returned IDs differ at {count}")
        ...
        if not all(fetched(value) for value in returned):
            raise ValueError(f"V164 SQ8 result was not fetched at {count}")
        ...
        if any(raw.get(name) != value for name, value in expected.items()):
            raise ValueError(f"V164 raw recount differs at {count}")
        ...
        count += 1
    if count != QUERIES:
        raise ValueError("V164 recount query count differs")
    ...
    if (summary.get("schema") != SCHEMA + "-summary"
        ...
        raise ValueError("V164 summary recount differs")
