#!/usr/bin/env python3
"""Launch one immutable pq96/source 100k decision on Causality Spot."""
...
def build_plan(**values: object) -> SpotLayoutPlan:
    ...
    if type(attempt) is not int or not 1 <= attempt <= 99 or type(actual_prefix) is not str:
        raise ValueError("pq96 Spot attempt or output prefix differs")
    suffix = f"/runs/relaion-100k-dev1000-a{attempt:04d}"
    if not actual_prefix.rstrip("/").endswith(suffix):
        raise ValueError("pq96 Spot attempt suffix differs")
    ...
    expected = (
        "s3://borsuk-bench-453182569524-euc1/research/native-hundred-thousand-pq96/"
        + plan.source_commit
        + f"/runs/relaion-100k-dev1000-a{attempt:04d}"
    )
    if plan.output_prefix.rstrip("/") != expected:
        raise ValueError("pq96 immutable output prefix differs")
    return plan


def build_launch_specs(plan: SpotLayoutPlan) -> list[dict[str, object]]:
    specs = geometric_launch_specs(plan)
    user_data = pq96_worker_script(plan)
    for spec in specs:
        zone = spec["NetworkInterfaces"][0]["SubnetId"]
        token = hashlib.sha256(
            f"pq96:{plan.source_commit}:{zone}:a{plan.attempt:04d}".encode()
        ).hexdigest()[:32]
        spec["ClientToken"] = "native-pq96-" + token
        spec["UserData"] = user_data
        tags = spec["TagSpecifications"][0]["Tags"]
        tags[0]["Value"] = "borsuk-native-pq96-range"
...
def validate_terminal_bytes(
    ...
    try:
        ...
        raise ValueError("pq96 terminal JSON differs") from error
    ...
    if (
        ...
        or terminal["schema"] != "borsuk-hundred-thousand-pq96-terminal-v1"
        ...
        raise ValueError("pq96 terminal authority differs")
    ...
    if not complete:
        if (
            ...
            raise ValueError("pq96 failed terminal differs")
        return terminal
    if terminal["decision"] not in ("advance", "reject") or terminal["failure_reason"] != "":
        raise ValueError("pq96 terminal decision differs")
    artifacts = terminal["artifacts"]
    if type(artifacts) is not dict or set(artifacts) != set(ARTIFACT_FILES):
        raise ValueError("pq96 terminal artifact roster differs")
    for role, path in ARTIFACT_FILES.items():
        ...
        if (
            ...
            raise ValueError("pq96 terminal artifact identity differs")
    return terminal
...
def launch_and_monitor(plan: SpotLayoutPlan) -> dict[str, object]:
    ...
    if existing.get("KeyCount", 0) or existing.get("Contents"):
        raise ValueError("pq96 immutable attempt already exists")
    reservation = {
    ...
    if instance_id is None:
        raise RuntimeError("pq96 Spot capacity unavailable; no instance launched")
                ...
                if state in {"terminated", "shutting-down", "stopped"}:
                    ...
                    if state in {"terminated", "stopped"}:
                        try:
                            ...
                            raise RuntimeError(
                                f"pq96 instance {instance_id} ended without terminal"
                            ) from error
                        return validate_terminal_bytes(response["Body"].read(), plan, instance_id)
                if time.monotonic() >= deadline:
                    raise TimeoutError("pq96 terminal deadline exceeded") from error
                time.sleep(15)
                continue
            return validate_terminal_bytes(response["Body"].read(), plan, instance_id)
    finally:
