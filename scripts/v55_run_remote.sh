#!/bin/bash
set -u
root=/mnt/v55-cross-polytope-f8f497e3
output_uri=s3://borsuk-bench-453182569524-euc1/research/v55-algorithm-first/cross-polytope-f8f497e3dec3f55a/a0001
mkdir -p "$root" && cd "$root" || exit 90
prefix=s3://borsuk-bench-453182569524-euc1/research/v55-algorithm-first/cross-polytope-f8f497e3dec3f55a
aws s3 cp "$prefix/v55_algorithm_first_cross_polytope.py" probe.py --only-show-errors || exit 91
...
f8f497e3dec3f55a9ff9cee4c8fd66cf90fee32f68dbc5745084c1569de176ea  probe.py
...
code=$?
set -e
printf '{"exit_code":%d,"script_sha256":"f8f497e3dec3f55a9ff9cee4c8fd66cf90fee32f68dbc5745084c1569de176ea"}\n' "$code" > terminal.json
