#!/usr/bin/env python3
"""Score sealed reachable 1M graph returns against paired V199/V155 GT witness."""
...
def run(args: argparse.Namespace) -> None:
    ...
    if (serving.get("schema") != "borsuk-v219-reachable-graph-1m-serving-v1"
            or serving.get("raw_sha256") != digest(args.raw)
            or serving.get("queries") != COUNT or len(raw) != COUNT
            or len(baseline) != COUNT):
        raise ValueError("V219 serving/quality identity differs")
    ...
    for ef, shortlist in ARMS:
        ...
        for ordinal, (row, old) in enumerate(zip(raw, baseline, strict=True)):
            ...
            if (row["ordinal"] != ordinal or old["ordinal"] != ordinal
                ...
                raise ValueError(f"V219 case geometry differs at {ordinal}")
    ...
    args.output.write_text(json.dumps({
        "schema": "borsuk-v219-reachable-graph-1m-quality-v1",
