#!/usr/bin/env python3
"""One preregistered 100k source graph and resident FP16 gate on Spot."""
...
SCHEMA = "borsuk-v213-resident-graph-100k-spot-v1"
TAG = "borsuk-v213-resident-graph-100k"
WALL_SECONDS = 10_800
...
if {row[0] for row in INPUTS} != {"source.parquet", "sq8.bin", "requests.jsonl",
                                  ...
                                  "v113-codes.npy"}:
    raise RuntimeError("V213 frozen input roster differs")
...
ARTIFACTS = ("raw.jsonl", "serving.json", "summary.json", "prepare.json",
             "graph-prep.json", "vectors.raw", "plane.bin", "old-to-new.u32", "new-to-old.u32", "books.raw",
...
def user_data(commit: str, archive_sha: str, archive_key: str, prefix: str) -> str:
    template = r'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v213-hard-stop --on-active=@@WALL@@s /usr/sbin/shutdown -h now
root=/mnt/v213-resident-graph-100k
...
phase=build
PYTHONPATH="$root/repo" .venv/bin/python -m scripts.v213_prepare_graph_100k \
  --source source.parquet --old-sq8 sq8.bin --new-to-old new-to-old.u32 \
  --plane plane.bin --preparation prepare.json --vectors vectors.raw --summary graph-prep.json
cd repo
"$CARGO_HOME/bin/cargo" build --release --locked -p borsuk --bin v213_resident_graph_100k --jobs 6 >"$root/build.log" 2>&1
cd "$root"
phase=serve
/usr/bin/time -v -o serving-resources.txt "$CARGO_TARGET_DIR/release/v213_resident_graph_100k" \
  graph-prep.json plane.bin vectors.raw requests.jsonl raw.jsonl serving.json
...
phase=truth
@@LATE_DOWNLOADS@@
.venv/bin/python -m scripts.v213_score_graph_100k \
...
'''
    ...
    if "@@" in template:
        raise ValueError("unresolved V213 worker placeholder")
    return template
...
def launch(attempt: str) -> None:
    ...
    archive_key = f"research/v213-resident-graph-100k/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v213-resident-graph-100k/{commit}/runs/{attempt}"
    ...
    put_if_absent(prefix + "/reservation.json", json.dumps({
        ...
        "ef_search": [256, 512, 1024, 2048],
        "gate": "hits>=25440;p05>=98;whole-Rust-query-p95<=10ms;0 vector GET",
    ...
    receipt = ec2.run_instances(
        ClientToken="v213-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
        ...
        while time.monotonic() < deadline:
            try:
                ...
                if (terminal.get("schema") != SCHEMA or terminal.get("instance_id") != instance_id
                    ...
                    raise ValueError("V213 terminal identity differs")
                terminal["terminal_sha256"] = hashlib.sha256(raw).hexdigest()
                print(json.dumps(terminal, sort_keys=True), flush=True)
                if terminal.get("status") != "complete":
                    raise RuntimeError("V213 failed; inspect only closed terminal artifacts")
                if set(terminal.get("artifacts", {})) != set(ARTIFACTS):
                    raise ValueError("V213 artifact roster differs")
                for name, identity in terminal["artifacts"].items():
                    ...
                    if size != identity["bytes"] or value.hexdigest() != identity["sha256"]:
                        raise ValueError(f"V213 artifact readback differs: {name}")
            ...
            if state in {"terminated", "shutting-down"}:
                raise RuntimeError("V213 worker stopped before terminal")
            time.sleep(20)
        raise TimeoutError("V213 Spot cell exceeded wall cap")
    finally:
        try:
            state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
...
if __name__ == "__main__":
    ...
    with open("/tmp/borsuk-v213-resident-graph-launch.lock", "a+") as lock:
