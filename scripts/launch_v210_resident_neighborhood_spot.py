#!/usr/bin/env python3
"""One preregistered 100k resident neighborhood falsifier on Spot."""
...
SCHEMA = "borsuk-v210-resident-neighborhood-100k-spot-v1"
TAG = "borsuk-v210-resident-neighborhood-100k"
WALL_SECONDS = 3_600
...
INPUTS = tuple(row for row in V163_INPUTS if row[0] == "source.parquet") + tuple(
    ...
    row for row in V170_INPUTS if row[0] == "order.npy")
if {row[0] for row in INPUTS} != {"source.parquet", "sq8.bin", "requests.jsonl", "order.npy"}:
    raise RuntimeError("V210 frozen input roster differs")
...
def user_data(commit: str, archive_sha: str, archive_key: str, prefix: str) -> str:
    template = r'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v210-hard-stop --on-active=@@WALL@@s /usr/sbin/shutdown -h now
root=/mnt/v210-resident-neighborhood-100k
...
@@DOWNLOADS@@
phase=prepare
/usr/bin/time -v -o prepare-resources.txt .venv/bin/python -m scripts.v210_resident_neighborhood_100k prepare \
...
phase=truth
@@LATE_DOWNLOADS@@
/usr/bin/time -v -o score-resources.txt .venv/bin/python -m scripts.v210_resident_neighborhood_100k score \
  --source source.parquet --old-sq8 sq8.bin --order order.npy \
  --requests requests.jsonl --raw raw.jsonl --seal pretruth-seal.json \
...
'''
    ...
    if "@@" in template:
        raise ValueError("unresolved V210 worker placeholder")
    return template
...
def launch(attempt: str) -> None:
    ...
    archive_key = f"research/v210-resident-neighborhood-100k/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v210-resident-neighborhood-100k/{commit}/runs/{attempt}"
    ...
    put_if_absent(prefix + "/reservation.json", json.dumps({
        ...
        "gate": "hits>=paired-V193-full-rank;p05>=98;FP16-score-p95<=10ms",
    ...
    receipt = ec2.run_instances(
        ClientToken="v210-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
        ...
        while time.monotonic() < deadline:
            try:
                ...
                if (terminal.get("schema") != SCHEMA or terminal.get("instance_id") != instance_id
                    ...
                    raise ValueError("V210 terminal identity differs")
                terminal["terminal_sha256"] = hashlib.sha256(raw).hexdigest()
                print(json.dumps(terminal, sort_keys=True), flush=True)
                if terminal.get("status") != "complete":
                    raise RuntimeError("V210 failed; inspect only closed terminal artifacts")
                if set(terminal.get("artifacts", {})) != set(ARTIFACTS):
                    raise ValueError("V210 artifact roster differs")
                for name, identity in terminal["artifacts"].items():
                    ...
                    if size != identity["bytes"] or value.hexdigest() != identity["sha256"]:
                        raise ValueError(f"V210 artifact readback differs: {name}")
            ...
            if state in {"terminated", "shutting-down"}:
                raise RuntimeError("V210 worker stopped before terminal")
            time.sleep(20)
        raise TimeoutError("V210 Spot cell exceeded wall cap")
    finally:
        try:
            state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
...
if __name__ == "__main__":
    ...
    with open("/tmp/borsuk-v210-resident-neighborhood-launch.lock", "a+") as lock:
