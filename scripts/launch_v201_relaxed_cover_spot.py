...
SCHEMA = "borsuk-v201-relaxed-cover-spot-v1"
TAG = "borsuk-v201-relaxed-cover"
...
def user_data(commit: str, archive_sha: str, archive_key: str,
    ...
    template = r'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v201-hard-stop --on-active=@@WALL@@s /usr/sbin/shutdown -h now
root=/mnt/v201-relaxed-cover
...
'''
    ...
    if "@@" in template:
        raise ValueError("unresolved V201 worker placeholder")
    return template
...
def launch(attempt: str) -> None:
    ...
    if subprocess.run(["git", "merge-base", "--is-ancestor", "HEAD", "origin/main"],
        ...
        raise ValueError("V201 source commit is not pushed to origin/main")
    archive = archive_source(commit)
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as tar:
        required = {"crates/borsuk/src/relaxed_priced_interval.rs",
                    "crates/borsuk/src/bin/v201_relaxed_cover_preflight.rs",
                    "docs/research/v201-one-cap-relaxation-prereg.md"}
        if not required.issubset(tar.getnames()):
            raise ValueError("V201 source archive lacks frozen gate")
    archive_sha = hashlib.sha256(archive).hexdigest()
    archive_key = f"research/v201-relaxed-cover/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v201-relaxed-cover/{commit}/runs/{attempt}"
    ...
    for _, key, size, _ in INPUTS:
        if s3.head_object(Bucket=BUCKET, Key=key)["ContentLength"] != size:
            raise ValueError("V201 frozen input length differs")
    ...
    receipt = ec2.run_instances(
        ClientToken="v201-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
        ...
        while time.monotonic() < deadline:
            try:
                ...
                if terminal.get("status") != "complete":
                    raise RuntimeError("V201 failed; inspect only closed terminal artifacts")
                if set(ARTIFACTS) != set(terminal.get("artifacts", {})):
                    raise ValueError("complete V201 artifact roster differs")
                for name, identity in terminal["artifacts"].items():
                    ...
                    if size != identity["bytes"] or digest.hexdigest() != identity["sha256"]:
                        raise ValueError(f"V201 S3 artifact read-back differs: {name}")
                ...
                if (bench.get("linear_exact") != 861
                        or sum(bench.get(key, 0) for key in
                               ("linear_exact", "unit_exact", "get_exact", "needs_2d")) != 1000):
                    raise ValueError("V201 hierarchy accounting differs")
            ...
            time.sleep(20)
        raise TimeoutError("V201 Spot cell exceeded wall cap")
    finally:
        try:
            state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
...
def main() -> None:
    ...
    with open("/tmp/borsuk-v201-relaxed-cover-launch.lock","a+") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as error:
            raise RuntimeError("V201 launcher is already active") from error
        launch(args.attempt)
