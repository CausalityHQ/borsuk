#!/usr/bin/env python3
"""Truth-separated 1M projection of mirrored 104/96-byte page waves."""
...
from scripts.native_progressive_code_wave import (
    page_lengths,
    plan_mirrored_wave,
)
from scripts.v97_row_width_screen import _authenticate_object

SCHEMA = "borsuk-one-million-progressive-mirrored-code-projection-v1"
PRIOR_PREFIX = (
...
PLAN_FILE = "progressive-code-wave-plans.json"
PLAN_SEAL_FILE = "progressive-code-wave-plan-seal.json"
EVIDENCE_FILE = "progressive-code-wave-evidence.json"
RESULT_FILE = "progressive-code-wave-result.json"
...
def _source_geometry(
...
) -> tuple[tuple[dict[str, tuple[int, ...]], dict[str, tuple[int, ...]]], int, np.ndarray]:
    ...
    lengths = page_lengths(tuple(int(count) for count in counts), base_pages=base_pages)
...
def run_plan(
    ...
    for ordinal, prior in enumerate(historical["samples"]):
        ...
        samples.append({"query_ordinal": ordinal, "code": plan_mirrored_wave(priority, *lengths)})
    plans = {
        "schema": SCHEMA + "-plans",
        ...
        "row_bytes": {"sign": 104, "magnitude": 96},
        "samples": samples,
    }
...
def _metrics(samples: list[dict[str, object]], plans: list[dict[str, object]]) -> dict[str, int]:
    ...
    code_plans = [plan["code"]["sign"] for plan in plans]
    magnitude_plans = [plan["code"]["magnitude"] for plan in plans]
    return {
        "gt100_hits": sum(hits),
        ...
        "maximum_magnitude_bytes": max(plan["encoded_bytes"] for plan in magnitude_plans),
        "total_gets": sum(plan["gets"] for plan in code_plans),
        "total_bytes": sum(plan["encoded_bytes"] for plan in code_plans),
        "total_magnitude_bytes": sum(plan["encoded_bytes"] for plan in magnitude_plans),
...
def run_evaluate(
    ...
    for ordinal, (item, prior) in enumerate(zip(plans["samples"], historical["samples"], strict=True)):
        if (
            ...
            or item["code"]["sign"]["priority_pages"] != prior["candidate"]["priority_pages"]
        ):
            raise ValueError("code-wave query plan differs")
        priority = _global_priority(prior, base_pages, expected_pages)
        if item["code"] != plan_mirrored_wave(priority, *lengths):
            raise ValueError("code-wave sealed plan replay differs")
        evaluation = evaluate_query_pages(
            item["code"]["sign"], tuple(int(page) for page in truth_pages[ordinal]), lengths[0],
        )
