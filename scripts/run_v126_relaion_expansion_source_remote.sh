#!/bin/bash
# One sealed fixed-route source-score diagnostic on Causality Spot.
set -euo pipefail
...
deadline_epoch=$((started_epoch + V126_WALL_SECONDS))
monitor_pid=
imds_token=
imds() {
...
finish() {
  ...
  for path in hashes.log install.log compile.log compile-resources.txt \
      tests.log tests-resources.txt replay.log replay-resources.txt \
      preflight.log preflight-resources.txt \
      evaluate.log evaluate-resources.txt wide-replay.jsonl evidence.jsonl \
      summary.json worker.log interrupt-stop.txt; do
    if [ -f "$path" ] && ! timeout 900 aws s3 cp "$path" \
        "$V126_OUTPUT_PREFIX/artifacts/$path" --only-show-errors; then
      upload_failed=1
    fi
  done
...
names=('hashes.log','install.log','compile.log','compile-resources.txt',
       'tests.log','tests-resources.txt','replay.log','replay-resources.txt',
       'preflight.log','preflight-resources.txt',
       'evaluate.log','evaluate-resources.txt','wide-replay.jsonl',
       'evidence.jsonl','summary.json','worker.log','interrupt-stop.txt')
artifacts={}
for name in names:
    path=Path(name)
    if path.is_file():
...
print(json.dumps({'schema':'borsuk-v126-relaion-expansion-source-spot-v1',
  'source_commit':os.environ['V126_SOURCE_COMMIT'],
...
PY
  aws s3 cp terminal.json "$V126_OUTPUT_PREFIX/terminal.json" --only-show-errors
  shutdown -h now || true
  exit "$code"
}
trap finish EXIT
...
download_checked() {
  role=$1; file=$2
  uri_var=V126_${role}_URI
  sha_var=V126_${role}_SHA256
  bytes_var=V126_${role}_BYTES
...
download_checked REQUESTS requests.jsonl
download_checked SEALED_REPLAY sealed-replay.jsonl
...
phase=tests
run_science tests .venv/bin/python -m unittest -q \
  scripts.test_v126_relaion_expansion_source
phase=replay
run_science replay "$CARGO_TARGET_DIR/release/borsuk-v114-score-gate" \
  ...
  requests.jsonl 512
cp replay.log wide-replay.jsonl
phase=preflight
run_science preflight .venv/bin/python -m scripts.v126_relaion_expansion_source \
  --preflight --requests requests.jsonl --sealed sealed-replay.jsonl \
  --wide wide-replay.jsonl
phase=truth-download
download_checked TRUTH validation-truth.parquet
phase=evaluate
run_science evaluate .venv/bin/python -m scripts.v126_relaion_expansion_source \
  --source source.parquet --layout layout.npy --requests requests.jsonl \
  --sealed sealed-replay.jsonl --wide wide-replay.jsonl \
  --truth validation-truth.parquet --evidence evidence.jsonl \
