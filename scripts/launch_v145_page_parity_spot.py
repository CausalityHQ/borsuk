#!/usr/bin/env python3
"""Launch and monitor one frozen V145 page-parity feasibility Spot attempt."""
...
REGION = "eu-central-1"
TAG = "borsuk-v145-page-parity"
RUNNER = "scripts/run_v145_page_parity_remote.sh"
INPUTS = (
...
def validate(plan: Plan) -> None:
    ...
    if (
        ...
        or not output_key.startswith(
            f"research/v145-page-parity/{plan.source_commit}/runs/"
        )
        ...
        raise ValueError("immutable V145 plan differs")
...
def user_data(plan: Plan) -> str:
    ...
    exports = "\n".join(
        (
            f"export V145_SOURCE_COMMIT={plan.source_commit}",
            f"export V145_ARCHIVE_SHA256={plan.archive_sha256}",
            f"export V145_OUTPUT_PREFIX={shlex.quote(plan.output_prefix)}",
            f"export V145_WALL_SECONDS={plan.wall_seconds}",
        )
    )
    return f"""#!/bin/bash
set -euo pipefail
systemd-run --unit=v145-hard-stop --on-active={plan.wall_seconds}s /usr/sbin/shutdown -h now
...
PY
    aws s3 cp terminal.json "$V145_OUTPUT_PREFIX/terminal.json" --only-show-errors || true
    shutdown -h now || true
  fi
}}
...
def launch_spec(plan: Plan) -> dict:
    token = "v145-" + hashlib.sha256(plan.output_prefix.encode()).hexdigest()[:48]
    return {
        "ClientToken": token,
...
def check_source_and_inputs(s3: object, plan: Plan) -> None:
    ...
    with tarfile.open(fileobj=archive, mode="r:gz") as source:
        if (RUNNER not in source.getnames()
                ...
                or "scripts/v114_score_rust/src/bin/v145_page_parity.rs" not in source.getnames()
            ...
            raise ValueError("source archive lacks V145 runner or evaluator")
...
def reserve(plan: Plan) -> None:
    receipt = json.dumps({
        "schema": "borsuk-v145-page-parity-reservation-v1",
...
def launch_and_monitor(plan: Plan) -> dict:
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V145 worker already active")
    ...
    try:
        while time.monotonic() < deadline:
            if not missing(s3, prefix + "/terminal.json"):
                ...
                if (terminal.get("schema") != "borsuk-v145-page-parity-feasibility-spot-v1"
                ...
                if terminal.get("status") == "complete":
                    ...
                    if decision != {"schema": "borsuk-v145-decision-v1",
                                    "deep_completed": True, "relaion_completed": True}:
                        raise ValueError("V145 decision differs")
                    artifacts = terminal.get("artifacts", {})
                    if not required.issubset(artifacts):
                        raise ValueError("V145 complete terminal lacks required artifacts")
                    for name, metadata in artifacts.items():
                        ...
                        if length != metadata["bytes"] or digest.hexdigest() != metadata["sha256"]:
                            raise ValueError(f"V145 artifact differs: {name}")
                    terminal["authenticated_artifact_count"] = len(artifacts)
                print(json.dumps(terminal, sort_keys=True), flush=True)
                return terminal
            state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
            if state == "terminated":
                raise RuntimeError("V145 instance terminated without terminal marker")
            time.sleep(15)
        raise TimeoutError(f"V145 terminal missing after worker deadline: {instance_id}")
...
def main() -> None:
    ...
    if terminal.get("status") != "complete" or terminal.get("exit_code") != 0:
        raise RuntimeError("V145 terminal reports a failed attempt")
