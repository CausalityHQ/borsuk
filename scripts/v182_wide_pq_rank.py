...
SCHEMA = "borsuk-v182-wide-pq-rank-v1"
FIRST = 1152
COUNT = 256
FIT = 128
WIDTH = 32
TOP_UNITS = (672, 1344)
HOLDOUT_THRESHOLD = 12745


def selected_allowance(top_totals: dict[str, int],
                       p05: dict[str, int]) -> int | None:
    if (set(top_totals) != {str(value) for value in TOP_UNITS}
            or set(p05) != set(top_totals)
            or any(not 0 <= hits <= FIT * 100 for hits in top_totals.values())
            or any(not 0 <= hits <= 100 for hits in p05.values())):
        raise ValueError("V182 allowance totals differ")
    return next((units for units in TOP_UNITS
                 if top_totals[str(units)] >= HOLDOUT_THRESHOLD
                 and p05[str(units)] >= 98), None)
...
def _normalized(vectors: np.ndarray) -> np.ndarray:
    ...
    for start in range(0, ROWS, 8192):
        ...
        if not np.isfinite(norms).all() or (norms <= 0).any():
            raise ValueError("V182 source norm differs")
...
def _features(args: argparse.Namespace) -> tuple[dict, list[dict]]:
    ...
    if sha256(seal_path) != args.prepare_sha256:
        raise ValueError("V182 prepare seal differs")
    ...
    if (seal.get("schema") != SCHEMA + "-prepare-seal"
            ...
            or seal.get("top_units") != list(TOP_UNITS)
            or seal.get("holdout_threshold") != HOLDOUT_THRESHOLD
        ...
        raise ValueError("V182 sealed features differ")
    for row in features:
        ...
        if (not isinstance(ranked, list) or not ranked
            ...
            raise ValueError("V182 feature unit geometry differs")
    return seal, features


def prepare(args: argparse.Namespace) -> None:
    if args.output.exists():
        raise ValueError("V182 output already exists")
    ...
    if not np.array_equal(inverse_new[new_order], np.arange(ROWS)):
        raise ValueError("V182 physical order differs")
    ...
    with (args.output / "features.jsonl").open("x") as output:
        for ordinal, stable_id in enumerate(chosen, start=FIRST):
            ...
            if nominees.shape != (512,) or len(np.unique(nominees)) != 512:
                raise ValueError("V182 nominee roster differs")
            ...
            if field.units != candidate or field.scores.size != len(candidate) * UNIT_ROWS:
                raise ValueError("V182 score field candidate geometry differs")
            scores = field.scores.reshape(len(candidate), UNIT_ROWS).copy()
            own = np.flatnonzero(field.old_rows == inverse_old[source_row])
            if own.size > 1:
                raise ValueError("V182 pseudoquery row appears twice in score field")
            if own.size == 1:
    ...
    (args.output / "prepare-seal.json").write_text(canonical({
        ...
        "fit": "1153-1280", "holdout": "1281-1408",
...
def _labels(features: list[dict], source: np.ndarray,
    ...
    for row in features:
        ...
        if (type(source_row) is not int or not 0 <= source_row < ROWS
            ...
            raise ValueError("V182 source identity differs")
        ...
        top_hits = {str(units): sum(masses.get(unit, 0)
                                    for unit in ranked[:units])
                    for units in TOP_UNITS}
...
def fit(args: argparse.Namespace) -> None:
    ...
    (args.output / "fit-seal.json").write_text(canonical({
        ...
        "fit_top_hits": {str(units): sum(row["top_hits"][str(units)]
                                        for row in labels)
                         for units in TOP_UNITS},
    }))
...
def evaluate(args: argparse.Namespace) -> None:
    ...
    if sha256(args.output / "fit-seal.json") != args.fit_sha256:
        raise ValueError("V182 fit seal differs")
    ...
    if (seal.get("schema") != SCHEMA + "-fit-seal"
        ...
        raise ValueError("V182 fit authority differs")
    ...
    with (args.output / "holdout-labels.jsonl").open("x") as output:
        for row in labels:
            output.write(canonical(row))
    observed = {str(units): [row["top_hits"][str(units)] for row in labels]
                for units in TOP_UNITS}
    predicted = {
        str(units): [sum(curve.expected_hits[:min(units,
                                                len(feature["ranked_units"]))])
                     for feature in features[FIT:]]
        for units in TOP_UNITS
    }
    candidate_total = sum(row["candidate_hits"] for row in labels)
    top_total = {units: sum(counts) for units, counts in observed.items()}
    p05 = {units: sorted(counts)[(FIT * 5 + 99) // 100 - 1]
           for units, counts in observed.items()}
    selected = selected_allowance(top_total, p05)
    (args.output / "summary.json").write_text(canonical({
        "schema": SCHEMA + "-summary", "dataset": "ReLAION-1M D768",
        "split": "source-pseudoquery-hash-ranks-1153-1408",
        "metric": "exact-source-truth-contained-in-PQ-top-unit-allowance",
        ...
        "holdout_p05": p05,
        "holdout_prediction_sum": {units: sum(values)
                                   for units, values in predicted.items()},
        "holdout_prediction_mae_per_query": {
            units: sum(abs(actual - estimate)
                       for actual, estimate in zip(observed[units], values,
                                                   strict=True)) / FIT
            for units, values in predicted.items()},
        "selected_allowance": selected,
        "decision": ("advance-to-physical-planner" if selected is not None
                     else "revise-candidate-or-pq-score"),
        "holdout_threshold": HOLDOUT_THRESHOLD,
...
def main() -> None:
    ...
    if args.phase == "prepare":
        ...
        if len(args.prepare_sha256) != 64:
            raise ValueError("V182 fit requires externally sealed prepare SHA")
        fit(args)
    else:
        if len(args.prepare_sha256) != 64 or len(args.fit_sha256) != 64:
            raise ValueError("V182 evaluate requires externally sealed SHA values")
        evaluate(args)
