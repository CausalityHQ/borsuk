#!/usr/bin/env python3
"""Launch one immutable source-only V177 candidate ceiling gate on Spot."""
...
SCHEMA = "borsuk-v177-source-candidate-ceiling-spot-v1"
TAG = "borsuk-v177-source-candidate-ceiling"
WALL_SECONDS = 14_400
...
ARTIFACTS = (
    "out/rosters.jsonl", "out/prepare-seal.json",
...
def user_data(commit: str, archive_sha: str, archive_key: str, prefix: str) -> str:
    return f'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v177-hard-stop --on-active={WALL_SECONDS}s /usr/sbin/shutdown -h now
root=/mnt/v177-source-candidate-ceiling
...
/usr/bin/time -v -o prepare-resources.txt .venv/bin/python -m \
  scripts.v177_source_candidate_ceiling prepare --output out \
...
# The sealed, label-blind rosters must be durable before source truth is scored.
aws s3 cp out/rosters.jsonl "s3://{BUCKET}/{prefix}/sealed/rosters.jsonl" --only-show-errors
...
/usr/bin/time -v -o evaluate-resources.txt .venv/bin/python -m \
  scripts.v177_source_candidate_ceiling evaluate --output out \
  --source source.parquet --old-layout old-layout.npy \
  --old-sq8 old-sq8.bin --router router --order order.npy \
...
def _launch() -> None:
    ...
    if subprocess.run(["git", "merge-base", "--is-ancestor", "HEAD",
        ...
        raise ValueError("V177 source commit is not pushed to origin/main")
    ...
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as tar:
        required = {"scripts/v166_surrogate_probe.py",
                    ...
                    "docs/research/v177-source-candidate-ceiling-prereg.md"}
        if not required.issubset(tar.getnames()):
            raise ValueError("source archive lacks V177 gate")
    archive_sha = hashlib.sha256(archive).hexdigest()
    archive_key = f"research/v177-source-candidate-ceiling/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v177-source-candidate-ceiling/{commit}/runs/{attempt}"
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V177 worker already active")
    ...
    receipt = ec2.run_instances(
        ClientToken="v177-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
    ...
    while time.monotonic() < deadline:
        if not missing(s3, prefix + "/terminal.json"):
            ...
            for name, identity in terminal.get("artifacts", {}).items():
                ...
                if size != identity["bytes"] or digest.hexdigest() != identity["sha256"]:
                    raise ValueError(f"V177 S3 artifact read-back differs: {name}")
            terminal["terminal_sha256"] = hashlib.sha256(raw).hexdigest()
            print(json.dumps(terminal, sort_keys=True), flush=True)
            if terminal.get("status") != "complete":
                raise RuntimeError("V177 terminal reports failed; see closed artifacts")
            return
        ...
        if state in {"terminated", "shutting-down"}:
            ...
            if time.monotonic() - started >= WALL_SECONDS - 60:
                raise TimeoutError("V177 worker reached wall cap before terminal")
            raise RuntimeError("Spot worker stopped before terminal; restart full cell")
        time.sleep(20)
    ec2.terminate_instances(InstanceIds=[instance_id])
    raise TimeoutError("V177 cell exceeded wall cap")


def main() -> None:
    # All V177 controllers on this devbox share one process-lifetime lock.
    # The EC2 tag check below also rejects an already running worker.
    with open("/tmp/borsuk-v177-source-candidate-ceiling-launch.lock", "a+") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as exc:
            raise RuntimeError("V177 launcher is already running here") from exc
        _launch()
