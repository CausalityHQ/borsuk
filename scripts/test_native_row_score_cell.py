...
class RowScoreCellTests(unittest.TestCase):
    def test_cli_requires_output_directory_for_evaluation(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            result = subprocess.run(
                [sys.executable, "-m", "scripts.native_row_score_cell", "evaluate",
    ...
    def test_construct_phase_does_not_read_query_or_truth(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            with patch("scripts.native_row_score_cell._read_inputs", return_value=(ids, vectors, membership)):
            ...
            with (
                patch("scripts.native_row_score_cell._read_inputs", return_value=(ids, vectors, membership)),
                patch("scripts.native_row_score_cell._read_queries_truth", return_value=(
                    vectors[180:181], (tuple(ids[index] for index in range(128, 228)),)
                )),
                patch("scripts.native_row_score_cell.read_geometric_router_parquet", return_value=router),
                patch("scripts.native_row_score_cell.route_geometric_query", return_value=SimpleNamespace(
            ...
            self.assertEqual(result["metrics"]["pq_mean_recall_at_100_ppm"], 1_000_000)
            with (
                patch("scripts.native_row_score_cell._read_inputs", return_value=(ids, vectors, membership)),
                patch("scripts.native_row_score_cell._read_queries_truth", return_value=(
                    vectors[180:181], (tuple(ids[index] for index in range(128, 228)),)
                ...
                patch("scripts.native_row_score_cell.read_geometric_router_parquet", return_value=router),
                patch("scripts.native_row_score_cell._independent_route", return_value=(
                    (0, 1), (0, 1), 2000, 0
                )),
            ):
