...
def _reference_hits(
    ...
    *, top_k: int,
    ...
    if not np.isfinite(primary).all() or not np.isfinite(exact).all():
    ...
    return hits(primary), hits(exact)
...
def validate_loaded(
    ...
    if (
        evidence.get("schema") != "borsuk-two-bit-returned-evidence-v1"
        or result.get("schema") != "borsuk-two-bit-returned-result-v1"
    ...
    validation = {
        "schema": "borsuk-two-bit-returned-validation-v1",
        "valid": True,
