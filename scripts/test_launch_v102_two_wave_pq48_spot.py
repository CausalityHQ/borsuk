"""Contract tests for the one-shot V102 Causality Spot launcher."""
...
from scripts.launch_v102_two_wave_pq48_spot import (
...
class V102SpotLauncherTests(unittest.TestCase):
    @staticmethod
    def plan():
        return build_plan(
            profile="causality",
            ...
            output_prefix="s3://fixture/v102/a0001",
    ...
    def test_plan_is_fixed_to_development_one_attempt_and_registered_caps(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "V102 Spot plan differs"):
    ...
    def test_worker_binds_all_inputs_and_executes_only_registered_remote_script(
        ...
        self.assertIn("V102_QUERY_COUNT=128", script)
        self.assertIn("V102_REGISTERED_QUERY_COUNT=1000", script)
        self.assertIn("V102_SOURCE_COMMIT=" + "a" * 40, script)
        for role in FROZEN_INPUTS:
            self.assertIn(f"V102_{role.upper()}_URI=", script)
        self.assertIn("v102_two_wave_pq48_run_remote.sh", script)
    ...
    def test_terminal_is_canonical_and_requires_complete_evidence(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "V102 terminal differs"):
            canonical_terminal_bytes(
                plan,
                instance_id="i-0123456789abcdef0",
                status="complete",
    ...
    def test_target_type_rejects_non_eu_central_zone(self) -> None:
        with self.assertRaisesRegex(ValueError, "V102 Spot target differs"):
    ...
    def test_launch_retries_only_capacity_and_creates_one_instance(self) -> None:
        class FakeEc2:
            ...
            def run_instances(self, **_request):
                ...
                return {"Instances": [{"InstanceId": "i-v102"}]}

        ec2 = FakeEc2()
        self.assertEqual(launch_one_spot(self.plan(), ec2_client=ec2), "i-v102")
    ...
    def test_monitor_reads_only_terminal_and_always_terminates(self) -> None:
        ...
        terminal = monitor_and_terminate(
            self.plan(), s3_client=s3, ec2_client=ec2, instance_id="i-v102"
        )
        self.assertEqual(terminal.status, "failed")
        self.assertEqual(s3.keys, [("fixture", "v102/a0001/terminal.json")])
        self.assertEqual(ec2.terminated, ["i-v102"])

    def test_remote_shell_runs_only_v102_producer_and_independent_reducer(self) -> None:
        body = Path("scripts/v102_two_wave_pq48_run_remote.sh").read_text()
        self.assertIn("from scripts.v102_two_wave_pq48_refinement import", body)
        self.assertIn("from scripts.v102_two_wave_pq48_rescore import", body)
        self.assertIn("canonical_v102_result_bytes(evaluate_v102(", body)
        self.assertIn("rescore_v102_result(", body)
    ...
    def test_reservation_and_launch_receipts_are_create_only_and_bound(self) -> None:
        ...
        claim_launched_attempt(self.plan(), s3_client=s3, instance_id="i-v102")
        ...
        self.assertEqual(json.loads(s3.requests[1]["Body"])["instance_id"], "i-v102")
