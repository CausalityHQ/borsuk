...
SCHEMA = "borsuk-v203-hard-dp-loop-spot-v1"
TAG = "borsuk-v203-hard-dp-loop"
...
def user_data(commit: str, archive_sha: str, archive_key: str,
    ...
    template = r'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v203-hard-stop --on-active=@@WALL@@s /usr/sbin/shutdown -h now
root=/mnt/v203-hard-dp-loop
...
'''
    ...
    if "@@" in template:
        raise ValueError("unresolved V203 worker placeholder")
    return template
...
def launch(attempt: str) -> None:
    ...
    if subprocess.run(["git", "merge-base", "--is-ancestor", "HEAD", "origin/main"],
        ...
        raise ValueError("V203 source commit is not pushed to origin/main")
    archive = archive_source(commit)
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as tar:
        required = {"crates/borsuk/src/hard_priced_interval.rs",
                    "crates/borsuk/src/bin/v202_two_cap_cover_preflight.rs",
                    "docs/research/v203-hard-dp-loop-prereg.md"}
        if not required.issubset(tar.getnames()):
            raise ValueError("V203 source archive lacks frozen gate")
    archive_sha = hashlib.sha256(archive).hexdigest()
    archive_key = f"research/v203-hard-dp-loop/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v203-hard-dp-loop/{commit}/runs/{attempt}"
    ...
    for _, key, size, _ in INPUTS:
        if s3.head_object(Bucket=BUCKET, Key=key)["ContentLength"] != size:
            raise ValueError("V203 frozen input length differs")
    ...
    receipt = ec2.run_instances(
        ClientToken="v203-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
        ...
        while time.monotonic() < deadline:
            try:
                ...
                if terminal.get("status") != "complete":
                    raise RuntimeError("V203 failed; inspect only closed terminal artifacts")
                if set(ARTIFACTS) != set(terminal.get("artifacts", {})):
                    raise ValueError("complete V203 artifact roster differs")
                for name, identity in terminal["artifacts"].items():
                    ...
                    if size != identity["bytes"] or digest.hexdigest() != identity["sha256"]:
                        raise ValueError(f"V203 S3 artifact read-back differs: {name}")
                ...
                if (bench.get("linear_exact") != 861
                    ...
                    raise ValueError("V203 hierarchy accounting differs")
            ...
            time.sleep(20)
        raise TimeoutError("V203 Spot cell exceeded wall cap")
    finally:
        try:
            state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
...
def main() -> None:
    ...
    with open("/tmp/borsuk-v203-hard-dp-loop-launch.lock","a+") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as error:
            raise RuntimeError("V203 launcher is already active") from error
        launch(args.attempt)
