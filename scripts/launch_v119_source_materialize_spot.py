#!/usr/bin/env python3
"""Launch one immutable corpus-only source materialization on Causality Spot."""
...
STAGING = (f"s3://{BUCKET}/publication/v3/20260812/datasets/"
           "deep-image-96/attempts/0001/STAGING_COMPLETE.json")
TRAIN_PREFIX = (f"s3://{BUCKET}/publication/v3/20260812/datasets/"
                "deep-image-96/attempts/0001/materialized/")
STAGING_SHA256 = "eb10a317cc778b30e2ee88eee8614b760e36f31d2d7af1a62a60f5f32c163230"
STAGING_BYTES = 18800
SCHEMA = "borsuk-v119-source-materialize-spot-v1"
...
def validate_plan(plan: Plan) -> None:
    ...
    if (not hex_value(plan.source_commit, 40)
        ...
        raise ValueError("V119 immutable source plan differs")


def user_data(plan: Plan) -> str:
    validate_plan(plan)
    exports = "\n".join((
        f"export V119_SOURCE_COMMIT={_q(plan.source_commit)}",
        f"export V119_OUTPUT_PREFIX={_q(plan.output_prefix.rstrip('/'))}",
        f"export V119_WALL_SECONDS={plan.wall_seconds}",
        f"export V119_STAGING_URI={_q(STAGING)}",
        f"export V119_STAGING_SHA256={STAGING_SHA256}",
        f"export V119_STAGING_BYTES={STAGING_BYTES}",
        f"export V119_TRAIN_URI_PREFIX={_q(TRAIN_PREFIX)}",
    ))
    return f"""#!/bin/bash
set -euo pipefail
...
import json,os
print(json.dumps({{"schema":"{SCHEMA}","source_commit":os.environ["V119_SOURCE_COMMIT"],
...
PY
    aws s3 cp terminal.json "$V119_OUTPUT_PREFIX/terminal.json" --only-show-errors || true
    shutdown -h now || true
  fi
}}
...
trap - EXIT
exec bash repo/scripts/run_v119_source_materialize_remote.sh
"""


def launch_spec(plan: Plan, zone: str, subnet: str) -> dict:
    token = "v119-" + hashlib.sha256(
    ...
    return {
        ...
        "TagSpecifications": [{"ResourceType": "instance", "Tags": [
            {"Key": "Name", "Value": "borsuk-v119-source-materialize"},
...
def launch_and_monitor(plan: Plan) -> dict:
    ...
    if not _missing(s3, bucket, f"{prefix}/reservation.json") or not _missing(
        ...
        raise ValueError("V119 immutable source attempt already registered")
    receipt = (json.dumps({"schema": SCHEMA, "source_commit": plan.source_commit,
                           "archive_sha256": plan.archive_sha256,
                           "staging_sha256": STAGING_SHA256, "attempt": 1},
                          sort_keys=True, separators=(",", ":")) + "\n").encode()
    ...
    if instance_id is None:
        raise RuntimeError("V119 Spot capacity unavailable; reservation retained")
        ...
        while time.monotonic() < deadline:
            if not _missing(s3, bucket, f"{prefix}/terminal.json"):
                ...
                if (terminal.get("schema") != SCHEMA
                    ...
                    raise ValueError("V119 terminal identity differs")
                return terminal
            state = ec2.describe_instances(InstanceIds=[instance_id])[
                "Reservations"][0]["Instances"][0]["State"]["Name"]
            if state in {"stopped", "shutting-down", "terminated"}:
                raise RuntimeError(f"V119 Spot {instance_id} ended without terminal")
            time.sleep(20)
        raise TimeoutError(f"V119 Spot {instance_id} terminal deadline exceeded")
    finally:
