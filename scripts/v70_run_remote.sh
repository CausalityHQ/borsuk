#!/bin/bash
set -u
digest=a4a695d66f508edfbc9ac79cbd3b20e816c4b5f80785c17ee6f9b337f3c477e1
root=/mnt/v70-single-a4a695d6
prefix=s3://borsuk-bench-453182569524-euc1/research/v70-algorithm-first/single-stage-a4a695d66f508edf
output_uri="$prefix/a0001"
terminal_file=/tmp/v70-terminal.json
phase=bootstrap
publish_terminal() {
  code=$?
  trap - EXIT
...
aws s3 cp "$prefix/v70_algorithm_first_single_stage_sq8.py" probe.py --only-show-errors || exit 92
...
set +e
/usr/bin/time -v .venv/bin/python probe.py --source source.parquet --development-query query.parquet   --ground-truth gt.parquet --layout-order layout-order.npy   --bucket borsuk-bench-453182569524-euc1 --prefix research/v70-algorithm-first/single-stage-a4a695d66f508edf/index   --output result.json > run.log 2> time.log
code=$?
set -e
[ "$code" -ne 0 ] && exit "$code"
