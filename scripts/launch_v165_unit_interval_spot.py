#!/usr/bin/env python3
"""Launch one frozen V165 GT-blind unit-interval resource screen on Spot."""
...
SCHEMA = "borsuk-v165-unit-interval-spot-v1"
TAG = "borsuk-v165-unit-interval"
WALL_SECONDS = 3_600
...
EARLY_INPUTS = (
    ...
    ("requests.jsonl", "research/v116-validation-paired/"
     "5e9b35ad40ea023eab4407aa611d759e1893bb34/"
     "runs/v116-validation-20260923T235426Z/a0001/artifacts/requests.jsonl",
     18_726_909, "c250ef3c871af55ee1ea91e61214a93903b3d575ef458926b1f8c5ad0547b2c9"),
...
ARTIFACTS = ("plans.jsonl", "plan-seal.json", "summary.json", "check.json",
             "plan-resources.txt", "check-resources.txt", "run.log")
...
def user_data(commit: str, archive_sha: str, archive_key: str, prefix: str) -> str:
    return f'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v165-hard-stop --on-active={WALL_SECONDS}s /usr/sbin/shutdown -h now
root=/mnt/v165-unit-interval
...
/usr/bin/time -v -o plan-resources.txt .venv/bin/python \
  repo/scripts/v165_unit_interval_resources.py plan \
  --requests requests.jsonl --sealed sealed.jsonl \
  --old-layout old-layout.npy --order order.npy \
  --v164-terminal v164-terminal.json --plans plans.jsonl \
  --plan-seal plan-seal.json --summary summary.json
phase=check
/usr/bin/time -v -o check-resources.txt .venv/bin/python \
  repo/scripts/v165_unit_interval_resources.py check \
  --requests requests.jsonl --sealed sealed.jsonl \
  --old-layout old-layout.npy --order order.npy \
  --v164-terminal v164-terminal.json --plans plans.jsonl \
  --plan-seal plan-seal.json --summary summary.json >check.json
phase=complete
'''
...
def main() -> None:
    ...
    if subprocess.run(["git", "merge-base", "--is-ancestor", "HEAD",
        ...
        raise ValueError("V165 source commit is not pushed to origin/main")
    archive = archive_source(commit)
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as tar:
        required = {"scripts/v165_unit_interval_resources.py",
                    "scripts/launch_v165_unit_interval_spot.py",
                    "docs/research/v165-unit-interval-resource-screen-prereg.md"}
        if not required.issubset(tar.getnames()):
            raise ValueError("source archive lacks V165 gate")
    archive_sha = hashlib.sha256(archive).hexdigest()
    archive_key = f"research/v165-unit-interval/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v165-unit-interval/{commit}/runs/{attempt}"
    ...
    if any(item.get("Instances") for item in active["Reservations"]):
        raise ValueError("V165 worker already active")
    for _, key, size, _ in EARLY_INPUTS:
    ...
    receipt = ec2.run_instances(
        ClientToken="v165-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
        ...
        if not missing(s3, prefix + "/terminal.json"):
            ...
            for name, identity in terminal.get("artifacts", {}).items():
                ...
                if size != identity["bytes"] or digest.hexdigest() != identity["sha256"]:
                    raise ValueError(f"V165 S3 artifact read-back differs: {name}")
            terminal["terminal_sha256"] = hashlib.sha256(raw).hexdigest()
            print(json.dumps(terminal, sort_keys=True), flush=True)
            if terminal.get("status") != "complete":
                raise RuntimeError("V165 terminal reports failed; see closed artifacts")
            return
        state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
        if state in {"terminated", "shutting-down"}:
            if time.monotonic() - started >= WALL_SECONDS - 60:
                raise TimeoutError("V165 worker reached wall cap before terminal")
            raise RuntimeError("Spot worker stopped before terminal; restart full cell")
        time.sleep(20)
    ec2.terminate_instances(InstanceIds=[instance_id])
    raise TimeoutError("V165 cell exceeded wall cap")
