#!/usr/bin/env python3
"""One preregistered 100k persisted PQ graph/FP16 gate on Spot."""
...
SCHEMA = "borsuk-v214-pq-graph-100k-spot-v1"
TAG = "borsuk-v214-pq-graph-100k"
WALL_SECONDS = 10_800
...
if {row[0] for row in INPUTS} != {"source.parquet", "sq8.bin", "requests.jsonl",
                                  ...
                                  "v113-codes.npy"}:
    raise RuntimeError("V214 frozen input roster differs")
...
ARTIFACTS = ("raw.jsonl", "serving.json", "summary.json", "prepare.json",
             "graph-prep.json", "build-summary.json", "graph.bin", "vectors.raw", "plane.bin",
             "old-to-new.u32", "new-to-old.u32", "books.raw",
...
def user_data(commit: str, archive_sha: str, archive_key: str, prefix: str) -> str:
    template = r'''#!/bin/bash
set -euo pipefail
systemd-run --unit=v214-hard-stop --on-active=@@WALL@@s /usr/sbin/shutdown -h now
root=/mnt/v214-pq-graph-100k
...
"$CARGO_HOME/bin/cargo" build --release --locked -p borsuk \
  --bin v214_build_pq_graph_100k --bin v214_serve_pq_graph_100k \
  --jobs 6 >"$root/build.log" 2>&1
cd "$root"
/usr/bin/time -v -o graph-build-resources.txt "$CARGO_TARGET_DIR/release/v214_build_pq_graph_100k" \
  graph-prep.json plane.bin vectors.raw graph.bin build-summary.json
phase=serve
/usr/bin/time -v -o serving-resources.txt "$CARGO_TARGET_DIR/release/v214_serve_pq_graph_100k" \
...
phase=truth
@@LATE_DOWNLOADS@@
.venv/bin/python -m scripts.v214_score_pq_graph_100k \
...
'''
    ...
    if "@@" in template:
        raise ValueError("unresolved V214 worker placeholder")
    return template
...
def launch(attempt: str) -> None:
    ...
    archive_key = f"research/v214-pq-graph-100k/{commit}/sources/{archive_sha}.tar.gz"
    prefix = f"research/v214-pq-graph-100k/{commit}/runs/{attempt}"
    ...
    put_if_absent(prefix + "/reservation.json", json.dumps({
        ...
        "gate": "hits>=25440;p05>=98;whole-Rust-query-p95<=10ms;RSS<=320000000B;0 vector GET",
    ...
    receipt = ec2.run_instances(
        ClientToken="v214-" + hashlib.sha256(prefix.encode()).hexdigest()[:48],
        ...
        while time.monotonic() < deadline:
            try:
                ...
                if (terminal.get("schema") != SCHEMA or terminal.get("instance_id") != instance_id
                    ...
                    raise ValueError("V214 terminal identity differs")
                terminal["terminal_sha256"] = hashlib.sha256(raw).hexdigest()
                print(json.dumps(terminal, sort_keys=True), flush=True)
                if terminal.get("status") != "complete":
                    raise RuntimeError("V214 failed; inspect only closed terminal artifacts")
                if set(terminal.get("artifacts", {})) != set(ARTIFACTS):
                    raise ValueError("V214 artifact roster differs")
                for name, identity in terminal["artifacts"].items():
                    ...
                    if size != identity["bytes"] or value.hexdigest() != identity["sha256"]:
                        raise ValueError(f"V214 artifact readback differs: {name}")
            ...
            if state in {"terminated", "shutting-down"}:
                raise RuntimeError("V214 worker stopped before terminal")
            time.sleep(20)
        raise TimeoutError("V214 Spot cell exceeded wall cap")
    finally:
        try:
            state = ec2.describe_instances(InstanceIds=[instance_id])["Reservations"][0]["Instances"][0]["State"]["Name"]
...
if __name__ == "__main__":
    ...
    with open("/tmp/borsuk-v214-pq-graph-launch.lock", "a+") as lock:
