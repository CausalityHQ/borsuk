...
if [ "${1:-}" = "--describe" ]; then
  printf '%s\n' '{"max_wall_seconds":1200,"queries_per_arm":32,"spot_only":true,"total_arms":2}'
  exit 0
fi
...
root=/mnt/v88-planner-objective
phase=bootstrap
...
publish_terminal() {
  ...
  if [ "$code" -eq 0 ]; then
    for name in dependency.log hashes.log hashes-check.log instance.json run.log time.log result.json; do
      if [ ! -s "$name" ] || ! aws s3 cp "$name" "$V88_OUTPUT_URI/$name" --only-show-errors; then
        code=98
    ...
    for name in dependency.log hashes.log hashes-check.log instance.json run.log time.log result.json; do
      if [ -n "${V88_OUTPUT_URI:-}" ] && [ -f "$name" ]; then
        aws s3 cp "$name" "$V88_OUTPUT_URI/$name" --only-show-errors || true
      fi
    done
  fi
  printf '{"exit_code":%d,"phase":"%s","source_commit":"%s"}\n' \
    "$code" "$phase" "${V88_SOURCE_COMMIT:-unknown}" >/tmp/v88-planner-objective-terminal.json
  if [ -n "${V88_OUTPUT_URI:-}" ] && ! aws s3 cp /tmp/v88-planner-objective-terminal.json \
    "$V88_OUTPUT_URI/terminal.json" --only-show-errors; then
    code=99
  fi
  unlink /tmp/v88-planner-objective-terminal.json
  sync
  shutdown -h now || true
  exit "$code"
}
...
"$0" --classify-lifecycle "$lifecycle" >/dev/null || exit 88
...
phase=inputs
aws s3 cp "$V88_SCREEN_URI" scripts/v88_planner_objective_screen.py --only-show-errors || exit 92
aws s3 cp "$V88_V86_URI" scripts/v86_coarse_to_fine_screen.py --only-show-errors || exit 92
aws s3 cp "$V88_V87_URI" scripts/v87_summary_capacity_screen.py --only-show-errors || exit 92
aws s3 cp "$V88_SHARED_URI" scripts/v85_shared_overlay_screen.py --only-show-errors || exit 92
printf '%s  scripts/v88_planner_objective_screen.py\n%s  scripts/v86_coarse_to_fine_screen.py\n%s  scripts/v87_summary_capacity_screen.py\n%s  scripts/v85_shared_overlay_screen.py\n' \
  "$V88_SCREEN_SHA256" "$V88_V86_SHA256" "$V88_V87_SHA256" "$V88_SHARED_SHA256" >hashes.log
...
/usr/bin/time -v -o time.log timeout --signal=TERM --kill-after=30s 1200s \
  .venv/bin/python -m scripts.v88_planner_objective_screen \
