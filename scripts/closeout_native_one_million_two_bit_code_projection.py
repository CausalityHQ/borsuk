...
from scripts.validate_native_one_million_two_bit_code_projection import validate_closed
...
def closeout_receipts(root: Path, plan: SelectorSpotPlan, instance_id: str) -> dict[str, object]:
    ...
    if (
        ...
        or reservation.get("schema") != "borsuk-one-million-two_bit_code_wave-selector-reservation-v1"
    ...
    validation = json.loads((root / "two-bit-code-wave-validation.json").read_bytes())
    if (
    ...
    return {
        "schema": "borsuk-one-million-two-bit-code-wave-closeout-v1",
        "status": "complete",
...
def closeout_closed_attempt(root: Path, plan: SelectorSpotPlan, instance_id: str) -> dict[str, object]:
    ...
    path = root / "two-bit-code-wave-validation.json"
