//! Local frozen-arm V26 PQ4 serving screen with no page or network execution surface.

use std::{collections::BTreeMap, io::Write, path::PathBuf};

use borsuk_v26::{
    V26ColdVectorManifest, V26LocalObjectPath, V26ObjectIdentity, V26Pq4ServingScreenRequest,
    run_v26_pq4_serving_screen,
};
...
fn parse_args(args: Vec<String>) -> Result<V26Pq4ServingScreenRequest, String> {
    let mut values = BTreeMap::new();
    let mut execute = false;
    let mut args = args.into_iter();
    while let Some(key) = args.next() {
        if key == "--execute-pq4-serving-screen" {
            if execute {
                return Err("duplicate --execute-pq4-serving-screen".to_owned());
            }
            execute = true;
            continue;
        }
    ...
    if !execute {
        return Err("missing --execute-pq4-serving-screen".to_owned());
    }
...
fn run() -> Result<(), String> {
    ...
    let bytes = run_v26_pq4_serving_screen(&request).map_err(|error| error.to_string())?;
    std::io::stdout()
        .write_all(&bytes)
...
mod tests {
    fn valid_args() -> Vec<String> {
        let mut args = vec![
            "--execute-pq4-serving-screen".to_owned(),
            "--generation".to_owned(),
            ...
            "s3://v26/pq4/latency.parquet".to_owned(),
        ];
        for role in [
            "pq4-manifest",
            "cold-vectors",
    ...
    #[test]
    fn v26_pq4_serving_cli_requires_exact_frozen_frontier_authority() {
        let request = super::parse_args(valid_args()).unwrap();
        assert_eq!(request.cold_vectors_manifest.row_count, 9_990_000);
        assert_eq!(
            request.frontier_result.identity.role,
            "pq4-fast-quality-result"
        );
        assert_eq!(request.evidence_output_uri, "s3://v26/pq4/latency.parquet");
    }

    #[test]
    fn v26_pq4_serving_cli_rejects_tuning_storage_page_and_d3_surface() {
        for flag in [
            "--ranked-row-limit",
            "--query-start",
            "--query-count",
