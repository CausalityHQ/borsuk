//! Development transport replay over locally hydrated SQ8 page ranges.
...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() != 4 {
        return Err(invalid(
            "usage: v133_local_sq8_transport prepared-dir output.jsonl summary.json",
        )
        .into());
    }
    ...
    let summary = json!({
        "schema":"borsuk-v133-local-sq8-transport-v1",
        ...
        "qualifies_placement":totals == [99_942,98_827]
            && candidate_p95 <= 2.0 * control_p95 && candidate_p95 <= 75.624654,
