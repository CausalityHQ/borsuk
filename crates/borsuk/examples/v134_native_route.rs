//! Development serial request with native PQ64 routing and physical planning.
//! It preserves V122's fixed method and V133's authenticated local placement.

use borsuk::exact_sq8_mirror::{ExactSq8Mirror, MirrorManifest, Placement};
use borsuk::exact_sq8_nominee::{Sq8Geometry, primary_ordinals};
use borsuk::native_source_id_map::NativeSourceIdMap;
use borsuk::native_source_tier::NativeSourceTier;
use borsuk::physical_interval::plan_weighted_nominee_pages;
...
struct PreparedRoute {
    ...
    nominee_ns: u128,
    planner_ns: u128,
    nominee_order_differs: bool,
    primary_order_differs: bool,
}
...
fn exact_page_spans(
    ...
    if ranges.is_empty() || ranges.len() > MAX_GETS {
        return Err(invalid("physical range count"));
    }
...
fn prepare_candidate_route(
    query_ordinal: usize,
    query: &[f32],
    sealed: &Evidence,
    mirror: &ExactSq8Mirror,
    generation: &ExactServingGeneration<'_>,
) -> Result<(PreparedRoute, Vec<[usize; 2]>), Box<dyn Error>> {
    let started = Instant::now();
    let pages = generation.base().pages();
    let page_count = pages.rows().div_ceil(pages.page_rows());
    let regions = (page_count * 1024).div_ceil(3907);
    let router_started = Instant::now();
    let nominees = generation
        .base()
        .router()
        .router
        .nominate(query, regions, 512)
        .map_err(|error| invalid(format!("query {query_ordinal} router: {error:?}")))?;
    let router_ns = router_started.elapsed().as_nanos();
    if nominees.len() != 512
        || nominees.iter().copied().collect::<HashSet<_>>()
            != sealed.nominees.iter().copied().collect::<HashSet<_>>()
    {
        return Err(invalid(format!("query {query_ordinal} nominee set differs")).into());
    }
    let nominee_started = Instant::now();
    let scored = mirror.score(&nominees, query)?;
    let primary = primary_ordinals(&scored, 100)
        .map_err(|error| invalid(format!("query {query_ordinal} primary: {error:?}")))?;
    let router_ids = scored
        .iter()
        .map(|row| u64::try_from(row.id).map_err(|_| invalid("negative nominee ID")))
        .collect::<Result<Vec<_>, _>>()?;
    let nominee_ns = nominee_started.elapsed().as_nanos();
    if primary.len() != 100
        || primary.iter().copied().collect::<HashSet<_>>()
            != sealed.primary.iter().copied().collect::<HashSet<_>>()
    {
        return Err(invalid(format!("query {query_ordinal} primary set differs")).into());
    }
    let planner_started = Instant::now();
    let (_, plan) = plan_weighted_nominee_pages(
        pages.rows(),
        pages.dimensions(),
        pages.page_rows(),
        &primary,
    ...
    let planner_ns = planner_started.elapsed().as_nanos();
    let ranges = plan
        .ranges
        .iter()
        .map(|range| [range.start, range.end])
        .collect::<Vec<_>>();
    if ranges != sealed.candidate_ranges {
        return Err(invalid(format!("query {query_ordinal} candidate ranges differ")).into());
    }
    Ok((
        PreparedRoute {
            started,
            ...
            nominee_ns,
            planner_ns,
            nominee_order_differs: nominees != sealed.nominees,
            primary_order_differs: primary != sealed.primary,
        },
        ranges,
    ))
}
...
fn run_arm(
    ...
    let total_started = prepared.map_or_else(Instant::now, |route| route.started);
    let (router_ids, mirror_ns) = if let Some(route) = prepared {
        (route.router_ids.clone(), route.nominee_ns)
    } else {
        ...
        let ids = mirror
            ...
            .map(|row| u64::try_from(row.id).map_err(|_| invalid("negative nominee ID")))
            .collect::<Result<Vec<_>, _>>()?;
        (ids, mirror_started.elapsed().as_nanos())
    };
    ...
    let fetched = spans
        .iter()
        .enumerate()
        .map(|(number, &(first, last, bytes))| {
            mirror
                .read_verified_pages(generation.base().pages(), first, last, bytes)
                .map_err(|error| {
                    invalid(format!(
                        "query {query_ordinal} {arm} local range {number} failed: {error:?}"
                    ))
                })
        })
        .collect::<Result<Vec<_>, _>>()?;
    ...
    if response_bytes != spans.iter().map(|span| span.2).sum::<usize>() {
        return Err(invalid("local SQ8 read byte inventory differs").into());
    }
    let sq8_block_reads = fetched
        .iter()
        .map(|range| range.verified_block_reads)
        .sum::<usize>();
    ...
    Ok(json!({
        ...
        "range_reads": ranges.len(), "sq8_read_bytes": response_bytes,
        "sq8_verified_block_reads": sq8_block_reads,
        "mirror_ns": mirror_ns, "sq8_read_ns": read_ns, "source_ns": source_ns,
        "router_ns":prepared.map_or(0, |route| route.router_ns),
        "planner_ns":prepared.map_or(0, |route| route.planner_ns),
        "nominee_order_differs":prepared.is_some_and(|route| route.nominee_order_differs),
        "primary_order_differs":prepared.is_some_and(|route| route.primary_order_differs),
...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    let mut sq8_blocks = [0usize; 2];
    let mut times = [Vec::<u128>::new(), Vec::<u128>::new()];
    let mut read_times = [Vec::<u128>::new(), Vec::<u128>::new()];
    let mut source_times = [Vec::<u128>::new(), Vec::<u128>::new()];
    let mut router_times = Vec::<u128>::new();
    let mut nominee_times = Vec::<u128>::new();
    let mut planner_times = Vec::<u128>::new();
    let mut nominee_order_differences = 0usize;
    let mut primary_order_differences = 0usize;
        ...
        queries.iter().zip(&evidence).zip(&replay).enumerate()
    {
        ...
        for (index, name, ranges, expected) in arms {
            let (prepared, dynamic_ranges) = if index == 0 {
                let (prepared, dynamic_ranges) =
                    prepare_candidate_route(ordinal, &query.query, sealed, &mirror, &generation)?;
                (Some(prepared), Some(dynamic_ranges))
            } else {
                (None, None)
            };
            let effective_ranges: &[[usize; 2]] = dynamic_ranges.as_deref().unwrap_or(ranges);
            let record = run_arm(
                ordinal,
                name,
                &query.query,
            ...
            if index == 0 {
                router_times.push(
                    record["router_ns"]
                        .as_u64()
                        .ok_or_else(|| invalid("router time record"))? as u128,
                );
                nominee_times.push(
                    record["mirror_ns"]
                        .as_u64()
                        .ok_or_else(|| invalid("nominee time record"))? as u128,
                );
                planner_times.push(
                    record["planner_ns"]
                        .as_u64()
                        .ok_or_else(|| invalid("planner time record"))? as u128,
                );
                nominee_order_differences += usize::from(
                    record["nominee_order_differs"]
                        .as_bool()
                        .ok_or_else(|| invalid("nominee order record"))?,
                );
                primary_order_differences += usize::from(
                    record["primary_order_differs"]
                        .as_bool()
                        .ok_or_else(|| invalid("primary order record"))?,
                );
            }
            totals[index] += record["hits"]
                .as_u64()
                .ok_or_else(|| invalid("hit record"))? as usize;
            ranges_read[index] += record["range_reads"]
                .as_u64()
                .ok_or_else(|| invalid("range read record"))?
                as usize;
            bytes[index] += record["sq8_read_bytes"]
                .as_u64()
                .ok_or_else(|| invalid("byte record"))? as usize;
            sq8_blocks[index] += record["sq8_verified_block_reads"]
                .as_u64()
                .ok_or_else(|| invalid("SQ8 block record"))?
                as usize;
            times[index].push(
                record["total_ns"]
                    .as_u64()
            ...
            read_times[index].push(
                record["sq8_read_ns"]
                    .as_u64()
                    .ok_or_else(|| invalid("SQ8 read time record"))? as u128,
            );
            source_times[index].push(
                record["source_ns"]
                    .as_u64()
                    .ok_or_else(|| invalid("source time record"))? as u128,
            );
    ...
    let summary = json!({
        "schema":"borsuk-v134-native-route-v1",
        ...
        "qualifies_serial_route":totals == [99_942,98_827] && candidate_p95 <= 75.624654,
        "nominee_order_differences":nominee_order_differences,
        "primary_order_differences":primary_order_differences,
        "candidate_router_p50_ms":percentile_ms(&router_times,50),
        "candidate_router_p95_ms":percentile_ms(&router_times,95),
        "candidate_router_p99_ms":percentile_ms(&router_times,99),
        "candidate_nominee_p50_ms":percentile_ms(&nominee_times,50),
        "candidate_nominee_p95_ms":percentile_ms(&nominee_times,95),
        "candidate_nominee_p99_ms":percentile_ms(&nominee_times,99),
        "candidate_planner_p50_ms":percentile_ms(&planner_times,50),
        "candidate_planner_p95_ms":percentile_ms(&planner_times,95),
        "candidate_planner_p99_ms":percentile_ms(&planner_times,99),
        "candidate": {"hits":totals[0],"range_reads":ranges_read[0],"sq8_read_bytes":bytes[0],
            "sq8_verified_block_reads":sq8_blocks[0],
            ...
            "sq8_read_p50_ms":percentile_ms(&read_times[0],50),
            "sq8_read_p95_ms":percentile_ms(&read_times[0],95),
            "sq8_read_p99_ms":percentile_ms(&read_times[0],99),
            "source_p50_ms":percentile_ms(&source_times[0],50),
            "source_p95_ms":percentile_ms(&source_times[0],95),
            "source_p99_ms":percentile_ms(&source_times[0],99)},
        "baseline": {"hits":totals[1],"range_reads":ranges_read[1],"sq8_read_bytes":bytes[1],
            "sq8_verified_block_reads":sq8_blocks[1],
            ...
            "sq8_read_p50_ms":percentile_ms(&read_times[1],50),
            "sq8_read_p95_ms":percentile_ms(&read_times[1],95),
            "sq8_read_p99_ms":percentile_ms(&read_times[1],99),
            "source_p50_ms":percentile_ms(&source_times[1],50),
            "source_p95_ms":percentile_ms(&source_times[1],95),
            "source_p99_ms":percentile_ms(&source_times[1],99)},
    });
