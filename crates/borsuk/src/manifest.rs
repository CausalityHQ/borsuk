...
pub struct Manifest {
    ...
    /// V15 full-epoch cell-card serving authority. Mutually exclusive with the
...
mod global_pq_layout_tests {
    ...
    fn offline_v14_base_has_one_codebook_root_and_no_v13_authority() {
        ...
        let ann = index
            ...
            .as_ref()
            .unwrap();
        assert_eq!(ann.layout_version(), 15);
    ...
    pub(crate) fn try_resident_bytes_estimate(&self) -> Result<u64> {
        ...
        if let Some(global_ann) = &self.global_cell_card_ann_ref {
            total = total
                ...
                .ok_or_else(|| {
                    BorsukError::InvalidStorage(
                        "manifest V15 resident RAM budget estimate overflow".into(),
                    )
                })?;
        }
        Ok(total)
