//! Frozen graph/PQ navigation and resident FP16 rerank gate.
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() != 11 {
        return Err("usage: v214_serve_pq_graph_100k PREP BUILD PLANE GRAPH MAP BOOKS CODES REQUESTS RAW SERVING".into());
    }
    ...
    for (index, request) in requests.iter().enumerate() {
        ...
        for (arm, (ef, shortlist)) in ARMS.into_iter().enumerate() {
            ...
            let (ids, visited) = graph.search_pq_with_visits(
                &request.query,
                &plane,
                &pq,
                &old_for_new,
    ...
    fs::write(
        ...
        format!(
            "{}\n",
            json!({
    "schema":"borsuk-v214-pq-graph-100k-v1","dataset":"ReLAION-100k D768",
    ...
    "pq_code_bytes":ROWS*64,"vector_body_gets":0,"wall_ns":wall_ns,
