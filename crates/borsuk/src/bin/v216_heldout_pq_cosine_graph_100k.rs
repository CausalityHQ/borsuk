//! Frozen method-held-out PQ-cosine graph and resident FP16 gate.
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() != 11 {
        return Err("usage: v216_heldout_pq_cosine_graph_100k PREP BUILD PLANE GRAPH MAP BOOKS CODES REQUESTS RAW SERVING".into());
    }
    ...
    let mut visits = [Vec::<u64>::new()];
    let wall = Instant::now();
    for (index, request) in requests.iter().enumerate() {
        if request.query_ordinal != index + 256 || request.query.len() != DIMS {
        ...
        serde_json::to_writer(
            &mut out,
            &json!({"ordinal":index+256,"arms":arms,"vector_body_gets":0}),
        )?;
        out.write_all(b"\n")?;
    }
    out.flush()?;
    ...
    for (arm, (ef, shortlist)) in ARMS.into_iter().enumerate() {
        ...
        "sequential_qps":744.0/(arm_wall_ns as f64/1e9)}),
        );
    }
    fs::write(
        &args[10],
        format!(
            "{}\n",
            json!({
    "schema":"borsuk-v216-pq-cosine-heldout-100k-v1","dataset":"ReLAION-100k D768",
    "split":"method-heldout-ordinals-256-999-prior-used","queries":744,"arms":summaries,
    ...
    "throughput_all_arms_qps":744.0/(wall_ns as f64/1e9),
