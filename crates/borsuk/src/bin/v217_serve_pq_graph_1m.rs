//! Frozen ReLAION-1M whole in-process source-graph/PQ/FP16 request gate.
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() != 11 {
        return Err("usage: v217_serve_pq_graph_1m PREP BUILD PLANE GRAPH MAP BOOKS CODES REQUESTS RAW SERVING".into());
    }
    ...
    if prep["schema"] != "borsuk-v217-graph-1m-preparation-v1"
        || build["schema"] != "borsuk-v217-graph-build-1m-v1"
    ...
    fs::write(
        ...
        format!(
            "{}\n",
            json!({
                "schema":"borsuk-v217-pq-cosine-graph-1m-serving-v1",
