//! GT-blind exact-witness and latency screen for boundary-price certificates.

use std::{collections::BTreeMap, env, error::Error, fs, io::Write, path::Path, time::Instant};

use borsuk::{
    hard_priced_interval::hard_priced_cover,
    relaxed_priced_interval::{FixedCap, get_cap_boundary_certificate, relaxed_priced_cover},
...
const MAX_EXTRA_PRICE: i64 = 65_536;
const MAX_CERT_PROBES: usize = 32;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() != 4 {
        return Err(
            "usage: v205_dual_certificate_preflight WEIGHTS_JSONL PLANS_JSONL RAW_JSONL".into(),
        );
    }
    ...
    if weights.len() != 1000 || plans.len() != 1000 {
        return Err("V205 cohort length differs".into());
    }
    ...
    let mut hard_elapsed = Vec::new();
    let mut cert_elapsed = Vec::new();
    let mut tier_counts = [0_usize; 5];
    let mut total_cert_probes = 0_usize;
    for (index, (source, plan)) in weights.iter().zip(&plans).enumerate() {
        if source.ordinal != index || plan.ordinal != index || !plan.optional_risk.feasible {
            return Err(format!("V205 source/plan ordinal or feasibility at {index}").into());
        }
        let mut weight_map = BTreeMap::new();
        for &[unit, weight] in &source.weights {
            let unit = usize::try_from(unit)?;
            if weight_map.insert(unit, weight).is_some() {
                return Err(format!("V205 duplicate weight at {index}").into());
            }
        }
        ...
        let linear =
            unconstrained_priced_cover(&weight_map, &source.mandatory, 31_250, 1000, 50000)
                .map_err(|error| format!("V205 linear cover at {index}: {error:?}"))?;
        let linear_ns = started.elapsed().as_nanos() as u64;
        linear_elapsed.push(linear_ns);
        let mut unit_ns = 0_u64;
        let mut get_ns = 0_u64;
        let mut hard_ns = 0_u64;
        let mut cert_ns = 0_u64;
        let mut cert_probes = 0_usize;
        let mut cert_penalty = None;
        let (tier, chosen) = if linear.units <= plan.unit_cap && linear.gets <= 32 {
            ...
            .map_err(|error| format!("V205 unit relaxation at {index}: {error:?}"))?;
            ...
            if unit.gets <= 32 {
                ...
                .map_err(|error| format!("V205 GET relaxation at {index}: {error:?}"))?;
                ...
                if get.units <= plan.unit_cap {
                    ...
                    let started = Instant::now();
                    let certificate = get_cap_boundary_certificate(
                        &weight_map,
                        &source.mandatory,
                        31_250,
                        32,
                    ...
                    .map_err(|error| format!("V205 certificate at {index}: {error:?}"))?;
                    cert_ns = started.elapsed().as_nanos() as u64;
                    cert_elapsed.push(cert_ns);
                    cert_probes = certificate.probes;
                    total_cert_probes += cert_probes;
                    cert_penalty = certificate.extra_unit_price;
                    if let Some(cover) = certificate.certified {
                        tier_counts[3] += 1;
                        ("certified", Some(cover))
                    } else {
                        let started = Instant::now();
                        let hard = hard_priced_cover(
                            &weight_map,
                            &source.mandatory,
                            31_250,
                            32,
                        ...
                        .map_err(|error| format!("V205 hard cover at {index}: {error:?}"))?;
                        hard_ns = started.elapsed().as_nanos() as u64;
                        hard_elapsed.push(hard_ns);
                        tier_counts[4] += 1;
                        ("hard", Some(hard))
                    }
                }
        ...
        if let Some(ref candidate) = chosen {
            if !matches(&plan.optional_risk, candidate) {
                return Err(format!("V205 exact feasible witness differs at {index} tier {tier}: actual {:?} vs reference {:?}",
        ...
        writeln!(
            ...
            "cert_ns":cert_ns,"cert_probes":cert_probes,
            "cert_extra_unit_price":cert_penalty,"hard_ns":hard_ns,
