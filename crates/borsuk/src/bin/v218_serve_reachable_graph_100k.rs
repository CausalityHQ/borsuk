//! Frozen ReLAION-100k reachable graph/PQ/FP16 request gate.
...
const ARMS: [(usize, usize); 1] = [(2048, 2048)];
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() != 11 {
        return Err("usage: v218_serve_reachable_graph_100k PREP BUILD PLANE GRAPH MAP BOOKS CODES REQUESTS RAW SERVING".into());
    }
    ...
    {
        return Err("100k graph/PQ serving identity differs".into());
    }
    ...
    let mut exact_times = Vec::<u64>::with_capacity(QUERIES);
    let mut exact_visits = Vec::<u64>::with_capacity(QUERIES);
    ...
    for (index, request) in requests.iter().enumerate() {
        ...
        let (exact_ids, exact_count) =
            graph.search_with_visits(&request.query, &plane, 100, 2048)?;
        let exact_elapsed = exact_start.elapsed().as_nanos() as u64;
        exact_times.push(exact_elapsed);
        exact_visits.push(exact_count as u64);
        serde_json::to_writer(
            &mut raw,
            &json!({"ordinal":index,"arms":arms,
                "exact_fp16_navigation":{"returned_ids":exact_ids,
                    "whole_ns":exact_elapsed,"base_visits":exact_count},
                "vector_body_gets":0}),
        )?;
        raw.write_all(b"\n")?;
    }
    ...
    fs::write(
        ...
        format!(
            "{}\n",
            json!({
                "schema":"borsuk-v218-reachable-graph-100k-serving-v1",
                ...
                "exact_fp16_navigation":{"p50_ns":percentile(&mut exact_times,50),
                    "p90_ns":percentile(&mut exact_times,90),
                    "p95_ns":percentile(&mut exact_times,95),
                    "p99_ns":percentile(&mut exact_times,99),
                    "base_visits_p95":percentile(&mut exact_visits,95)},
