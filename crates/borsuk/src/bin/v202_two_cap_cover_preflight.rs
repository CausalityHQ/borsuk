//! GT-blind exact-witness and latency screen for the complete cap hierarchy.
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() != 4 {
        return Err(
            "usage: v202_two_cap_cover_preflight WEIGHTS_JSONL PLANS_JSONL RAW_JSONL".into(),
        );
    }
    ...
    if weights.len() != 1000 || plans.len() != 1000 {
        return Err("V202 cohort length differs".into());
    }
    ...
    for (index, (source, plan)) in weights.iter().zip(&plans).enumerate() {
        if source.ordinal != index || plan.ordinal != index || !plan.optional_risk.feasible {
            return Err(format!("V202 source/plan ordinal or feasibility at {index}").into());
        }
        let mut weight_map = BTreeMap::new();
        for &[unit, weight] in &source.weights {
            let unit = usize::try_from(unit)?;
            if weight_map.insert(unit, weight).is_some() {
                return Err(format!("V202 duplicate weight at {index}").into());
            }
        }
        ...
        let linear =
            unconstrained_priced_cover(&weight_map, &source.mandatory, 31_250, 1000, 50000)
                .map_err(|error| format!("V202 linear cover at {index}: {error:?}"))?;
        ...
        let (tier, chosen) = if linear.units <= plan.unit_cap && linear.gets <= 32 {
            ...
            .map_err(|error| format!("V202 unit relaxation at {index}: {error:?}"))?;
            ...
            if unit.gets <= 32 {
                ...
                .map_err(|error| format!("V202 GET relaxation at {index}: {error:?}"))?;
                ...
                if get.units <= plan.unit_cap {
                    ...
                    .map_err(|error| format!("V202 hard cover at {index}: {error:?}"))?;
        ...
        if let Some(ref candidate) = chosen {
            if !matches(&plan.optional_risk, candidate) {
                return Err(format!("V202 exact feasible witness differs at {index} tier {tier}: actual {:?} vs reference {:?}",
        ...
        writeln!(
            ...
            "linear_ns":linear_ns,"unit_ns":unit_ns,"get_ns":get_ns,"hard_ns":hard_ns,
